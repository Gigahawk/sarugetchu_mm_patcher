from collections import defaultdict
import sarugetchu_mm_patcher.util as util

# Presumably these encodings never change
BYTES_TO_CHAR_SPECIAL = {
    # Angle brackets as furigana markers
    # TODO: this should really be square brackets [ ]
    b"\x5B": "<",
    b"\x5D": ">",

    # String formatting placeholder (%s)
    b"\x25\x73": "%",
    # Tab characters always seem to get inserted at the end
    # of the line no matter where they are in the string
    # Does not seem to do anything if they show up past the first
    # newline \n
    b"\x09": "\t",
    b"\x0A": "\n",

    # Despite the above codes matching up to ASCII
    # these don't seem to be implmented.
    # The standard alphanums are also not implemented
    #b"\x08": "\b",
    #b"\x0B": "\v",
    #b"\x0C": "\f",
    #b"\x0D": "\r",

    # Seems to be used to indicate a paginated string
    b"\x3E": "\f",

    # No idea what this is supposed to be, seems to show up
    # in some paginated strings.
    # It happens to match the ASCII code for a space,
    # but for convenience I'm using an ASCII space to map
    # to the Japanese space, assign the unit separator char for now
    b"\x20": "\x1F",

}

BYTES_TO_CHAR_DEFAULT = {
    **BYTES_TO_CHAR_SPECIAL,

    # Numerics
    b"\x88\x9F": "1",
    b"\x88\xA0": "2",
    b"\x88\xA1": "3",
    b"\x88\xA2": "4",
    b"\x88\xA3": "5",
    b"\x88\xA4": "6",
    b"\x88\xA5": "7",
    b"\x88\xA6": "8",
    b"\x88\xA7": "9",
    b"\x88\xA8": "0",

    # Capital letters
    b"\x88\xA9": "A",
    b"\x88\xAA": "B",
    b"\x88\xAB": "C",
    b"\x88\xAC": "D",
    b"\x88\xAD": "E",
    b"\x88\xAE": "F",
    b"\x88\xAF": "G",
    b"\x88\xB0": "H",
    b"\x88\xB1": "I",
    b"\x88\xB2": "J",
    b"\x88\xB3": "K",
    b"\x88\xB4": "L",
    b"\x88\xB5": "M",
    b"\x88\xB6": "N",
    b"\x88\xB7": "O",
    b"\x88\xB8": "P",
    b"\x88\xB9": "Q",
    b"\x88\xBA": "R",
    b"\x88\xBB": "S",
    b"\x88\xBC": "T",
    b"\x88\xBD": "U",
    b"\x88\xBE": "V",
    b"\x88\xBF": "W",
    b"\x88\xC0": "X",
    b"\x88\xC1": "Y",
    b"\x88\xC2": "Z",

    # Lowercase letters
    b"\x88\xC3": "a",
    b"\x88\xC4": "b",
    b"\x88\xC5": "c",
    b"\x88\xC6": "d",
    b"\x88\xC7": "e",
    b"\x88\xC8": "f",
    b"\x88\xC9": "g",
    b"\x88\xCA": "h",
    b"\x88\xCB": "i",
    b"\x88\xCC": "j",
    b"\x88\xCD": "k",
    b"\x88\xCE": "l",
    b"\x88\xCF": "m",
    b"\x88\xD0": "n",
    b"\x88\xD1": "o",
    b"\x88\xD2": "p",
    b"\x88\xD3": "q",
    b"\x88\xD4": "r",
    b"\x88\xD5": "s",
    b"\x88\xD6": "t",
    b"\x88\xD7": "u",
    b"\x88\xD8": "v",
    b"\x88\xD9": "w",
    b"\x88\xDA": "x",
    b"\x88\xDB": "y",
    b"\x88\xDC": "z",

    # Kana
    b"\x88\xDD": "あ",
    b"\x88\xDE": "い",
    b"\x88\xDF": "う",
    b"\x88\xE0": "え",
    b"\x88\xE1": "お",
    b"\x88\xE2": "か",
    b"\x88\xE3": "き",
    b"\x88\xE4": "く",
    b"\x88\xE5": "け",
    b"\x88\xE6": "こ",
    b"\x88\xE7": "さ",
    b"\x88\xE8": "し",
    b"\x88\xE9": "す",
    b"\x88\xEA": "せ",
    b"\x88\xEB": "そ",
    b"\x88\xEC": "た",
    b"\x88\xED": "ち",
    b"\x88\xEE": "つ",
    b"\x88\xEF": "て",
    b"\x88\xF0": "と",
    b"\x88\xF1": "な",
    b"\x88\xF2": "に",
    b"\x88\xF3": "ぬ",
    b"\x88\xF4": "ね",
    b"\x88\xF5": "の",
    b"\x88\xF6": "は",
    b"\x88\xF7": "ひ",
    b"\x88\xF8": "ふ",
    b"\x88\xF9": "へ",
    b"\x88\xFA": "ほ",
    b"\x88\xFB": "ま",
    b"\x88\xFC": "み",

    b"\x89\x40": "む",
    b"\x89\x41": "め",
    b"\x89\x42": "も",
    b"\x89\x43": "や",
    b"\x89\x44": "ゆ",
    b"\x89\x45": "よ",
    b"\x89\x46": "ら",
    b"\x89\x47": "り",
    b"\x89\x48": "る",
    b"\x89\x49": "れ",
    b"\x89\x4A": "ろ",
    b"\x89\x4B": "わ",
    b"\x89\x4C": "を",
    b"\x89\x4D": "ん",
    b"\x89\x4E": "ー",
    b"\x89\x4F": "、",
    b"\x89\x50": "。",
    b"\x89\x51": "ぁ",
    b"\x89\x52": "ぃ",
    b"\x89\x53": "ぅ",
    b"\x89\x54": "ぇ",
    b"\x89\x55": "ぉ",
    b"\x89\x56": "が",
    b"\x89\x57": "ぎ",
    b"\x89\x58": "ぐ",
    b"\x89\x59": "げ",
    b"\x89\x5A": "ご",
    b"\x89\x5B": "ざ",
    b"\x89\x5C": "じ",
    b"\x89\x5D": "ず",
    b"\x89\x5E": "ぜ",
    b"\x89\x5F": "ぞ",
    b"\x89\x60": "だ",
    b"\x89\x61": "ぢ",
    b"\x89\x62": "づ",
    b"\x89\x63": "で",
    b"\x89\x64": "ど",
    b"\x89\x65": "っ",
    b"\x89\x66": "ば",
    b"\x89\x67": "び",
    b"\x89\x68": "ぶ",
    b"\x89\x69": "べ",
    b"\x89\x6A": "ぼ",
    b"\x89\x6B": "ぱ",
    b"\x89\x6C": "ぴ",
    b"\x89\x6D": "ぷ",
    b"\x89\x6E": "ぺ",
    b"\x89\x6F": "ぽ",
    b"\x89\x70": "ゃ",
    b"\x89\x71": "ゅ",
    b"\x89\x72": "ょ",
    b"\x89\x73": "ゎ",
    #b"\x89\x74": "　",
    b"\x89\x74": " ",
    b"\x89\x75": "ァ",
    b"\x89\x76": "ア",
    b"\x89\x77": "ィ",
    b"\x89\x78": "イ",
    b"\x89\x79": "ゥ",
    b"\x89\x7A": "ウ",
    b"\x89\x7B": "ェ",
    b"\x89\x7C": "エ",
    b"\x89\x7D": "ォ",
    b"\x89\x7E": "オ",
    # Not a valid token? Prints as ル but probably just a glitch related
    # to forcing the game to parse an invalid token
    #b"\x89\x7F": "??",
    b"\x89\x80": "カ",
    b"\x89\x81": "ガ",
    b"\x89\x82": "キ",
    b"\x89\x83": "ギ",
    b"\x89\x84": "ク",
    b"\x89\x85": "グ",
    b"\x89\x86": "ケ",
    b"\x89\x87": "ゲ",
    b"\x89\x88": "コ",
    b"\x89\x89": "ゴ",
    b"\x89\x8A": "サ",
    b"\x89\x8B": "ザ",
    b"\x89\x8C": "シ",
    b"\x89\x8D": "ジ",
    b"\x89\x8E": "ス",
    b"\x89\x8F": "ズ",
    b"\x89\x90": "セ",
    b"\x89\x91": "ゼ",
    b"\x89\x92": "ソ",
    b"\x89\x93": "ゾ",
    b"\x89\x94": "タ",
    b"\x89\x95": "ダ",
    b"\x89\x96": "チ",
    b"\x89\x97": "ヂ",
    b"\x89\x98": "ッ",
    b"\x89\x99": "ツ",
    b"\x89\x9A": "ヅ",
    b"\x89\x9B": "テ",
    b"\x89\x9C": "デ",
    b"\x89\x9D": "ト",
    b"\x89\x9E": "ド",
    b"\x89\x9F": "ナ",
    b"\x89\xA0": "ニ",
    b"\x89\xA1": "ヌ",
    b"\x89\xA2": "ネ",
    b"\x89\xA3": "ノ",
    b"\x89\xA4": "ハ",
    b"\x89\xA5": "バ",
    b"\x89\xA6": "パ",
    b"\x89\xA7": "ヒ",
    b"\x89\xA8": "ビ",
    b"\x89\xA9": "ピ",
    b"\x89\xAA": "フ",
    b"\x89\xAB": "ブ",
    b"\x89\xAC": "プ",
    b"\x89\xAD": "ヘ",
    b"\x89\xAE": "ベ",
    b"\x89\xAF": "ペ",
    b"\x89\xB0": "ホ",
    b"\x89\xB1": "ボ",
    b"\x89\xB2": "ポ",
    b"\x89\xB3": "マ",
    b"\x89\xB4": "ミ",
    b"\x89\xB5": "ム",
    b"\x89\xB6": "メ",
    b"\x89\xB7": "モ",
    b"\x89\xB8": "ャ",
    b"\x89\xB9": "ヤ",
    b"\x89\xBA": "ュ",
    b"\x89\xBB": "ユ",
    b"\x89\xBC": "ョ",
    b"\x89\xBD": "ヨ",
    b"\x89\xBE": "ラ",
    b"\x89\xBF": "リ",
    b"\x89\xC0": "ル",
    b"\x89\xC1": "レ",
    b"\x89\xC2": "ロ",
    b"\x89\xC3": "ヮ",
    b"\x89\xC4": "ワ",
    b"\x89\xC5": "ヲ",
    b"\x89\xC6": "ン",
    b"\x89\xC7": "ヴ",
    b"\x89\xC8": "ヵ",
    b"\x89\xC9": "ヶ",

    # Punctuation and special chars
    # Playstation button symbols, no real match with Unicode
    # but these should be pretty close
    b"\x89\xCA": "○",
    b"\x89\xCB": "Χ",
    b"\x89\xCC": "□",
    b"\x89\xCD": "△",
    b"\x89\xCE": "!",
    b"\x89\xCF": "?",
    b"\x89\xD0": "(",
    b"\x89\xD1": ")",
    b"\x89\xD2": "*",
    b"\x89\xD3": "+",
    b"\x89\xD4": "-",
    b"\x89\xD5": "&",
    b"\x89\xD6": "@",
    b"\x89\xD7": "⋅",
    b"\x89\xD8": ".",
    b"\x89\xD9": ":",
    b"\x89\xDA": "^",
    b"\x89\xDB": "_",
    b"\x89\xDC": "ω",
    b"\x89\xDD": "Д",
    b"\x89\xDE": "∀",
    b"\x89\xDF": "⊃",
    b"\x89\xE0": "♂",
    b"\x89\xE1": "♀",
    b"\x89\xE2": "♪",
    # Not sure which of these is correct
    b"\x89\xE3": "~",
    b"\x89\xE4": "➝",

    # Kanji
    b"\x89\xE5": "亜",
    b"\x89\xE6": "哀",
    b"\x89\xE7": "愛",
    b"\x89\xE8": "悪",
    b"\x89\xE9": "握",
    b"\x89\xEA": "圧",
    b"\x89\xEB": "安",
    b"\x89\xEC": "暗",
    b"\x89\xED": "案",
    b"\x89\xEE": "以",
    b"\x89\xEF": "位",
    b"\x89\xF0": "依",
    b"\x89\xF1": "偉",
    b"\x89\xF2": "医",
    b"\x89\xF3": "囲",
    b"\x89\xF4": "委",
    b"\x89\xF5": "威",
    b"\x89\xF6": "尉",
    b"\x89\xF7": "意",
    b"\x89\xF8": "慰",
    b"\x89\xF9": "胃",
    b"\x89\xFA": "為",
    b"\x89\xFB": "異",
    b"\x89\xFC": "移",

    b"\x8A\x40": "維",
    b"\x8A\x41": "緯",
    b"\x8A\x42": "衣",
    b"\x8A\x43": "違",
    b"\x8A\x44": "遺",
    b"\x8A\x45": "域",
    b"\x8A\x46": "育",
    b"\x8A\x47": "一",
    b"\x8A\x48": "壱",
    b"\x8A\x49": "逸",
    b"\x8A\x4A": "印",
    b"\x8A\x4B": "員",
    b"\x8A\x4C": "因",
    b"\x8A\x4D": "姻",
    b"\x8A\x4E": "引",
    b"\x8A\x4F": "院",
    b"\x8A\x50": "陰",
    b"\x8A\x51": "隠",
    b"\x8A\x52": "韻",
    b"\x8A\x53": "飲",
    b"\x8A\x54": "右",
    b"\x8A\x55": "宇",
    b"\x8A\x56": "羽",
    b"\x8A\x57": "雨",
    b"\x8A\x58": "運",
    b"\x8A\x59": "雲",
    b"\x8A\x5A": "宮",
    b"\x8A\x5B": "影",
    b"\x8A\x5C": "衛",
    b"\x8A\x5D": "映",
    b"\x8A\x5E": "栄",
    b"\x8A\x5F": "永",
    b"\x8A\x60": "泳",
    b"\x8A\x61": "英",
    b"\x8A\x62": "詠",
    b"\x8A\x63": "鋭",
    b"\x8A\x64": "易",
    b"\x8A\x65": "液",
    b"\x8A\x66": "疫",
    b"\x8A\x67": "益",
    b"\x8A\x68": "駅",
    b"\x8A\x69": "悦",
    b"\x8A\x6A": "謁",
    b"\x8A\x6B": "越",
    b"\x8A\x6C": "閲",
    b"\x8A\x6D": "円",
    b"\x8A\x6E": "園",
    b"\x8A\x6F": "塩",
    b"\x8A\x70": "宴",
    b"\x8A\x71": "延",
    b"\x8A\x72": "援",
    b"\x8A\x73": "沿",
    b"\x8A\x74": "演",
    b"\x8A\x75": "炎",
    b"\x8A\x76": "煙",
    b"\x8A\x77": "猿",
    b"\x8A\x78": "縁",
    b"\x8A\x79": "遠",
    b"\x8A\x7A": "鉛",
    b"\x8A\x7B": "汚",
    b"\x8A\x7C": "凹",
    b"\x8A\x7D": "央",
    b"\x8A\x7E": "奥",
    b"\x8A\x7F": "??",  # Appears unused
    b"\x8A\x80": "往",
    b"\x8A\x81": "応",
    b"\x8A\x82": "押",
    b"\x8A\x83": "桜",
    b"\x8A\x84": "横",
    b"\x8A\x85": "欧",
    b"\x8A\x86": "殴",
    b"\x8A\x87": "王",
    b"\x8A\x88": "翁",
    b"\x8A\x89": "億",
    b"\x8A\x8A": "屋",
    b"\x8A\x8B": "憶",
    b"\x8A\x8C": "乙",
    b"\x8A\x8D": "恩",
    b"\x8A\x8E": "温",
    b"\x8A\x8F": "穏",
    b"\x8A\x90": "音",
    b"\x8A\x91": "下",
    b"\x8A\x92": "化",
    b"\x8A\x93": "仮",
    b"\x8A\x94": "何",
    b"\x8A\x95": "価",
    b"\x8A\x96": "佳",
    b"\x8A\x97": "加",
    b"\x8A\x98": "可",
    b"\x8A\x99": "夏",
    b"\x8A\x9A": "嫁",
    b"\x8A\x9B": "家",
    b"\x8A\x9C": "寡",
    b"\x8A\x9D": "暇",
    b"\x8A\x9E": "果",
    b"\x8A\x9F": "架",
    b"\x8A\xA0": "歌",
    b"\x8A\xA1": "河",
    b"\x8A\xA2": "渦",
    b"\x8A\xA3": "火",
    b"\x8A\xA4": "禍",
    b"\x8A\xA5": "科",
    b"\x8A\xA6": "稼",
    b"\x8A\xA7": "箇",
    b"\x8A\xA8": "花",
    b"\x8A\xA9": "荷",
    b"\x8A\xAA": "華",
    b"\x8A\xAB": "菓",
    b"\x8A\xAC": "蚊",
    b"\x8A\xAD": "課",
    b"\x8A\xAE": "貨",
    b"\x8A\xAF": "過",
    b"\x8A\xB0": "靴",
    b"\x8A\xB1": "画",
    b"\x8A\xB2": "我",
    b"\x8A\xB3": "芽",
    b"\x8A\xB4": "賀",
    b"\x8A\xB5": "雅",
    b"\x8A\xB6": "餓",
    b"\x8A\xB7": "介",
    b"\x8A\xB8": "会",
    b"\x8A\xB9": "回",
    b"\x8A\xBA": "塊",
    b"\x8A\xBB": "壊",
    b"\x8A\xBC": "快",
    b"\x8A\xBD": "怪",
    b"\x8A\xBE": "悔",
    b"\x8A\xBF": "懐",
    b"\x8A\xC0": "戒",
    b"\x8A\xC1": "拐",
    b"\x8A\xC2": "改",
    b"\x8A\xC3": "械",
    b"\x8A\xC4": "海",
    b"\x8A\xC5": "灰",
    b"\x8A\xC6": "界",
    b"\x8A\xC7": "皆",
    b"\x8A\xC8": "絵",
    b"\x8A\xC9": "解",
    b"\x8A\xCA": "開",
    b"\x8A\xCB": "階",
    b"\x8A\xCC": "劾",
    b"\x8A\xCD": "外",
    b"\x8A\xCE": "害",
    b"\x8A\xCF": "街",
    b"\x8A\xD0": "慨",
    b"\x8A\xD1": "概",
    b"\x8A\xD2": "涯",
    b"\x8A\xD3": "該",
    b"\x8A\xD4": "各",
    b"\x8A\xD5": "??",  # Appears unused
    b"\x8A\xD6": "拡",
    b"\x8A\xD7": "格",
    b"\x8A\xD8": "核",
    b"\x8A\xD9": "殻",
    b"\x8A\xDA": "獲",
    b"\x8A\xDB": "確",
    b"\x8A\xDC": "穫",
    b"\x8A\xDD": "覚",
    b"\x8A\xDE": "角",
    b"\x8A\xDF": "較",
    b"\x8A\xE0": "郭",
    b"\x8A\xE1": "閣",
    b"\x8A\xE2": "隔",
    b"\x8A\xE3": "革",
    b"\x8A\xE4": "学",
    b"\x8A\xE5": "岳",
    b"\x8A\xE6": "楽",
    b"\x8A\xE7": "額",
    b"\x8A\xE8": "割",
    b"\x8A\xE9": "喝",
    b"\x8A\xEA": "括",
    b"\x8A\xEB": "活",
    b"\x8A\xEC": "渇",
    b"\x8A\xED": "滑",
    b"\x8A\xEE": "褐",
    b"\x8A\xEF": "轄",
    b"\x8A\xF0": "乾",
    b"\x8A\xF1": "冠",
    b"\x8A\xF2": "刊",
    b"\x8A\xF3": "勘",
    b"\x8A\xF4": "勧",
    b"\x8A\xF5": "喚",
    b"\x8A\xF6": "堪",
    b"\x8A\xF7": "完",
    b"\x8A\xF8": "官",
    b"\x8A\xF9": "寒",
    b"\x8A\xFA": "寛",
    b"\x8A\xFB": "巻",
    b"\x8A\xFC": "干",

    b"\x8B\x40": "幹",
    b"\x8B\x41": "患",
    b"\x8B\x42": "感",
    b"\x8B\x43": "慣",
    b"\x8B\x44": "憾",
    b"\x8B\x45": "換",
    b"\x8B\x46": "敢",
    b"\x8B\x47": "肝",
    b"\x8B\x48": "??",  # Appears unused
    b"\x8B\x49": "款",
    b"\x8B\x4A": "歓",
    b"\x8B\x4B": "汗",
    b"\x8B\x4C": "漢",
    b"\x8B\x4D": "環",
    b"\x8B\x4E": "甘",
    b"\x8B\x4F": "監",
    b"\x8B\x50": "看",
    b"\x8B\x51": "管",
    b"\x8B\x52": "簡",
    b"\x8B\x53": "??",  # Appears unused
    b"\x8B\x54": "缶",
    b"\x8B\x55": "艦",
    b"\x8B\x56": "??",  # Appears unused
    b"\x8B\x57": "??",  # Appears unused
    b"\x8B\x58": "還",
    b"\x8B\x59": "鑑",
    b"\x8B\x5A": "間",
    b"\x8B\x5B": "閑",
    b"\x8B\x5C": "関",
    b"\x8B\x5D": "陥",
    b"\x8B\x5E": "館",
    b"\x8B\x5F": "丸",
    b"\x8B\x60": "含",
    b"\x8B\x61": "岸",
    b"\x8B\x62": "岩",
    b"\x8B\x63": "眼",
    b"\x8B\x64": "頑",
    b"\x8B\x65": "顔",
    b"\x8B\x66": "願",
    b"\x8B\x67": "企",
    b"\x8B\x68": "危",
    b"\x8B\x69": "喜",
    b"\x8B\x6A": "器",
    b"\x8B\x6B": "基",
    b"\x8B\x6C": "奇",
    b"\x8B\x6D": "季",
    b"\x8B\x6E": "寄",
    b"\x8B\x6F": "岐",
    b"\x8B\x70": "希",
    b"\x8B\x71": "帰",
    b"\x8B\x72": "??",  # Appears unused
    b"\x8B\x73": "忌",
    b"\x8B\x74": "揮",
    b"\x8B\x75": "旗",
    b"\x8B\x76": "既",
    b"\x8B\x77": "期",
    b"\x8B\x78": "机",
    b"\x8B\x79": "??",  # Appears unused
    b"\x8B\x7A": "棄",
    b"\x8B\x7B": "機",
    b"\x8B\x7C": "気",
    b"\x8B\x7D": "汽",
    b"\x8B\x7E": "祈",
    b"\x8B\x7F": "??",  # Appears unused
    b"\x8B\x80": "紀",
    b"\x8B\x81": "規",
    b"\x8B\x82": "記",
    b"\x8B\x83": "??",  # Appears unused
    b"\x8B\x84": "起",
    b"\x8B\x85": "軌",
    b"\x8B\x86": "輝",
    b"\x8B\x87": "??",  # Appears unused
    b"\x8B\x88": "騎",
    b"\x8B\x89": "鬼",
    b"\x8B\x8A": "偽",
    b"\x8B\x8B": "儀",  # Shows up more than once???
    b"\x8B\x8C": "??",  # Appears unused
    b"\x8B\x8D": "??",  # Appears unused
    b"\x8B\x8E": "技",
    b"\x8B\x8F": "??",  # Appears unused
    b"\x8B\x90": "??",  # Appears unused
    b"\x8B\x91": "犠",
    b"\x8B\x92": "疑",
    b"\x8B\x93": "義",
    b"\x8B\x94": "議",
    b"\x8B\x95": "菊",
    b"\x8B\x96": "吉",
    b"\x8B\x97": "喫",
    b"\x8B\x98": "詰",
    b"\x8B\x99": "却",
    b"\x8B\x9A": "客",
    b"\x8B\x9B": "脚",
    b"\x8B\x9C": "虐",
    b"\x8B\x9D": "逆",
    b"\x8B\x9E": "丘",
    b"\x8B\x9F": "久",
    b"\x8B\xA0": "及",
    b"\x8B\xA1": "九",
    b"\x8B\xA2": "休",
    b"\x8B\xA3": "吸",
    b"\x8B\xA4": "宮",
    b"\x8B\xA5": "弓",
    b"\x8B\xA6": "急",
    b"\x8B\xA7": "救",
    b"\x8B\xA8": "旧",
    b"\x8B\xA9": "朽",
    b"\x8B\xAA": "求",
    b"\x8B\xAB": "泣",
    b"\x8B\xAC": "球",
    b"\x8B\xAD": "究",
    b"\x8B\xAE": "??",  # Appears unused
    b"\x8B\xAF": "級",
    b"\x8B\xB0": "??",  # Appears unused
    b"\x8B\xB1": "給",
    b"\x8B\xB2": "牛",
    b"\x8B\xB3": "巨",
    b"\x8B\xB4": "去",
    b"\x8B\xB5": "居",
    b"\x8B\xB6": "??",  # Appears unused
    b"\x8B\xB7": "拒",
    b"\x8B\xB8": "拠",
    b"\x8B\xB9": "虚",
    b"\x8B\xBA": "許",
    b"\x8B\xBB": "距",
    b"\x8B\xBC": "御",
    b"\x8B\xBD": "漁",
    b"\x8B\xBE": "魚",
    b"\x8B\xBF": "京",
    b"\x8B\xC0": "享",
    b"\x8B\xC1": "供",
    b"\x8B\xC2": "共",
    b"\x8B\xC3": "凶",
    b"\x8B\xC4": "協",
    b"\x8B\xC5": "叫",
    b"\x8B\xC6": "境",
    b"\x8B\xC7": "??",  # Appears unused
    b"\x8B\xC8": "強",
    b"\x8B\xC9": "恭",
    b"\x8B\xCA": "恐",
    b"\x8B\xCB": "挟",
    b"\x8B\xCC": "教",
    b"\x8B\xCD": "胸",
    b"\x8B\xCE": "脅",
    b"\x8B\xCF": "橋",
    b"\x8B\xD0": "況",
    b"\x8B\xD1": "??",  # Appears unused
    b"\x8B\xD2": "狭",
    b"\x8B\xD3": "??",  # Appears unused
    b"\x8B\xD4": "競",
    b"\x8B\xD5": "郷",
    b"\x8B\xD6": "鏡",
    b"\x8B\xD7": "響",
    b"\x8B\xD8": "??",  # Appears unused
    b"\x8B\xD9": "仰",
    b"\x8B\xDA": "局",
    b"\x8B\xDB": "曲",
    b"\x8B\xDC": "極",
    b"\x8B\xDD": "凝",
    b"\x8B\xDE": "??",  # Appears unused
    b"\x8B\xDF": "業",
    b"\x8B\xE0": "玉",
    b"\x8B\xE1": "勤",
    b"\x8B\xE2": "均",
    b"\x8B\xE3": "??",  # Appears unused
    b"\x8B\xE4": "琴",
    b"\x8B\xE5": "禁",
    b"\x8B\xE6": "筋",
    b"\x8B\xE7": "緊",
    b"\x8B\xE8": "菌",
    b"\x8B\xE9": "??",  # Appears unused
    b"\x8B\xEA": "??",  # Appears unused
    b"\x8B\xEB": "近",
    b"\x8B\xEC": "金",
    b"\x8B\xED": "??",  # Appears unused
    b"\x8B\xEE": "??",  # Appears unused
    b"\x8B\xEF": "区",
    b"\x8B\xF0": "苦",
    b"\x8B\xF1": "駆",
    b"\x8B\xF2": "具",
    b"\x8B\xF3": "愚",
    b"\x8B\xF4": "空",
    b"\x8B\xF5": "偶",
    b"\x8B\xF6": "遇",
    b"\x8B\xF7": "??",  # Appears unused
    b"\x8B\xF8": "屈",
    b"\x8B\xF9": "掘",
    b"\x8B\xFA": "君",
    b"\x8B\xFB": "勲",
    b"\x8B\xFC": "??",  # Appears unused

    b"\x8C\x40": "訓",
    b"\x8C\x41": "群",
    b"\x8C\x42": "軍",
    b"\x8C\x43": "郡",
    b"\x8C\x44": "句",
    b"\x8C\x45": "係",
    b"\x8C\x46": "傾",
    b"\x8C\x47": "兄",
    b"\x8C\x48": "刑",
    b"\x8C\x49": "??",
    b"\x8C\x4A": "型",
    b"\x8C\x4B": "契",
    b"\x8C\x4C": "形",
    b"\x8C\x4D": "径",
    b"\x8C\x4E": "恵",
    b"\x8C\x4F": "??",
    b"\x8C\x50": "??",
    b"\x8C\x51": "掲",
    b"\x8C\x52": "携",
    b"\x8C\x53": "??",
    b"\x8C\x54": "景",
    b"\x8C\x55": "渓",
    b"\x8C\x56": "系",
    b"\x8C\x57": "経",
    b"\x8C\x58": "継",
    b"\x8C\x59": "茎",
    b"\x8C\x5A": "??",
    b"\x8C\x5B": "計",
    b"\x8C\x5C": "警",
    b"\x8C\x5D": "軽",
    b"\x8C\x5E": "鶏",
    b"\x8C\x5F": "芸",
    b"\x8C\x60": "迎",
    b"\x8C\x61": "??",
    b"\x8C\x62": "劇",
    b"\x8C\x63": "撃",
    b"\x8C\x64": "激",
    b"\x8C\x65": "傑",
    b"\x8C\x66": "欠",
    b"\x8C\x67": "決",
    b"\x8C\x68": "潔",
    b"\x8C\x69": "穴",
    b"\x8C\x6A": "結",
    b"\x8C\x6B": "血",
    b"\x8C\x6C": "月",
    b"\x8C\x6D": "件",
    b"\x8C\x6E": "??",
    b"\x8C\x6F": "健",
    b"\x8C\x70": "兼",
    b"\x8C\x71": "券",
    b"\x8C\x72": "剣",
    b"\x8C\x73": "圏",
    b"\x8C\x74": "堅",
    b"\x8C\x75": "嫌",
    b"\x8C\x76": "建",
    b"\x8C\x77": "??",
    b"\x8C\x78": "懸",
    b"\x8C\x79": "肩",
    b"\x8C\x7A": "検",
    b"\x8C\x7B": "権",
    b"\x8C\x7C": "犬",
    b"\x8C\x7D": "献",
    b"\x8C\x7E": "県",
    b"\x8C\x7F": "??",
    b"\x8C\x80": "研",
    b"\x8C\x81": "絹",
    b"\x8C\x82": "??",
    b"\x8C\x83": "見",
    b"\x8C\x84": "??",
    b"\x8C\x85": "賢",
    b"\x8C\x86": "軒",
    b"\x8C\x87": "??",
    b"\x8C\x88": "険",
    b"\x8C\x89": "顕",
    b"\x8C\x8A": "験",
    b"\x8C\x8B": "元",
    b"\x8C\x8C": "原",
    b"\x8C\x8D": "厳",
    b"\x8C\x8E": "幻",
    b"\x8C\x8F": "弦",
    b"\x8C\x90": "減",
    b"\x8C\x91": "源",
    b"\x8C\x92": "玄",
    b"\x8C\x93": "現",
    b"\x8C\x94": "言",
    b"\x8C\x95": "限",
    b"\x8C\x96": "個",
    b"\x8C\x97": "古",
    b"\x8C\x98": "呼",
    b"\x8C\x99": "固",
    b"\x8C\x9A": "孤",
    b"\x8C\x9B": "己",
    b"\x8C\x9C": "庫",
    b"\x8C\x9D": "弧",
    b"\x8C\x9E": "戸",
    b"\x8C\x9F": "故",
    b"\x8C\xA0": "枯",
    b"\x8C\xA1": "湖",
    b"\x8C\xA2": "??",
    b"\x8C\xA3": "雇",
    b"\x8C\xA4": "??",
    b"\x8C\xA5": "鼓",
    b"\x8C\xA6": "五",
    b"\x8C\xA7": "互",
    b"\x8C\xA8": "午",
    b"\x8C\xA9": "??",
    b"\x8C\xAA": "娯",
    b"\x8C\xAB": "後",
    b"\x8C\xAC": "悟",
    b"\x8C\xAD": "??",
    b"\x8C\xAE": "語",
    b"\x8C\xAF": "誤",
    b"\x8C\xB0": "護",
    b"\x8C\xB1": "交",
    b"\x8C\xB2": "??",
    b"\x8C\xB3": "候",
    b"\x8C\xB4": "光",
    b"\x8C\xB5": "公",
    b"\x8C\xB6": "功",
    b"\x8C\xB7": "効",
    b"\x8C\xB8": "厚",
    b"\x8C\xB9": "口",
    b"\x8C\xBA": "向",
    b"\x8C\xBB": "后",
    b"\x8C\xBC": "??",
    b"\x8C\xBD": "好",
    b"\x8C\xBE": "孔",
    b"\x8C\xBF": "工",
    b"\x8C\xC0": "巧",
    b"\x8C\xC1": "幸",
    b"\x8C\xC2": "広",
    b"\x8C\xC3": "康",
    b"\x8C\xC4": "行",
    b"\x8C\xC5": "衡",
    b"\x8C\xC6": "恒",
    b"\x8C\xC7": "慌",
    b"\x8C\xC8": "抗",
    b"\x8C\xC9": "拘",
    b"\x8C\xCA": "控",
    b"\x8C\xCB": "攻",
    b"\x8C\xCC": "更",
    b"\x8C\xCD": "??",
    b"\x8C\xCE": "校",
    b"\x8C\xCF": "構",
    b"\x8C\xD0": "江",
    b"\x8C\xD1": "洪",
    b"\x8C\xD2": "港",
    b"\x8C\xD3": "溝",
    b"\x8C\xD4": "甲",
    b"\x8C\xD5": "皇",
    b"\x8C\xD6": "硬",
    b"\x8C\xD7": "稿",
    b"\x8C\xD8": "紅",
    b"\x8C\xD9": "絞",
    b"\x8C\xDA": "綱",
    b"\x8C\xDB": "考",
    b"\x8C\xDC": "孝",
    b"\x8C\xDD": "耕",
    b"\x8C\xDE": "興",
    b"\x8C\xDF": "航",
    b"\x8C\xE0": "荒",
    b"\x8C\xE1": "講",
    b"\x8C\xE2": "貢",
    b"\x8C\xE3": "購",
    b"\x8C\xE4": "郊",
    b"\x8C\xE5": "酵",
    b"\x8C\xE6": "鉱",
    b"\x8C\xE7": "鋼",
    b"\x8C\xE8": "降",
    b"\x8C\xE9": "項",
    b"\x8C\xEA": "香",
    b"\x8C\xEB": "高",
    b"\x8C\xEC": "黄",
    b"\x8C\xED": "剛",
    b"\x8C\xEE": "号",
    b"\x8C\xEF": "合",
    b"\x8C\xF0": "??",
    b"\x8C\xF1": "豪",
    b"\x8C\xF2": "克",
    b"\x8C\xF3": "刻",
    b"\x8C\xF4": "告",
    b"\x8C\xF5": "国",
    b"\x8C\xF6": "穀",
    b"\x8C\xF7": "??",
    b"\x8C\xF8": "酷",
    b"\x8C\xF9": "黒",
    b"\x8C\xFA": "??",
    b"\x8C\xFB": "骨",
    b"\x8C\xFC": "今",

    b"\x8D\x40": "困",
    b"\x8D\x41": "??",
    b"\x8D\x42": "婚",
    b"\x8D\x43": "??",
    b"\x8D\x44": "恨",
    b"\x8D\x45": "昆",
    b"\x8D\x46": "根",
    b"\x8D\x47": "混",
    b"\x8D\x48": "紺",
    b"\x8D\x49": "魂",
    b"\x8D\x4A": "佐",
    b"\x8D\x4B": "唆",
    b"\x8D\x4C": "左",
    b"\x8D\x4D": "差",
    b"\x8D\x4E": "??",
    b"\x8D\x4F": "砂",
    b"\x8D\x50": "詐",
    b"\x8D\x51": "鎖",
    b"\x8D\x52": "座",
    b"\x8D\x53": "催",
    b"\x8D\x54": "債",
    b"\x8D\x55": "再",
    b"\x8D\x56": "妻",
    b"\x8D\x57": "??",
    b"\x8D\x58": "彩",
    b"\x8D\x59": "才",
    b"\x8D\x5A": "採",
    b"\x8D\x5B": "最",
    b"\x8D\x5C": "栽",
    b"\x8D\x5D": "済",
    b"\x8D\x5E": "災",
    b"\x8D\x5F": "砕",
    b"\x8D\x60": "祭",
    b"\x8D\x61": "細",
    b"\x8D\x62": "菜",
    b"\x8D\x63": "裁",
    b"\x8D\x64": "載",
    b"\x8D\x65": "際",
    b"\x8D\x66": "斎",
    b"\x8D\x67": "剤",
    b"\x8D\x68": "在",
    b"\x8D\x69": "材",
    b"\x8D\x6A": "罪",
    b"\x8D\x6B": "財",
    b"\x8D\x6C": "??",
    b"\x8D\x6D": "作",
    b"\x8D\x6E": "削",
    b"\x8D\x6F": "搾",
    b"\x8D\x70": "昨",
    b"\x8D\x71": "策",
    b"\x8D\x72": "索",
    b"\x8D\x73": "酢",
    b"\x8D\x74": "錯",
    b"\x8D\x75": "冊",
    b"\x8D\x76": "??",
    b"\x8D\x77": "察",
    b"\x8D\x78": "撮",
    b"\x8D\x79": "擦",
    b"\x8D\x7A": "札",
    b"\x8D\x7B": "殺",
    b"\x8D\x7C": "雑",
    b"\x8D\x7D": "三",
    b"\x8D\x7E": "??",
    b"\x8D\x7F": "??",
    b"\x8D\x80": "参",
    b"\x8D\x81": "山",
    b"\x8D\x82": "惨",
    b"\x8D\x83": "散",
    b"\x8D\x84": "??",
    b"\x8D\x85": "産",
    b"\x8D\x86": "算",
    b"\x8D\x87": "??",
    b"\x8D\x88": "??",
    b"\x8D\x89": "酸",
    b"\x8D\x8A": "??",
    b"\x8D\x8B": "残",
    b"\x8D\x8C": "仕",
    b"\x8D\x8D": "伺",
    b"\x8D\x8E": "使",
    b"\x8D\x8F": "刺",
    b"\x8D\x90": "史",
    b"\x8D\x91": "司",
    b"\x8D\x92": "嗣",
    b"\x8D\x93": "四",
    b"\x8D\x94": "土",
    b"\x8D\x95": "始",
    b"\x8D\x96": "姉",
    b"\x8D\x97": "姿",
    b"\x8D\x98": "子",
    b"\x8D\x99": "市",
    b"\x8D\x9A": "師",
    b"\x8D\x9B": "志",
    b"\x8D\x9C": "思",
    b"\x8D\x9D": "指",
    b"\x8D\x9E": "支",
    b"\x8D\x9F": "施",
    b"\x8D\xA0": "旨",
    b"\x8D\xA1": "肢",
    b"\x8D\xA2": "脂",
    b"\x8D\xA3": "枝",
    b"\x8D\xA4": "止",
    b"\x8D\xA5": "死",
    b"\x8D\xA6": "氏",
    b"\x8D\xA7": "矢",
    b"\x8D\xA8": "祉",
    b"\x8D\xA9": "私",
    b"\x8D\xAA": "糸",
    b"\x8D\xAB": "紙",
    b"\x8D\xAC": "??",
    b"\x8D\xAD": "至",
    b"\x8D\xAE": "視",
    b"\x8D\xAF": "詞",
    b"\x8D\xB0": "詩",
    b"\x8D\xB1": "試",
    b"\x8D\xB2": "誌",
    b"\x8D\xB3": "??",
    b"\x8D\xB4": "資",
    b"\x8D\xB5": "賜",
    b"\x8D\xB6": "雌",
    b"\x8D\xB7": "飼",
    b"\x8D\xB8": "歯",
    b"\x8D\xB9": "事",
    b"\x8D\xBA": "似",
    b"\x8D\xBB": "侍",
    b"\x8D\xBC": "児",
    b"\x8D\xBD": "字",
    b"\x8D\xBE": "寺",
    b"\x8D\xBF": "慈",
    b"\x8D\xC0": "持",
    b"\x8D\xC1": "時",
    b"\x8D\xC2": "次",
    b"\x8D\xC3": "治",
    b"\x8D\xC4": "滋",
    b"\x8D\xC5": "??",
    b"\x8D\xC6": "磁",
    b"\x8D\xC7": "示",
    b"\x8D\xC8": "耳",
    b"\x8D\xC9": "自",
    b"\x8D\xCA": "辞",
    b"\x8D\xCB": "式",
    b"\x8D\xCC": "識",
    b"\x8D\xCD": "軸",
    b"\x8D\xCE": "七",
    b"\x8D\xCF": "執",
    b"\x8D\xD0": "失",
    b"\x8D\xD1": "室",
    b"\x8D\xD2": "??",
    b"\x8D\xD3": "漆",
    b"\x8D\xD4": "疾",
    b"\x8D\xD5": "質",
    b"\x8D\xD6": "実",
    b"\x8D\xD7": "??",
    b"\x8D\xD8": "写",
    b"\x8D\xD9": "射",
    b"\x8D\xDA": "捨",
    b"\x8D\xDB": "赦",
    b"\x8D\xDC": "斜",
    b"\x8D\xDD": "??",
    b"\x8D\xDE": "社",
    b"\x8D\xDF": "者",
    b"\x8D\xE0": "謝",
    b"\x8D\xE1": "車",
    b"\x8D\xE2": "遮",
    b"\x8D\xE3": "蛇",
    b"\x8D\xE4": "邪",
    b"\x8D\xE5": "借",
    b"\x8D\xE6": "??",
    b"\x8D\xE7": "尺",
    b"\x8D\xE8": "??",
    b"\x8D\xE9": "??",
    b"\x8D\xEA": "釈",
    b"\x8D\xEB": "寂",
    b"\x8D\xEC": "弱",
    b"\x8D\xED": "若",
    b"\x8D\xEE": "主",
    b"\x8D\xEF": "取",
    b"\x8D\xF0": "守",
    b"\x8D\xF1": "手",
    b"\x8D\xF2": "朱",
    b"\x8D\xF3": "殊",
    b"\x8D\xF4": "狩",
    b"\x8D\xF5": "珠",
    b"\x8D\xF6": "種",
    b"\x8D\xF7": "趣",
    b"\x8D\xF8": "酒",
    b"\x8D\xF9": "首",
    b"\x8D\xFA": "??",
    b"\x8D\xFB": "受",
    b"\x8D\xFC": "寿",

    b"\x8E\x40": "授",
    b"\x8E\x41": "樹",
    b"\x8E\x42": "需",
    b"\x8E\x43": "収",
    b"\x8E\x44": "周",
    b"\x8E\x45": "囚",
    b"\x8E\x46": "宗",
    b"\x8E\x47": "就",
    b"\x8E\x48": "州",
    b"\x8E\x49": "修",
    b"\x8E\x4A": "??",
    b"\x8E\x4B": "拾",
    b"\x8E\x4C": "秀",
    b"\x8E\x4D": "秋",
    b"\x8E\x4E": "終",
    b"\x8E\x4F": "習",
    b"\x8E\x50": "臭",
    b"\x8E\x51": "舟",
    b"\x8E\x52": "衆",
    b"\x8E\x53": "襲",
    b"\x8E\x54": "週",
    b"\x8E\x55": "??",
    b"\x8E\x56": "集",
    b"\x8E\x57": "??",
    b"\x8E\x58": "住",
    b"\x8E\x59": "充",
    b"\x8E\x5A": "十",
    b"\x8E\x5B": "従",
    b"\x8E\x5C": "柔",
    b"\x8E\x5D": "汁",
    b"\x8E\x5E": "渋",
    b"\x8E\x5F": "獣",
    b"\x8E\x60": "縦",
    b"\x8E\x61": "重",
    b"\x8E\x62": "銃",
    b"\x8E\x63": "叔",
    b"\x8E\x64": "宿",
    b"\x8E\x65": "??",
    b"\x8E\x66": "祝",
    b"\x8E\x67": "縮",
    b"\x8E\x68": "??",
    b"\x8E\x69": "??",
    b"\x8E\x6A": "??",
    b"\x8E\x6B": "出",
    b"\x8E\x6C": "術",
    b"\x8E\x6D": "述",
    b"\x8E\x6E": "俊",
    b"\x8E\x6F": "春",
    b"\x8E\x70": "瞬",
    b"\x8E\x71": "准",
    b"\x8E\x72": "循",
    b"\x8E\x73": "旬",
    b"\x8E\x74": "??",
    b"\x8E\x75": "準",
    b"\x8E\x76": "潤",
    b"\x8E\x77": "盾",
    b"\x8E\x78": "純",
    b"\x8E\x79": "巡",
    b"\x8E\x7A": "遵",
    b"\x8E\x7B": "順",
    b"\x8E\x7C": "処",
    b"\x8E\x7D": "初",
    b"\x8E\x7E": "??",
    b"\x8E\x7F": "??",
    b"\x8E\x80": "所",
    b"\x8E\x81": "書",
    b"\x8E\x82": "??",
    b"\x8E\x83": "緒",
    b"\x8E\x84": "??",
    b"\x8E\x85": "諸",
    b"\x8E\x86": "助",
    b"\x8E\x87": "叙",
    b"\x8E\x88": "女",
    b"\x8E\x89": "如",
    b"\x8E\x8A": "序",
    b"\x8E\x8B": "徐",
    b"\x8E\x8C": "除",
    b"\x8E\x8D": "商",
    b"\x8E\x8E": "傷",
    b"\x8E\x8F": "償",
    b"\x8E\x90": "勝",
    b"\x8E\x91": "匠",
    b"\x8E\x92": "召",
    b"\x8E\x93": "唱",
    b"\x8E\x94": "??",
    b"\x8E\x95": "??",
    b"\x8E\x96": "将",
    b"\x8E\x97": "小",
    b"\x8E\x98": "少",
    b"\x8E\x99": "??",
    b"\x8E\x9A": "床",
    b"\x8E\x9B": "??",
    b"\x8E\x9C": "??",
    b"\x8E\x9D": "衝",
    b"\x8E\x9E": "承",
    b"\x8E\x9F": "??",
    b"\x8E\xA0": "抄",
    b"\x8E\xA1": "招",
    b"\x8E\xA2": "昇",
    b"\x8E\xA3": "昭",
    b"\x8E\xA4": "肖",
    b"\x8E\xA5": "松",
    b"\x8E\xA6": "沼",
    b"\x8E\xA7": "消",
    b"\x8E\xA8": "渉",
    b"\x8E\xA9": "焼",
    b"\x8E\xAA": "焦",
    b"\x8E\xAB": "??",
    b"\x8E\xAC": "症",
    b"\x8E\xAD": "硝",
    b"\x8E\xAE": "礁",
    b"\x8E\xAF": "祥",
    b"\x8E\xB0": "称",
    b"\x8E\xB1": "章",
    b"\x8E\xB2": "笑",
    b"\x8E\xB3": "粧",
    b"\x8E\xB4": "紹",
    b"\x8E\xB5": "訟",
    b"\x8E\xB6": "証",
    b"\x8E\xB7": "??",
    b"\x8E\xB8": "詳",
    b"\x8E\xB9": "象",
    b"\x8E\xBA": "賞",
    b"\x8E\xBB": "鐘",
    b"\x8E\xBC": "障",
    b"\x8E\xBD": "上",
    b"\x8E\xBE": "丈",
    b"\x8E\xBF": "乗",
    b"\x8E\xC0": "??",
    b"\x8E\xC1": "剰",
    b"\x8E\xC2": "城",
    b"\x8E\xC3": "場",
    b"\x8E\xC4": "壌",
    b"\x8E\xC5": "嬢",
    b"\x8E\xC6": "常",
    b"\x8E\xC7": "情",
    b"\x8E\xC8": "条",
    b"\x8E\xC9": "浄",
    b"\x8E\xCA": "状",
    b"\x8E\xCB": "??",
    b"\x8E\xCC": "縄",
    b"\x8E\xCD": "??",
    b"\x8E\xCE": "??",
    b"\x8E\xCF": "醸",
    b"\x8E\xD0": "錠",
    b"\x8E\xD1": "嘱",
    b"\x8E\xD2": "植",
    b"\x8E\xD3": "殖",
    b"\x8E\xD4": "織",
    b"\x8E\xD5": "職",
    b"\x8E\xD6": "色",
    b"\x8E\xD7": "触",
    b"\x8E\xD8": "食",
    b"\x8E\xD9": "飾",
    b"\x8E\xDA": "辱",
    b"\x8E\xDB": "伸",
    b"\x8E\xDC": "信",
    b"\x8E\xDD": "侵",
    b"\x8E\xDE": "唇",
    b"\x8E\xDF": "娠",
    b"\x8E\xE0": "寝",
    b"\x8E\xE1": "??",
    b"\x8E\xE2": "心",
    b"\x8E\xE3": "慎",
    b"\x8E\xE4": "振",
    b"\x8E\xE5": "新",
    b"\x8E\xE6": "森",
    b"\x8E\xE7": "津",
    b"\x8E\xE8": "浸",
    b"\x8E\xE9": "深",
    b"\x8E\xEA": "申",
    b"\x8E\xEB": "真",
    b"\x8E\xEC": "神",
    b"\x8E\xED": "紳",
    b"\x8E\xEE": "??",
    b"\x8E\xEF": "臣",
    b"\x8E\xF0": "親",
    b"\x8E\xF1": "診",
    b"\x8E\xF2": "身",
    b"\x8E\xF3": "辛",
    b"\x8E\xF4": "進",
    b"\x8E\xF5": "針",
    b"\x8E\xF6": "震",
    b"\x8E\xF7": "人",
    b"\x8E\xF8": "仁",
    b"\x8E\xF9": "刃",
    b"\x8E\xFA": "尋",
    b"\x8E\xFB": "尽",
    b"\x8E\xFC": "??",

    b"\x8F\x40": "迅",
    b"\x8F\x41": "陣",
    b"\x8F\x42": "図",
    b"\x8F\x43": "吹",
    b"\x8F\x44": "垂",
    b"\x8F\x45": "帥",
    b"\x8F\x46": "推",
    b"\x8F\x47": "水",
    b"\x8F\x48": "炊",
    b"\x8F\x49": "睡",
    b"\x8F\x4A": "??",
    b"\x8F\x4B": "粋",
    b"\x8F\x4C": "衰",
    b"\x8F\x4D": "??",
    b"\x8F\x4E": "酔",
    b"\x8F\x4F": "??",
    b"\x8F\x50": "随",
    b"\x8F\x51": "髄",
    b"\x8F\x52": "崇",
    b"\x8F\x53": "数",
    b"\x8F\x54": "枢",
    b"\x8F\x55": "寸",
    b"\x8F\x56": "是",
    b"\x8F\x57": "世",
    b"\x8F\x58": "井",
    b"\x8F\x59": "制",
    b"\x8F\x5A": "勢",
    b"\x8F\x5B": "声",
    b"\x8F\x5C": "姓",
    b"\x8F\x5D": "??",
    b"\x8F\x5E": "征",
    b"\x8F\x5F": "性",
    b"\x8F\x60": "成",
    b"\x8F\x61": "政",
    b"\x8F\x62": "??",
    b"\x8F\x63": "星",
    b"\x8F\x64": "晶",
    b"\x8F\x65": "晴",
    b"\x8F\x66": "正",
    b"\x8F\x67": "清",
    b"\x8F\x68": "牲",
    b"\x8F\x69": "生",
    b"\x8F\x6A": "盛",
    b"\x8F\x6B": "省",
    b"\x8F\x6C": "精",
    b"\x8F\x6D": "聖",
    b"\x8F\x6E": "製",
    b"\x8F\x6F": "西",
    b"\x8F\x70": "誠",
    b"\x8F\x71": "敬",
    b"\x8F\x72": "請",
    b"\x8F\x73": "逝",
    b"\x8F\x74": "青",
    b"\x8F\x75": "静",
    b"\x8F\x76": "斉",
    b"\x8F\x77": "税",
    b"\x8F\x78": "夕",
    b"\x8F\x79": "席",
    b"\x8F\x7A": "惜",
    b"\x8F\x7B": "??",
    b"\x8F\x7C": "昔",
    b"\x8F\x7D": "析",
    b"\x8F\x7E": "石",
    b"\x8F\x7F": "??",
    b"\x8F\x80": "積",
    b"\x8F\x81": "籍",
    b"\x8F\x82": "??",
    b"\x8F\x83": "責",
    b"\x8F\x84": "赤",
    b"\x8F\x85": "跡",
    b"\x8F\x86": "隻",
    b"\x8F\x87": "切",
    b"\x8F\x88": "折",
    b"\x8F\x89": "??",
    b"\x8F\x8A": "接",
    b"\x8F\x8B": "摂",
    b"\x8F\x8C": "??",
    b"\x8F\x8D": "節",
    b"\x8F\x8E": "設",
    b"\x8F\x8F": "説",
    b"\x8F\x90": "雪",
    b"\x8F\x91": "絶",
    b"\x8F\x92": "??",
    b"\x8F\x93": "仙",
    b"\x8F\x94": "先",
    b"\x8F\x95": "千",
    b"\x8F\x96": "占",
    b"\x8F\x97": "??",
    b"\x8F\x98": "専",
    b"\x8F\x99": "川",
    b"\x8F\x9A": "戦",
    b"\x8F\x9B": "??",
    b"\x8F\x9C": "旋",
    b"\x8F\x9D": "染",
    b"\x8F\x9E": "??",
    b"\x8F\x9F": "泉",
    b"\x8F\xA0": "浅",
    b"\x8F\xA1": "洗",
    b"\x8F\xA2": "潜",
    b"\x8F\xA3": "線",
    b"\x8F\xA4": "??",
    b"\x8F\xA5": "船",
    b"\x8F\xA6": "薦",
    b"\x8F\xA7": "践",
    b"\x8F\xA8": "選",
    b"\x8F\xA9": "??",
    b"\x8F\xAA": "銭",
    b"\x8F\xAB": "??",
    b"\x8F\xAC": "鮮",
    b"\x8F\xAD": "全",
    b"\x8F\xAE": "前",
    b"\x8F\xAF": "善",
    b"\x8F\xB0": "??",
    b"\x8F\xB1": "然",
    b"\x8F\xB2": "??",
    b"\x8F\xB3": "??",
    b"\x8F\xB4": "??",
    b"\x8F\xB5": "措",
    b"\x8F\xB6": "??",
    b"\x8F\xB7": "礎",
    b"\x8F\xB8": "祖",
    b"\x8F\xB9": "租",
    b"\x8F\xBA": "粗",
    b"\x8F\xBB": "素",
    b"\x8F\xBC": "組",
    b"\x8F\xBD": "訴",
    b"\x8F\xBE": "阻",
    b"\x8F\xBF": "争",
    b"\x8F\xC0": "倉",
    b"\x8F\xC1": "僧",
    b"\x8F\xC2": "??",
    b"\x8F\xC3": "双",
    b"\x8F\xC4": "??",
    b"\x8F\xC5": "壮",
    b"\x8F\xC6": "奏",
    b"\x8F\xC7": "巣",
    b"\x8F\xC8": "層",
    b"\x8F\xC9": "想",
    b"\x8F\xCA": "??",
    b"\x8F\xCB": "挿",
    b"\x8F\xCC": "掃",
    b"\x8F\xCD": "操",
    b"\x8F\xCE": "早",
    b"\x8F\xCF": "曹",
    b"\x8F\xD0": "??",
    b"\x8F\xD1": "模",
    b"\x8F\xD2": "燥",
    b"\x8F\xD3": "相",
    b"\x8F\xD4": "窓",
    b"\x8F\xD5": "??",
    b"\x8F\xD6": "草",
    b"\x8F\xD7": "??",
    b"\x8F\xD8": "??",
    b"\x8F\xD9": "??",
    b"\x8F\xDA": "装",
    b"\x8F\xDB": "走",
    b"\x8F\xDC": "送",
    b"\x8F\xDD": "遭",
    b"\x8F\xDE": "??",
    b"\x8F\xDF": "騒",
    b"\x8F\xE0": "像",
    b"\x8F\xE1": "増",
    b"\x8F\xE2": "??",
    b"\x8F\xE3": "??",
    b"\x8F\xE4": "??",
    b"\x8F\xE5": "贈",
    b"\x8F\xE6": "造",
    b"\x8F\xE7": "促",
    b"\x8F\xE8": "側",
    b"\x8F\xE9": "則",
    b"\x8F\xEA": "即",
    b"\x8F\xEB": "息",
    b"\x8F\xEC": "??",
    b"\x8F\xED": "測",
    b"\x8F\xEE": "足",
    b"\x8F\xEF": "速",
    b"\x8F\xF0": "俗",
    b"\x8F\xF1": "属",
    b"\x8F\xF2": "族",
    b"\x8F\xF3": "続",
    b"\x8F\xF4": "??",
    b"\x8F\xF5": "卒",
    b"\x8F\xF6": "率",
    b"\x8F\xF7": "存",
    b"\x8F\xF8": "孫",
    b"\x8F\xF9": "尊",
    b"\x8F\xFA": "損",
    b"\x8F\xFB": "村",
    b"\x8F\xFC": "他",

    b"\x90\x40": "多",
    b"\x90\x41": "堕",
    b"\x90\x42": "??",
    b"\x90\x43": "??",
    b"\x90\x44": "打",
    b"\x90\x45": "駄",
    b"\x90\x46": "体",
    b"\x90\x47": "太",
    b"\x90\x48": "対",
    b"\x90\x49": "??",
    b"\x90\x4A": "帯",
    b"\x90\x4B": "待",
    b"\x90\x4C": "??",
    b"\x90\x4D": "態",
    b"\x90\x4E": "替",
    b"\x90\x4F": "胎",
    b"\x90\x50": "泰",
    b"\x90\x51": "滞",
    b"\x90\x52": "袋",
    b"\x90\x53": "??",
    b"\x90\x54": "退",
    b"\x90\x55": "逮",
    b"\x90\x56": "隊",
    b"\x90\x57": "代",
    b"\x90\x58": "台",
    b"\x90\x59": "大",
    b"\x90\x5A": "第",
    b"\x90\x5B": "題",
    b"\x90\x5C": "卓",
    b"\x90\x5D": "宅",
    b"\x90\x5E": "択",
    b"\x90\x5F": "拓",
    b"\x90\x60": "沢",
    b"\x90\x61": "濯",
    b"\x90\x62": "託",
    b"\x90\x63": "濁",
    b"\x90\x64": "諾",
    b"\x90\x65": "達",
    b"\x90\x66": "??",
    b"\x90\x67": "脱",
    b"\x90\x68": "丹",
    b"\x90\x69": "嘆",
    b"\x90\x6A": "単",
    b"\x90\x6B": "??",
    b"\x90\x6C": "探",
    b"\x90\x6D": "胆",
    b"\x90\x6E": "淡",
    b"\x90\x6F": "炭",
    b"\x90\x70": "短",
    b"\x90\x71": "端",
    b"\x90\x72": "誕",
    b"\x90\x73": "??",
    b"\x90\x74": "団",
    b"\x90\x75": "壇",
    b"\x90\x76": "弾",
    b"\x90\x77": "断",
    b"\x90\x78": "暖",
    b"\x90\x79": "段",
    b"\x90\x7A": "男",
    b"\x90\x7B": "談",
    b"\x90\x7C": "値",
    b"\x90\x7D": "地",
    b"\x90\x7E": "恥",
    b"\x90\x7F": "??",
    b"\x90\x80": "池",
    b"\x90\x81": "痴",
    b"\x90\x82": "知",
    b"\x90\x83": "稚",
    b"\x90\x84": "置",
    b"\x90\x85": "致",
    b"\x90\x86": "遅",
    b"\x90\x87": "畜",
    b"\x90\x88": "竹",
    b"\x90\x89": "築",
    b"\x90\x8A": "蓄",
    b"\x90\x8B": "??",
    b"\x90\x8C": "秩",
    b"\x90\x8D": "窒",
    b"\x90\x8E": "茶",
    b"\x90\x8F": "??",
    b"\x90\x90": "着",
    b"\x90\x91": "中",
    b"\x90\x92": "仲",
    b"\x90\x93": "宙",
    b"\x90\x94": "忠",
    b"\x90\x95": "抽",
    b"\x90\x96": "昼",
    b"\x90\x97": "柱",
    b"\x90\x98": "沖",
    b"\x90\x99": "注",
    b"\x90\x9A": "??",
    b"\x90\x9B": "??",
    b"\x90\x9C": "??",
    b"\x90\x9D": "??",
    b"\x90\x9E": "??",
    b"\x90\x9F": "??",
    b"\x90\xA0": "丁",
    b"\x90\xA1": "兆",
    b"\x90\xA2": "帳",
    b"\x90\xA3": "??",
    b"\x90\xA4": "??",
    b"\x90\xA5": "張",
    b"\x90\xA6": "??",
    b"\x90\xA7": "徴",
    b"\x90\xA8": "懲",
    b"\x90\xA9": "挑",
    b"\x90\xAA": "朝",
    b"\x90\xAB": "??",
    b"\x90\xAC": "腸",
    b"\x90\xAD": "潮",
    b"\x90\xAE": "澄",
    b"\x90\xAF": "町",
    b"\x90\xB0": "眺",
    b"\x90\xB1": "聴",
    b"\x90\xB2": "調",
    b"\x90\xB3": "超",
    b"\x90\xB4": "跳",
    b"\x90\xB5": "釣",
    b"\x90\xB6": "長",
    b"\x90\xB7": "頂",
    b"\x90\xB8": "??",
    b"\x90\xB9": "勅",
    b"\x90\xBA": "直",
    b"\x90\xBB": "??",
    b"\x90\xBC": "沈",
    b"\x90\xBD": "珍",
    b"\x90\xBE": "賃",
    b"\x90\xBF": "??",
    b"\x90\xC0": "陳",
    b"\x90\xC1": "墜",
    b"\x90\xC2": "追",
    b"\x90\xC3": "痛",
    b"\x90\xC4": "通",
    b"\x90\xC5": "??",
    b"\x90\xC6": "低",
    b"\x90\xC7": "停",
    b"\x90\xC8": "偵",
    b"\x90\xC9": "呈",
    b"\x90\xCA": "堤",
    b"\x90\xCB": "定",
    b"\x90\xCC": "帝",
    b"\x90\xCD": "底",
    b"\x90\xCE": "庭",
    b"\x90\xCF": "延",
    b"\x90\xD0": "弟",
    b"\x90\xD1": "抵",
    b"\x90\xD2": "提",
    b"\x90\xD3": "程",
    b"\x90\xD4": "締",
    b"\x90\xD5": "艇",
    b"\x90\xD6": "訂",
    b"\x90\xD7": "貞",
    b"\x90\xD8": "??",
    b"\x90\xD9": "邸",
    b"\x90\xDA": "泥",
    b"\x90\xDB": "摘",
    b"\x90\xDC": "敵",
    b"\x90\xDD": "滴",
    b"\x90\xDE": "的",
    b"\x90\xDF": "??",
    b"\x90\xE0": "適",
    b"\x90\xE1": "哲",
    b"\x90\xE2": "徹",
    b"\x90\xE3": "撤",
    b"\x90\xE4": "送",
    b"\x90\xE5": "鉄",
    b"\x90\xE6": "典",
    b"\x90\xE7": "天",
    b"\x90\xE8": "展",
    b"\x90\xE9": "店",
    b"\x90\xEA": "添",
    b"\x90\xEB": "点",
    b"\x90\xEC": "転",
    b"\x90\xED": "伝",
    b"\x90\xEE": "??",
    b"\x90\xEF": "田",
    b"\x90\xF0": "電",
    b"\x90\xF1": "吐",
    b"\x90\xF2": "塗",
    b"\x90\xF3": "徒",
    b"\x90\xF4": "斗",
    b"\x90\xF5": "渡",
    b"\x90\xF6": "途",
    b"\x90\xF7": "都",
    b"\x90\xF8": "努",
    b"\x90\xF9": "土",
    b"\x90\xFA": "奴",
    b"\x90\xFB": "度",
    b"\x90\xFC": "??",

    b"\x91\x40": "倒",
    b"\x91\x41": "??",
    b"\x91\x42": "凍",
    b"\x91\x43": "刀",
    b"\x91\x44": "到",
    b"\x91\x45": "唐",
    b"\x91\x46": "塔",
    b"\x91\x47": "冬",
    b"\x91\x48": "当",
    b"\x91\x49": "島",
    b"\x91\x4A": "悼",
    b"\x91\x4B": "投",
    b"\x91\x4C": "搭",
    b"\x91\x4D": "東",
    b"\x91\x4E": "桃",
    b"\x91\x4F": "棟",
    b"\x91\x50": "湯",
    b"\x91\x51": "灯",
    b"\x91\x52": "??",
    b"\x91\x53": "登",
    b"\x91\x54": "盗",
    b"\x91\x55": "稲",
    b"\x91\x56": "答",
    b"\x91\x57": "等",
    b"\x91\x58": "筒",
    b"\x91\x59": "糖",
    b"\x91\x5A": "統",
    b"\x91\x5B": "討",
    b"\x91\x5C": "??",
    b"\x91\x5D": "豆",
    b"\x91\x5E": "踏",
    b"\x91\x5F": "逃",
    b"\x91\x60": "透",
    b"\x91\x61": "闘",
    b"\x91\x62": "陶",
    b"\x91\x63": "頭",
    b"\x91\x64": "騰",
    b"\x91\x65": "働",
    b"\x91\x66": "同",
    b"\x91\x67": "動",
    b"\x91\x68": "堂",
    b"\x91\x69": "導",
    b"\x91\x6A": "胴",
    b"\x91\x6B": "洞",
    b"\x91\x6C": "童",
    b"\x91\x6D": "道",
    b"\x91\x6E": "銅",
    b"\x91\x6F": "??",
    b"\x91\x70": "得",
    b"\x91\x71": "徳",
    b"\x91\x72": "特",
    b"\x91\x73": "??",
    b"\x91\x74": "篤",
    b"\x91\x75": "毒",
    b"\x91\x76": "独",
    b"\x91\x77": "読",
    b"\x91\x78": "凸",
    b"\x91\x79": "突",
    b"\x91\x7A": "屯",
    b"\x91\x7B": "豚",
    b"\x91\x7C": "曇",
    b"\x91\x7D": "鈍",
    b"\x91\x7E": "内",
    b"\x91\x7F": "??",
    b"\x91\x80": "南",
    b"\x91\x81": "軟",
    b"\x91\x82": "難",
    b"\x91\x83": "二",
    b"\x91\x84": "尼",
    b"\x91\x85": "弐",
    b"\x91\x86": "肉",
    b"\x91\x87": "日",
    b"\x91\x88": "乳",
    b"\x91\x89": "入",
    b"\x91\x8A": "尿",
    b"\x91\x8B": "任",
    b"\x91\x8C": "妊",
    b"\x91\x8D": "忍",
    b"\x91\x8E": "認",
    b"\x91\x8F": "寧",
    b"\x91\x90": "熱",
    b"\x91\x91": "年",
    b"\x91\x92": "??",
    b"\x91\x93": "燃",
    b"\x91\x94": "粘",
    b"\x91\x95": "悩",
    b"\x91\x96": "能",
    b"\x91\x97": "脳",
    b"\x91\x98": "濃",
    b"\x91\x99": "納",
    b"\x91\x9A": "農",
    b"\x91\x9B": "把",
    b"\x91\x9C": "波",
    b"\x91\x9D": "派",
    b"\x91\x9E": "破",
    b"\x91\x9F": "覇",
    b"\x91\xA0": "婆",
    b"\x91\xA1": "馬",
    b"\x91\xA2": "俳",
    b"\x91\xA3": "廃",
    b"\x91\xA4": "拝",
    b"\x91\xA5": "排",
    b"\x91\xA6": "敗",
    b"\x91\xA7": "肺",
    b"\x91\xA8": "背",
    b"\x91\xA9": "杯",
    b"\x91\xAA": "輩",
    b"\x91\xAB": "配",
    b"\x91\xAC": "倍",
    b"\x91\xAD": "培",
    b"\x91\xAE": "売",
    b"\x91\xAF": "媒",
    b"\x91\xB0": "梅",
    b"\x91\xB1": "買",
    b"\x91\xB2": "賠",
    b"\x91\xB3": "陪",
    b"\x91\xB4": "伯",
    b"\x91\xB5": "博",
    b"\x91\xB6": "泊",
    b"\x91\xB7": "白",
    b"\x91\xB8": "舶",
    b"\x91\xB9": "薄",
    b"\x91\xBA": "迫",
    b"\x91\xBB": "漠",
    b"\x91\xBC": "爆",
    b"\x91\xBD": "縛",
    b"\x91\xBE": "麦",
    b"\x91\xBF": "八",
    b"\x91\xC0": "鉢",
    b"\x91\xC1": "発",
    b"\x91\xC2": "髪",
    b"\x91\xC3": "伐",
    b"\x91\xC4": "抜",
    b"\x91\xC5": "罰",
    b"\x91\xC6": "閥",
    b"\x91\xC7": "拍",
    b"\x91\xC8": "伴",
    b"\x91\xC9": "判",
    b"\x91\xCA": "半",
    b"\x91\xCB": "反",
    b"\x91\xCC": "坂",
    b"\x91\xCD": "帆",
    b"\x91\xCE": "搬",
    b"\x91\xCF": "板",
    b"\x91\xD0": "煩",
    b"\x91\xD1": "版",
    b"\x91\xD2": "犯",
    b"\x91\xD3": "班",
    b"\x91\xD4": "畔",
    b"\x91\xD5": "範",
    b"\x91\xD6": "繁",
    b"\x91\xD7": "般",
    b"\x91\xD8": "藩",
    b"\x91\xD9": "販",
    b"\x91\xDA": "頒",
    b"\x91\xDB": "飯",
    b"\x91\xDC": "晩",
    b"\x91\xDD": "番",
    b"\x91\xDE": "??",
    b"\x91\xDF": "蛮",
    b"\x91\xE0": "卑",
    b"\x91\xE1": "否",
    b"\x91\xE2": "妃",
    b"\x91\xE3": "彼",
    b"\x91\xE4": "悲",
    b"\x91\xE5": "扉",
    b"\x91\xE6": "批",
    b"\x91\xE7": "披",
    b"\x91\xE8": "肥",
    b"\x91\xE9": "比",
    b"\x91\xEA": "疲",
    b"\x91\xEB": "皮",
    b"\x91\xEC": "碑",
    b"\x91\xED": "秘",
    b"\x91\xEE": "??",
    b"\x91\xEF": "被",
    b"\x91\xF0": "費",
    b"\x91\xF1": "避",
    b"\x91\xF2": "非",
    b"\x91\xF3": "飛",
    b"\x91\xF4": "備",
    b"\x91\xF5": "尾",
    b"\x91\xF6": "微",
    b"\x91\xF7": "美",
    b"\x91\xF8": "鼻",
    b"\x91\xF9": "匹",
    b"\x91\xFA": "必",
    b"\x91\xFB": "泌",
    b"\x91\xFC": "筆",

    b"\x92\x40": "百",
    b"\x92\x41": "俵",
    b"\x92\x42": "標",
    b"\x92\x43": "氷",
    b"\x92\x44": "漂",
    b"\x92\x45": "票",
    b"\x92\x46": "表",
    b"\x92\x47": "評",
    b"\x92\x48": "描",
    b"\x92\x49": "猫",
    b"\x92\x4A": "病",
    b"\x92\x4B": "秒",
    b"\x92\x4C": "苗",
    b"\x92\x4D": "品",
    b"\x92\x4E": "浜",
    b"\x92\x4F": "貧",
    b"\x92\x50": "賓",
    b"\x92\x51": "頻",
    b"\x92\x52": "敏",
    b"\x92\x53": "瓶",
    b"\x92\x54": "不",
    b"\x92\x55": "付",
    b"\x92\x56": "夫",
    b"\x92\x57": "??",
    b"\x92\x58": "富",
    b"\x92\x59": "布",
    b"\x92\x5A": "府",
    b"\x92\x5B": "怖",
    b"\x92\x5C": "扶",
    b"\x92\x5D": "敷",
    b"\x92\x5E": "普",
    b"\x92\x5F": "膚",
    b"\x92\x60": "浮",
    b"\x92\x61": "父",
    b"\x92\x62": "符",
    b"\x92\x63": "腐",
    b"\x92\x64": "譜",
    b"\x92\x65": "負",
    b"\x92\x66": "賦",
    b"\x92\x67": "赴",
    b"\x92\x68": "附",
    b"\x92\x69": "悔",
    b"\x92\x6A": "武",
    b"\x92\x6B": "舞",
    b"\x92\x6C": "部",
    b"\x92\x6D": "風",
    b"\x92\x6E": "封",
    b"\x92\x6F": "伏",
    b"\x92\x70": "副",
    b"\x92\x71": "幅",
    b"\x92\x72": "復",
    b"\x92\x73": "服",
    b"\x92\x74": "腹",
    b"\x92\x75": "福",
    b"\x92\x76": "複",
    b"\x92\x77": "覆",
    b"\x92\x78": "払",
    b"\x92\x79": "沸",
    b"\x92\x7A": "仏",
    b"\x92\x7B": "物",
    b"\x92\x7C": "噴",
    b"\x92\x7D": "墳",
    b"\x92\x7E": "奮",
    b"\x92\x7F": "??",
    b"\x92\x80": "憤",
    b"\x92\x81": "粉",
    b"\x92\x82": "紛",
    b"\x92\x83": "雰",
    b"\x92\x84": "分",
    b"\x92\x85": "文",
    b"\x92\x86": "聞",
    b"\x92\x87": "丙",
    b"\x92\x88": "並",
    b"\x92\x89": "併",
    b"\x92\x8A": "兵",
    b"\x92\x8B": "塀",
    b"\x92\x8C": "幣",
    b"\x92\x8D": "平",
    b"\x92\x8E": "弊",
    b"\x92\x8F": "柄",
    b"\x92\x90": "閉",
    b"\x92\x91": "陛",
    b"\x92\x92": "米",
    b"\x92\x93": "壁",
    b"\x92\x94": "癖",
    b"\x92\x95": "別",
    b"\x92\x96": "偏",
    b"\x92\x97": "変",
    b"\x92\x98": "片",
    b"\x92\x99": "編",
    b"\x92\x9A": "辺",
    b"\x92\x9B": "返",
    b"\x92\x9C": "遍",
    b"\x92\x9D": "便",
    b"\x92\x9E": "勉",
    b"\x92\x9F": "弁",
    b"\x92\xA0": "保",
    b"\x92\xA1": "舗",
    b"\x92\xA2": "捕",
    b"\x92\xA3": "歩",
    b"\x92\xA4": "浦",
    b"\x92\xA5": "補",
    b"\x92\xA6": "募",
    b"\x92\xA7": "墓",
    b"\x92\xA8": "慕",
    b"\x92\xA9": "暮",
    b"\x92\xAA": "母",
    b"\x92\xAB": "簿",
    b"\x92\xAC": "倣",
    b"\x92\xAD": "俸",
    b"\x92\xAE": "包",
    b"\x92\xAF": "報",
    b"\x92\xB0": "奉",
    b"\x92\xB1": "宝",
    b"\x92\xB2": "??",
    b"\x92\xB3": "崩",
    b"\x92\xB4": "抱",
    b"\x92\xB5": "放",
    b"\x92\xB6": "方",
    b"\x92\xB7": "胞",
    b"\x92\xB8": "法",
    b"\x92\xB9": "泡",
    b"\x92\xBA": "砲",
    b"\x92\xBB": "縫",
    b"\x92\xBC": "芳",
    b"\x92\xBD": "褒",
    b"\x92\xBE": "訪",
    b"\x92\xBF": "豊",
    b"\x92\xC0": "邦",
    b"\x92\xC1": "飽",
    b"\x92\xC2": "乏",
    b"\x92\xC3": "亡",
    b"\x92\xC4": "傍",
    b"\x92\xC5": "剖",
    b"\x92\xC6": "坊",
    b"\x92\xC7": "妨",
    b"\x92\xC8": "帽",
    b"\x92\xC9": "忘",
    b"\x92\xCA": "忙",
    b"\x92\xCB": "房",
    b"\x92\xCC": "??",
    b"\x92\xCD": "暴",
    b"\x92\xCE": "肪",
    b"\x92\xCF": "望",
    b"\x92\xD0": "膨",
    b"\x92\xD1": "某",
    b"\x92\xD2": "??",
    b"\x92\xD3": "紡",
    b"\x92\xD4": "謀",
    b"\x92\xD5": "貿",
    b"\x92\xD6": "防",
    b"\x92\xD7": "北",
    b"\x92\xD8": "僕",
    b"\x92\xD9": "墨",
    b"\x92\xDA": "撲",
    b"\x92\xDB": "木",
    b"\x92\xDC": "朴",
    b"\x92\xDD": "牧",
    b"\x92\xDE": "没",
    b"\x92\xDF": "奔",
    b"\x92\xE0": "本",
    b"\x92\xE1": "??",
    b"\x92\xE2": "凡",
    b"\x92\xE3": "盆",
    b"\x92\xE4": "摩",
    b"\x92\xE5": "磨",
    b"\x92\xE6": "魔",
    b"\x92\xE7": "麻",
    b"\x92\xE8": "埋",
    b"\x92\xE9": "妹",
    b"\x92\xEA": "枚",
    b"\x92\xEB": "毎",
    b"\x92\xEC": "幕",
    b"\x92\xED": "膜",
    b"\x92\xEE": "抹",
    b"\x92\xEF": "末",
    b"\x92\xF0": "万",
    b"\x92\xF1": "慢",
    b"\x92\xF2": "満",
    b"\x92\xF3": "??",
    b"\x92\xF4": "味",
    b"\x92\xF5": "未",
    b"\x92\xF6": "魅",
    b"\x92\xF7": "密",
    b"\x92\xF8": "脈",
    b"\x92\xF9": "妙",
    b"\x92\xFA": "民",
    b"\x92\xFB": "眠",
    b"\x92\xFC": "務",

    b"\x93\x40": "夢",
    b"\x93\x41": "無",
    b"\x93\x42": "矛",
    b"\x93\x43": "霧",
    b"\x93\x44": "命",
    b"\x93\x45": "名",
    b"\x93\x46": "明",
    b"\x93\x47": "盟",
    b"\x93\x48": "迷",
    b"\x93\x49": "銘",
    b"\x93\x4A": "鳴",
    b"\x93\x4B": "滅",
    b"\x93\x4C": "免",
    b"\x93\x4D": "綿",
    b"\x93\x4E": "面",
    b"\x93\x4F": "模",
    b"\x93\x50": "茂",
    b"\x93\x51": "妄",
    b"\x93\x52": "毛",
    b"\x93\x53": "猛",
    b"\x93\x54": "盲",
    b"\x93\x55": "網",
    b"\x93\x56": "耗",
    b"\x93\x57": "目",
    b"\x93\x58": "黙",
    b"\x93\x59": "問",
    b"\x93\x5A": "紋",
    b"\x93\x5B": "門",
    b"\x93\x5C": "夜",
    b"\x93\x5D": "野",
    b"\x93\x5E": "厄",
    b"\x93\x5F": "役",
    b"\x93\x60": "約",
    b"\x93\x61": "??",
    b"\x93\x62": "訳",
    b"\x93\x63": "躍",
    b"\x93\x64": "??",
    b"\x93\x65": "油",
    b"\x93\x66": "由",
    b"\x93\x67": "??",
    b"\x93\x68": "論",
    b"\x93\x69": "輸",
    b"\x93\x6A": "唯",
    b"\x93\x6B": "優",
    b"\x93\x6C": "??",
    b"\x93\x6D": "友",
    b"\x93\x6E": "幽",
    b"\x93\x6F": "悠",
    b"\x93\x70": "??",
    b"\x93\x71": "有",
    b"\x93\x72": "??",
    b"\x93\x73": "裕",
    b"\x93\x74": "誘",
    b"\x93\x75": "遊",
    b"\x93\x76": "郵",
    b"\x93\x77": "雄",
    b"\x93\x78": "融",
    b"\x93\x79": "与",
    b"\x93\x7A": "予",
    b"\x93\x7B": "余",
    b"\x93\x7C": "??",
    b"\x93\x7D": "預",
    b"\x93\x7E": "幼",
    b"\x93\x7F": "??",
    b"\x93\x80": "容",
    b"\x93\x81": "??",
    b"\x93\x82": "揚",
    b"\x93\x83": "??",
    b"\x93\x84": "擁",
    b"\x93\x85": "曜",
    b"\x93\x86": "腰",
    b"\x93\x87": "様",
    b"\x93\x88": "洋",
    b"\x93\x89": "浴",
    b"\x93\x8A": "用",
    b"\x93\x8B": "??",
    b"\x93\x8C": "??",
    b"\x93\x8D": "葉",
    b"\x93\x8E": "要",
    b"\x93\x8F": "??",  # Seems unused
    b"\x93\x90": "踊",
    b"\x93\x91": "陽",
    b"\x93\x92": "養",
    b"\x93\x93": "抑",
    b"\x93\x94": "欲",
    b"\x93\x95": "浴",
    b"\x93\x96": "翌",
    b"\x93\x97": "翼",
    b"\x93\x98": "??",
    b"\x93\x99": "裸",
    b"\x93\x9A": "来",
    b"\x93\x9B": "雷",
    b"\x93\x9C": "頼",
    b"\x93\x9D": "絡",
    b"\x93\x9E": "落",
    b"\x93\x9F": "酪",
    b"\x93\xA0": "乱",
    b"\x93\xA1": "卵",
    b"\x93\xA2": "欄",
    b"\x93\xA3": "濫",
    b"\x93\xA4": "覧",
    b"\x93\xA5": "利",
    b"\x93\xA6": "吏",
    b"\x93\xA7": "履",
    b"\x93\xA8": "理",
    b"\x93\xA9": "痢",
    b"\x93\xAA": "裏",
    b"\x93\xAB": "里",
    b"\x93\xAC": "離",
    b"\x93\xAD": "陸",
    b"\x93\xAE": "律",
    b"\x93\xAF": "立",
    b"\x93\xB0": "??",
    b"\x93\xB1": "柳",
    b"\x93\xB2": "流",
    b"\x93\xB3": "留",
    b"\x93\xB4": "硫",
    b"\x93\xB5": "粒",
    b"\x93\xB6": "隆",
    b"\x93\xB7": "竜",
    b"\x93\xB8": "??",
    b"\x93\xB9": "旅",
    b"\x93\xBA": "虜",
    b"\x93\xBB": "両",
    b"\x93\xBC": "了",
    b"\x93\xBD": "僚",
    b"\x93\xBE": "寮",
    b"\x93\xBF": "料",
    b"\x93\xC0": "涼",
    b"\x93\xC1": "猟",
    b"\x93\xC2": "療",
    b"\x93\xC3": "糧",
    b"\x93\xC4": "良",
    b"\x93\xC5": "量",
    b"\x93\xC6": "陵",
    b"\x93\xC7": "領",
    b"\x93\xC8": "力",
    b"\x93\xC9": "緑",
    b"\x93\xCA": "倫",
    b"\x93\xCB": "厘",
    b"\x93\xCC": "林",
    b"\x93\xCD": "臨",
    b"\x93\xCE": "輪",
    b"\x93\xCF": "隣",
    b"\x93\xD0": "塁",
    b"\x93\xD1": "涙",
    b"\x93\xD2": "累",
    b"\x93\xD3": "類",
    b"\x93\xD4": "令",
    b"\x93\xD5": "例",
    b"\x93\xD6": "冷",
    b"\x93\xD7": "励",
    b"\x93\xD8": "戻",
    b"\x93\xD9": "礼",
    b"\x93\xDA": "鈴",
    b"\x93\xDB": "隷",
    b"\x93\xDC": "??",
    b"\x93\xDD": "霊",
    b"\x93\xDE": "??",
    b"\x93\xDF": "??",
    b"\x93\xE0": "暦",
    b"\x93\xE1": "歴",
    b"\x93\xE2": "列",
    b"\x93\xE3": "劣",
    b"\x93\xE4": "烈",
    b"\x93\xE5": "裂",
    b"\x93\xE6": "??",
    b"\x93\xE7": "恋",
    b"\x93\xE8": "練",
    b"\x93\xE9": "連",
    b"\x93\xEA": "錬",
    b"\x93\xEB": "炉",
    b"\x93\xEC": "路",
    b"\x93\xED": "露",
    b"\x93\xEE": "労",
    b"\x93\xEF": "廊",
    b"\x93\xF0": "朗",
    b"\x93\xF1": "楼",
    b"\x93\xF2": "浪",
    b"\x93\xF3": "漏",
    b"\x93\xF4": "老",
    b"\x93\xF5": "郎",
    b"\x93\xF6": "六",
    b"\x93\xF7": "録",
    b"\x93\xF8": "論",
    b"\x93\xF9": "和",
    b"\x93\xFA": "話",
    b"\x93\xFB": "賄",
    b"\x93\xFC": "惑",

    b"\x94\x40": "腕",
    b"\x94\x41": "湾",
    b"\x94\x42": "扱",
    b"\x94\x43": "阿",
    b"\x94\x44": "藍",
    b"\x94\x45": "葵",
    b"\x94\x46": "茜",
    b"\x94\x47": "旭",
    b"\x94\x48": "昌",
    b"\x94\x49": "晃",
    b"\x94\x4A": "晟",
    b"\x94\x4B": "彬",
    b"\x94\x4C": "渥",
    b"\x94\x4D": "曙",
    b"\x94\x4E": "梓",
    b"\x94\x4F": "敦",
    b"\x94\x50": "綾",
    b"\x94\x51": "鮎",
    b"\x94\x52": "嵐",
    b"\x94\x53": "晏",
    b"\x94\x54": "杏",
    b"\x94\x55": "芋",
    b"\x94\x56": "亥",
    b"\x94\x57": "伊",
    b"\x94\x58": "莞",
    b"\x94\x59": "斐",
    b"\x94\x5A": "郁",
    b"\x94\x5B": "磯",
    b"\x94\x5C": "猪",
    b"\x94\x5D": "巌",
    b"\x94\x5E": "允",
    b"\x94\x5F": "胤",
    b"\x94\x60": "畝",
    b"\x94\x61": "卯",
    b"\x94\x62": "丑",
    b"\x94\x63": "唄",
    b"\x94\x64": "慧",
    b"\x94\x65": "瑛",
    b"\x94\x66": "叡",
    b"\x94\x67": "苑",
    b"\x94\x68": "卸",
    b"\x94\x69": "虞",
    b"\x94\x6A": "於",
    b"\x94\x6B": "旺",
    b"\x94\x6C": "鳳",
    b"\x94\x6D": "治",
    b"\x94\x6E": "掛",
    b"\x94\x6F": "且",
    b"\x94\x70": "垣",
    b"\x94\x71": "刈",
    b"\x94\x72": "株",
    b"\x94\x73": "潟",
    b"\x94\x74": "貝",
    b"\x94\x75": "伽",
    b"\x94\x76": "袈",
    b"\x94\x77": "??",
    b"\x94\x78": "樺",
    b"\x94\x79": "魁",
    b"\x94\x7A": "凱",
    b"\x94\x7B": "楓",
    b"\x94\x7C": "馨",
    b"\x94\x7D": "??",
    b"\x94\x7E": "??",
    b"\x94\x7F": "??",
    b"\x94\x80": "嵩",
    b"\x94\x81": "霞",
    b"\x94\x82": "叶",
    b"\x94\x83": "??",
    b"\x94\x84": "??",
    b"\x94\x85": "茅",
    b"\x94\x86": "??",
    b"\x94\x87": "綺",
    b"\x94\x88": "??",
    b"\x94\x89": "??",
    b"\x94\x8A": "??",
    b"\x94\x8B": "??",
    b"\x94\x8C": "??",
    b"\x94\x8D": "??",
    b"\x94\x8E": "??",
    b"\x94\x8F": "??",
    b"\x94\x90": "??",
    b"\x94\x91": "桐",
    b"\x94\x92": "??",
    b"\x94\x93": "??",
    b"\x94\x94": "??",
    b"\x94\x95": "玖",
    b"\x94\x96": "??",
    b"\x94\x97": "楠",
    b"\x94\x98": "熊",
    b"\x94\x99": "??",
    b"\x94\x9A": "圭",
    b"\x94\x9B": "??",
    b"\x94\x9C": "桂",
    b"\x94\x9D": "拳",
    b"\x94\x9E": "絢",
    b"\x94\x9F": "??",
    b"\x94\xA0": "込",
    b"\x94\xA1": "胡",
    b"\x94\xA2": "??",
    b"\x94\xA3": "??",
    b"\x94\xA4": "??",
    b"\x94\xA5": "??",
    b"\x94\xA6": "鯉",
    b"\x94\xA7": "??",
    b"\x94\xA8": "??",
    b"\x94\xA9": "??",
    b"\x94\xAA": "??",
    b"\x94\xAB": "鴻",
    b"\x94\xAC": "??",
    b"\x94\xAD": "駒",
    b"\x94\xAE": "??",
    b"\x94\xAF": "??",
    b"\x94\xB0": "崎",
    b"\x94\xB1": "咲",
    b"\x94\xB2": "皿",
    b"\x94\xB3": "沙",
    b"\x94\xB4": "??",
    b"\x94\xB5": "??",
    b"\x94\xB6": "??",
    b"\x94\xB7": "??",
    b"\x94\xB8": "??",
    b"\x94\xB9": "??",
    b"\x94\xBA": "??",
    b"\x94\xBB": "笹",
    b"\x94\xBC": "??",
    b"\x94\xBD": "??",
    b"\x94\xBE": "聡",
    b"\x94\xBF": "爽",
    b"\x94\xC0": "??",
    b"\x94\xC1": "芝",
    b"\x94\xC2": "爾",
    b"\x94\xC3": "倖",
    b"\x94\xC4": "椎",
    b"\x94\xC5": "汐",
    b"\x94\xC6": "??",
    b"\x94\xC7": "鹿",
    b"\x94\xC8": "??",
    b"\x94\xC9": "洲",
    b"\x94\xCA": "??",
    b"\x94\xCB": "??",
    b"\x94\xCC": "??",
    b"\x94\xCD": "??",
    b"\x94\xCE": "??",
    b"\x94\xCF": "??",
    b"\x94\xD0": "駿",
    b"\x94\xD1": "??",
    b"\x94\xD2": "??",
    b"\x94\xD3": "??",
    b"\x94\xD4": "??",
    b"\x94\xD5": "??",
    b"\x94\xD6": "圧",
    b"\x94\xD7": "??",
    b"\x94\xD8": "??",
    b"\x94\xD9": "??",
    b"\x94\xDA": "翔",
    b"\x94\xDB": "??",
    b"\x94\xDC": "??",
    b"\x94\xDD": "??",
    b"\x94\xDE": "??",
    b"\x94\xDF": "晋",
    b"\x94\xE0": "??",
    b"\x94\xE1": "晨",
    b"\x94\xE2": "据",
    b"\x94\xE3": "??",
    b"\x94\xE4": "須",
    b"\x94\xE5": "??",
    b"\x94\xE6": "翠",
    b"\x94\xE7": "輔",
    b"\x94\xE8": "??",
    b"\x94\xE9": "??",
    b"\x94\xEA": "??",
    b"\x94\xEB": "??",
    b"\x94\xEC": "??",
    b"\x94\xED": "??",
    b"\x94\xEE": "??",
    b"\x94\xEF": "??",
    b"\x94\xF0": "??",
    b"\x94\xF1": "但",
    b"\x94\xF2": "棚",
    b"\x94\xF3": "滝",
    b"\x94\xF4": "汰",
    b"\x94\xF5": "鯛",
    b"\x94\xF6": "??",
    b"\x94\xF7": "??",
    b"\x94\xF8": "??",
    b"\x94\xF9": "??",
    b"\x94\xFA": "只",
    b"\x94\xFB": "橘",
    b"\x94\xFC": "辰",

    b"\x95\x40": "??",
    b"\x95\x41": "旦",
    b"\x95\x42": "??",
    b"\x95\x43": "??",
    b"\x95\x44": "??",
    b"\x95\x45": "蝶",
    b"\x95\x46": "塚",
    b"\x95\x47": "坪",
    b"\x95\x48": "??",
    b"\x95\x49": "槻",
    b"\x95\x4A": "??",
    b"\x95\x4B": "椿",
    b"\x95\x4C": "??",
    b"\x95\x4D": "艶",
    b"\x95\x4E": "??",
    b"\x95\x4F": "??",
    b"\x95\x50": "??",
    b"\x95\x51": "??",
    b"\x95\x52": "??",
    b"\x95\x53": "??",
    b"\x95\x54": "届",
    b"\x95\x55": "憶",
    b"\x95\x56": "??",
    b"\x95\x57": "朋",
    b"\x95\x58": "??",
    b"\x95\x59": "巴",
    b"\x95\x5A": "虎",
    b"\x95\x5B": "寅",
    b"\x95\x5C": "西",
    b"\x95\x5D": "那",
    b"\x95\x5E": "奈",
    b"\x95\x5F": "??",
    b"\x95\x60": "耶",
    b"\x95\x61": "凪",
    b"\x95\x62": "??",
    b"\x95\x63": "清",
    b"\x95\x64": "??",
    b"\x95\x65": "虹",
    b"\x95\x66": "錦",
    b"\x95\x67": "乃",
    b"\x95\x68": "之",
    b"\x95\x69": "肌",
    b"\x95\x6A": "畑",
    b"\x95\x6B": "箱",
    b"\x95\x6C": "萩",
    b"\x95\x6D": "鳩",
    b"\x95\x6E": "隼",
    b"\x95\x6F": "榛",
    b"\x95\x70": "遥",
    b"\x95\x71": "姫",
    b"\x95\x72": "緋",
    b"\x95\x73": "柊",
    b"\x95\x74": "彦",
    b"\x95\x75": "??",
    b"\x95\x76": "雛",
    b"\x95\x77": "媛",
    b"\x95\x78": "弘",
    b"\x95\x79": "宏",
    b"\x95\x7A": "浩",
    b"\x95\x7B": "皓",
    b"\x95\x7C": "??",
    b"\x95\x7D": "??",
    b"\x95\x7E": "碧",
    b"\x95\x7F": "??",
    b"\x95\x80": "堀",
    b"\x95\x81": "甫",
    b"\x95\x82": "鵬",
    b"\x95\x83": "瞳",
    b"\x95\x84": "又",
    b"\x95\x85": "蒔",
    b"\x95\x86": "槙",
    b"\x95\x87": "惇",
    b"\x95\x88": "??",
    b"\x95\x89": "亦",
    b"\x95\x8A": "茉",
    b"\x95\x8B": "眉",
    b"\x95\x8C": "黛",
    b"\x95\x8D": "毬",
    b"\x95\x8E": "鞠",
    b"\x95\x8F": "稀",
    b"\x95\x90": "麿",
    b"\x95\x91": "岬",
    b"\x95\x92": "巳",
    b"\x95\x93": "芙",
    b"\x95\x94": "??",
    b"\x95\x95": "瑞",
    b"\x95\x96": "嶺",
    b"\x95\x97": "稔",
    b"\x95\x98": "娘",
    b"\x95\x99": "孟",
    b"\x95\x9A": "萌",
    b"\x95\x9B": "杜",
    b"\x95\x9C": "也",
    b"\x95\x9D": "弥",
    b"\x95\x9E": "哉",
    b"\x95\x9F": "椰",
    b"\x95\xA0": "靖",
    b"\x95\xA1": "佑",
    b"\x95\xA2": "邑",
    b"\x95\xA3": "??",
    b"\x95\xA4": "??",
    b"\x95\xA5": "祐",
    b"\x95\xA6": "湧",
    b"\x95\xA7": "柚",
    b"\x95\xA8": "楊",
    b"\x95\xA9": "??",
    b"\x95\xAA": "??",
    b"\x95\xAB": "??",
    b"\x95\xAC": "??",
    b"\x95\xAD": "李",
    b"\x95\xAE": "莉",
    b"\x95\xAF": "梨",
    b"\x95\xB0": "璃",
    b"\x95\xB1": "睦",
    b"\x95\xB2": "琉",
    b"\x95\xB3": "亮",
    b"\x95\xB4": "凌",
    b"\x95\xB5": "椋",
    b"\x95\xB6": "稜",
    b"\x95\xB7": "諒",
    b"\x95\xB8": "遼",
    b"\x95\xB9": "瞭",
    b"\x95\xBA": "瞭",
    b"\x95\xBB": "琳",
    b"\x95\xBC": "??",
    b"\x95\xBD": "凜",
    b"\x95\xBE": "麟",
    b"\x95\xBF": "瑠",
    b"\x95\xC0": "??",
    b"\x95\xC1": "??",
    b"\x95\xC2": "玲",
    b"\x95\xC3": "??",
    b"\x95\xC4": "??",
    b"\x95\xC5": "呂",
    b"\x95\xC6": "禄",
    b"\x95\xC7": "枠",
    b"\x95\xC8": "倭",
    b"\x95\xC9": "亘",
    b"\x95\xCA": "諦",
    b"\x95\xCB": "謎",
    b"\x95\xCC": "罠",
    b"\x95\xCD": "骸",
    b"\x95\xCE": "狙",  # This has more than one encoding used in certain strings???
    b"\x95\xCF": "儀",
    b"\x95\xD0": "狙",
    b"\x95\xD1": "α",
    b"\x95\xD2": "β",
    b"\x95\xD3": "γ",  # Lower case gamma
    b"\x95\xD4": "/",
    b"\x95\xD5": "'",
    b"\x95\xD6": "\"",  # Is this ever used? seems to show up inside a typo (ID df da 2e a2)
    b"\x95\xD7": "％",  # Use full width % sign to avoid conflict
    b"\x95\xD8": "儀",
    b"\x95\xD9": "冑",
    b"\x95\xDA": "鎧",
    b"\x95\xDB": "「",
    b"\x95\xDC": "」",
    b"\x95\xDD": "掴",
    b"\x95\xDE": "閃",
    b"\x95\xDF": "填",
    b"\x95\xE0": "蹴",
    b"\x95\xE1": "牙",
    b"\x95\xE2": "痺",
    b"\x95\xE3": "隕",
    b"\x95\xE4": "呪",
    b"\x95\xE5": "誰",
    b"\x95\xE6": "??",
    b"\x95\xE7": "??",
    b"\x95\xE8": "??",
    b"\x95\xE9": "??",
    b"\x95\xEA": "??",
    b"\x95\xEB": "??",
    b"\x95\xEC": "??",
    b"\x95\xED": "??",
    b"\x95\xEE": "??",
    b"\x95\xEF": "??",
    b"\x95\xF0": "??",
    b"\x95\xF1": "??",
    b"\x95\xF2": "??",
    b"\x95\xF3": "??",
    b"\x95\xF4": "??",
    b"\x95\xF5": "??",
    b"\x95\xF6": "??",
    b"\x95\xF7": "??",
    b"\x95\xF8": "??",
    b"\x95\xF9": "??",
    b"\x95\xFA": "??",
    b"\x95\xFB": "??",
    b"\x95\xFC": "??",
    #b"\x95\xFD": "??",
    #b"\x95\xFE": "??",
    b"\x95\xFF": "哉",  # ??? shouldn't this be invalid?
# Characters appear to end here
}

# Some strings are encoded with password.gf
PASSWORD_STR_IDS = [
    "d254c9df",
    "3355c9df",
    "9455c9df",
    "f555c9df",
    "5656c9df",
    "b756c9df",
    "1857c9df",
    "7957c9df",

    "a837ce8c",
    "0938ce8c",
    "6a38ce8c",
    "cb38ce8c",

    "0496f38d",
    "6596f38d",
    "c696f38d",
    "2797f38d",
    "8897f38d",
    "e997f38d",
    "4a98f38d",
    "ab98f38d",

    "7a0349d5",
    "db0349d5",
    "3c0449d5",
    "9d0449d5",
    "fe0449d5",
]
BYTES_TO_CHAR_PASSWORD = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "評",
    b"\x88\xA0": "価",
    b"\x88\xA1": "ひ",
    b"\x88\xA2": "ょ",
    b"\x88\xA3": "う",
    b"\x88\xA4": "か",
    b"\x88\xA5": "ポ",
    b"\x88\xA6": "イ",
    b"\x88\xA7": "ン",
    b"\x88\xA8": "ト",
    b"\x88\xA9": "獲",
    b"\x88\xAA": "得",
    b"\x88\xAB": "く",
    b"\x88\xAC": "と",
    b"\x88\xAD": "チ",
    b"\x88\xAE": "ッ",
    b"\x88\xAF": "プ",
    b"\x88\xB0": "ラ",
    b"\x88\xB1": "ウ",
    b"\x88\xB2": "ド",
    b"\x88\xB3": "結",
    b"\x88\xB4": "果",
    b"\x88\xB5": "け",
    b"\x88\xB6": "っ",
    b"\x88\xB7": "途",
    b"\x88\xB8": "中",
    b"\x88\xB9": "集",
    b"\x88\xBA": "計",
    b"\x88\xBB": "ち",
    b"\x88\xBC": "ゅ",
    b"\x88\xBD": "し",  # TODO: doublecheck this, the font is kinda ass
    b"\x88\xBE": "い",
    b"\x88\xBF": "最",
    b"\x88\xC0": "終",
    b"\x88\xC1": "さ",
    b"\x88\xC2": "ミ",
    b"\x88\xC3": "シ",
    b"\x88\xC4": "ョ",
    b"\x88\xC5": "フ",
    b"\x88\xC6": "ェ",
    b"\x88\xC7": "ズ",
    b"\x88\xC8": "ゲ",
    b"\x88\xC9": "ュ",
    b"\x88\xCA": "ボ",
    b"\x88\xCB": "ー",
    b"\x88\xCC": "ナ",
    b"\x88\xCD": "ス",
    b"\x88\xCE": "コ",
    b"\x88\xCF": "H",
    b"\x88\xD0": "P",
    b"\x88\xD1": "パ",
    b"\x88\xD2": "ル",
    b"\x88\xD3": "タ",
    b"\x88\xD4": "ム",
    b"\x88\xD5": "撃",
    b"\x88\xD6": "破",
    b"\x88\xD7": "1",
    b"\x88\xD8": "2",
    b"\x88\xD9": "3",
    b"\x88\xDA": "4",
    b"\x88\xDB": "5",
    b"\x88\xDC": "6",
    b"\x88\xDD": "7",
    b"\x88\xDE": "8",
    b"\x88\xDF": "9",
    b"\x88\xE0": "+",
    b"\x88\xE1": "A",
    b"\x88\xE2": "B",
    b"\x88\xE3": "C",
    b"\x88\xE4": "D",
    b"\x88\xE5": "E",
    b"\x88\xE6": "F",
    b"\x88\xE7": "G",
    b"\x88\xE8": "I",
    b"\x88\xE9": "J",
    b"\x88\xEA": "K",
    b"\x88\xEB": "L",
    b"\x88\xEC": "M",
    b"\x88\xED": "N",
    b"\x88\xEE": "-",
    b"\x88\xEF": "Q",
    b"\x88\xF0": "R",
    b"\x88\xF1": "S",
    b"\x88\xF2": "T",
    b"\x88\xF3": "U",
    b"\x88\xF4": "V",
    b"\x88\xF5": "W",
    b"\x88\xF6": "X",
    b"\x88\xF7": "Y",
    b"\x88\xF8": "Z",
    b"\x88\xF9": "a",
    b"\x88\xFA": "b",
    b"\x88\xFB": "c",
    b"\x88\xFC": "d",
    b"\x89\x40": "e",
    b"\x89\x41": "f",
    b"\x89\x42": "g",
    b"\x89\x43": "h",
    b"\x89\x44": "i",
    b"\x89\x45": "j",
    b"\x89\x46": "k",
    b"\x89\x47": "l",
    b"\x89\x48": "m",
    b"\x89\x49": "n",
    b"\x89\x4A": "o",
    b"\x89\x4B": "p",
    b"\x89\x4C": "q",
    b"\x89\x4D": "r",
    b"\x89\x4E": "s",
    b"\x89\x4F": "t",
    b"\x89\x50": "u",
    b"\x89\x51": "v",
    b"\x89\x52": "w",
    b"\x89\x53": "x",
    b"\x89\x54": "y",
    b"\x89\x55": "z",
    b"\x89\x56": "!",
    b"\x89\x57": "?",
    b"\x89\x58": "ネ",
    b"\x89\x59": "ワ",
    b"\x89\x5A": "ク",
    b"\x89\x5B": " ",
    b"\x89\x5C": "エ",
    b"\x89\x5D": "エ", # Duplicated?
    # Valid encodings seem to end here

    # HACK: patch in custom characters to support all strings
    b"\x89\x5E": ".",
}

BYTES_TO_CHAR_TOKYO_TELEPORT = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "東",
    b"\x88\xA0": "京",
    b"\x88\xA1": "テ",
    b"\x88\xA2": "レ",
    b"\x88\xA3": "ポ",
    b"\x88\xA4": "ー",
    b"\x88\xA5": "ト",
    b"\x88\xA6": " ",
    b"\x88\xA7": "P",
    b"\x88\xA8": "M",
    b"\x88\xA9": "0",
    b"\x88\xAA": ":",
    b"\x88\xAB": "リ",
    b"\x88\xAC": "ラ",
    b"\x88\xAD": "イ",
    b"\x88\xAE": "タ",
    b"\x88\xAF": "ア",
    b"\x88\xB0": "け",
    b"\x88\xB1": "っ",
    b"\x88\xB2": "て",
    b"\x88\xB3": "い",
    b"\x88\xB4": "も",
    b"\x88\xB5": "ど",
    b"\x88\xB6": "る",
    b"\x88\xB7": "へ",
    b"\x88\xB8": "ん",
    b"\x88\xB9": "こ",
    b"\x88\xBA": "う",
    b"\x88\xBB": "ズ",
    b"\x88\xBC": "メ",
    b"\x88\xBD": "ニ",
    b"\x88\xBE": "ュ",
    b"\x88\xBF": "オ",
    b"\x88\xC0": "プ",
    b"\x88\xC1": "シ",
    b"\x88\xC2": "ョ",
    b"\x88\xC3": "ン",
    b"\x88\xC4": "設",
    b"\x88\xC5": "定",
    b"\x88\xC6": "そ",
    b"\x88\xC7": "さ",
    b"\x88\xC8": "説",
    b"\x88\xC9": "明",
    b"\x88\xCA": "ゲ",
    b"\x88\xCB": "ム",
    b"\x88\xCC": "に",
    b"\x88\xCD": "本",
    b"\x88\xCE": "当",
    b"\x88\xCF": "し",
    b"\x88\xD0": "ま",
    b"\x88\xD1": "す",
    b"\x88\xD2": "か",
    b"\x88\xD3": "?",
    b"\x88\xD4": "は",
    b"\x88\xD5": "え",
    b"\x88\xD6": "せ",
    b"\x88\xD7": "サ",
    b"\x88\xD8": "ウ",
    b"\x88\xD9": "ド",
    b"\x88\xDA": "バ",
    b"\x88\xDB": "ブ",
    b"\x88\xDC": "B",
    b"\x88\xDD": "G",
    b"\x88\xDE": "S",
    b"\x88\xDF": "E",
    b"\x88\xE0": "モ",
    b"\x88\xE1": "ノ",
    b"\x88\xE2": "ル",
    b"\x88\xE3": "ス",
    b"\x88\xE4": "ビ",
    b"\x88\xE5": "ロ",
    b"\x88\xE6": "ジ",
    b"\x88\xE7": "ッ",
    b"\x88\xE8": "ク",
    b"\x88\xE9": "2",
    b"\x88\xEA": "の",
    b"\x88\xEB": "出",
    b"\x88\xEC": "カ",
    b"\x88\xED": "形",
    b"\x88\xEE": "式",
    b"\x88\xEF": "ゅ",
    b"\x88\xF0": "つ",
    b"\x88\xF1": "り",
    b"\x88\xF2": "ょ",
    b"\x88\xF3": "く",
    b"\x88\xF4": "き",
    b"\x88\xF5": "を",
    b"\x88\xF6": "選",
    b"\x88\xF7": "択",
    b"\x88\xF8": "た",
    b"\x88\xF9": "だ",
    b"\x88\xFA": "。",
    b"\x88\xFB": "フ",
    b"\x88\xFC": "⋅",

    b"\x89\x40": "I",
    b"\x89\x41": "N",
    b"\x89\x42": "A",
    b"\x89\x43": "X",
    b"\x89\x44": "音",
    b"\x89\x45": "量",
    b"\x89\x46": "お", # For some reason this is duplicated
    #b"\x89\x47": "お",
    # Valid encodings seem to end here

}

BYTES_TO_CHAR_SHINJUKU = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "新",
    b"\x88\xA0": "宿",
    b"\x88\xA1": "南",
    b"\x88\xA2": "口",
    b"\x88\xA3": " ",
    b"\x88\xA4": "P",
    b"\x88\xA5": "M",
    b"\x88\xA6": "0",
    b"\x88\xA7": "2",
    b"\x88\xA8": ":",
    b"\x88\xA9": "1",
    b"\x88\xAA": "6",
    b"\x88\xAB": "ピ",
    b"\x88\xAC": "エ",
    b"\x88\xAD": "ー",
    b"\x88\xAE": "ル",
    b"\x88\xAF": "を",
    b"\x88\xB0": "発",
    b"\x88\xB1": "見",
    b"\x88\xB2": "し",
    b"\x88\xB3": "ま",
    b"\x88\xB4": "た",
    b"\x88\xB5": "!",
    b"\x88\xB6": "リ",
    b"\x88\xB7": "ト",
    b"\x88\xB8": "ラ",
    b"\x88\xB9": "イ",
    b"\x88\xBA": "タ",
    b"\x88\xBB": "ア",
    b"\x88\xBC": "け",
    b"\x88\xBD": "っ",
    b"\x88\xBE": "て",
    b"\x88\xBF": "い",
    b"\x88\xC0": "も",
    b"\x88\xC1": "ど",
    b"\x88\xC2": "る",
    b"\x88\xC3": "へ",
    b"\x88\xC4": "ん",
    b"\x88\xC5": "こ",
    b"\x88\xC6": "う",
    b"\x88\xC7": "ポ",
    b"\x88\xC8": "ズ",
    b"\x88\xC9": "メ",
    b"\x88\xCA": "ニ",
    b"\x88\xCB": "ュ",
    b"\x88\xCC": "オ",
    b"\x88\xCD": "プ",
    b"\x88\xCE": "シ",
    b"\x88\xCF": "ョ",
    b"\x88\xD0": "ン",
    b"\x88\xD1": "設",
    b"\x88\xD2": "定",
    b"\x88\xD3": "そ",
    b"\x88\xD4": "さ",
    b"\x88\xD5": "説",
    b"\x88\xD6": "明",
    b"\x88\xD7": "ゲ",
    b"\x88\xD8": "ム",
    b"\x88\xD9": "に",
    b"\x88\xDA": "本",
    b"\x88\xDB": "当",
    b"\x88\xDC": "す",
    b"\x88\xDD": "か",
    b"\x88\xDE": "?",
    b"\x88\xDF": "は",
    b"\x88\xE0": "え",
    b"\x88\xE1": "せ",
    b"\x88\xE2": "サ",
    b"\x88\xE3": "ウ",
    b"\x88\xE4": "ド",
    b"\x88\xE5": "バ",
    b"\x88\xE6": "ブ",
    b"\x88\xE7": "レ",
    b"\x88\xE8": "B",
    b"\x88\xE9": "G",
    b"\x88\xEA": "S",
    b"\x88\xEB": "E",
    b"\x88\xEC": "ホ",
    b"\x88\xED": "ノ",
    b"\x88\xEE": "ス",
    b"\x88\xEF": "テ",
    b"\x88\xF0": "ビ",
    b"\x88\xF1": "ロ",
    b"\x88\xF2": "ジ",
    b"\x88\xF3": "ッ",
    b"\x88\xF4": "ク",
    b"\x88\xF5": "の",
    b"\x88\xF6": "出",
    b"\x88\xF7": "カ",
    b"\x88\xF8": "形",
    b"\x88\xF9": "式",
    b"\x88\xFA": "ゅ",
    b"\x88\xFB": "つ",
    b"\x88\xFC": "り",
    b"\x89\x40": "ょ",
    b"\x89\x41": "く",
    b"\x89\x42": "き",
    b"\x89\x43": "選",
    b"\x89\x44": "択",
    b"\x89\x45": "だ",
    b"\x89\x46": "。", # For some reason this is duplicated
    b"\x89\x47": "フ",
    b"\x89\x48": "⋅",

    b"\x89\x49": "I",
    b"\x89\x4A": "N",
    b"\x89\x4B": "A",
    b"\x89\x4C": "X",
    b"\x89\x4D": "音",
    b"\x89\x4E": "量",
    b"\x89\x4F": "お",
    # Valid encodings seem to end here
}

BYTES_TO_CHAR_AKIHABARA = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "秋",
    b"\x88\xA0": "葉",
    b"\x88\xA1": "原",
    b"\x88\xA2": " ",
    b"\x88\xA3": "P",
    b"\x88\xA4": "M",
    b"\x88\xA5": "0",
    b"\x88\xA6": "4",
    b"\x88\xA7": ":",
    b"\x88\xA8": "1",
    b"\x88\xA9": "エ",
    b"\x88\xAA": "ン",
    b"\x88\xAB": "ジ",
    b"\x88\xAC": "破",
    b"\x88\xAD": "壊",
    b"\x88\xAE": "!",
    b"\x88\xAF": "残",
    b"\x88\xB0": "り",
    b"\x88\xB1": "3",
    b"\x88\xB2": "基",
    b"\x88\xB3": "で",
    b"\x88\xB4": "す",
    b"\x88\xB5": "2",
    b"\x88\xB6": "リ",
    b"\x88\xB7": "ト",
    b"\x88\xB8": "ラ",
    b"\x88\xB9": "イ",
    b"\x88\xBA": "タ",
    b"\x88\xBB": "ア",
    b"\x88\xBC": "け",
    b"\x88\xBD": "っ",
    b"\x88\xBE": "て",
    b"\x88\xBF": "い",
    b"\x88\xC0": "も",
    b"\x88\xC1": "ど",
    b"\x88\xC2": "る",
    b"\x88\xC3": "へ",
    b"\x88\xC4": "ん",
    b"\x88\xC5": "こ",
    b"\x88\xC6": "う",
    b"\x88\xC7": "ポ",
    b"\x88\xC8": "ー",
    b"\x88\xC9": "ズ",
    b"\x88\xCA": "メ",
    b"\x88\xCB": "ニ",
    b"\x88\xCC": "ュ",
    b"\x88\xCD": "オ",
    b"\x88\xCE": "プ",
    b"\x88\xCF": "シ",
    b"\x88\xD0": "ョ",
    b"\x88\xD1": "設",
    b"\x88\xD2": "定",
    b"\x88\xD3": "そ",
    b"\x88\xD4": "さ",
    b"\x88\xD5": "説",
    b"\x88\xD6": "明",
    b"\x88\xD7": "ゲ",
    b"\x88\xD8": "ム",
    b"\x88\xD9": "に",
    b"\x88\xDA": "本",
    b"\x88\xDB": "当",
    b"\x88\xDC": "し",
    b"\x88\xDD": "ま",
    b"\x88\xDE": "か",
    b"\x88\xDF": "?",
    b"\x88\xE0": "は",
    b"\x88\xE1": "え",
    b"\x88\xE2": "せ",
    b"\x88\xE3": "サ",
    b"\x88\xE4": "ウ",
    b"\x88\xE5": "ド",
    b"\x88\xE6": "バ",
    b"\x88\xE7": "ブ",
    b"\x88\xE8": "レ",
    b"\x88\xE9": "B",
    b"\x88\xEA": "G",
    b"\x88\xEB": "S",
    b"\x88\xEC": "E",
    b"\x88\xED": "モ",
    b"\x88\xEE": "ノ",
    b"\x88\xEF": "ル",
    b"\x88\xF0": "ス",
    b"\x88\xF1": "テ",
    b"\x88\xF2": "ビ",
    b"\x88\xF3": "ロ",
    b"\x88\xF4": "ッ",
    b"\x88\xF5": "ク",
    b"\x88\xF6": "の",
    b"\x88\xF7": "出",
    b"\x88\xF8": "カ",
    b"\x88\xF9": "形",
    b"\x88\xFA": "式",
    b"\x88\xFB": "ゅ",
    b"\x88\xFC": "つ",
    b"\x89\x40": "ょ",
    b"\x89\x41": "く",
    b"\x89\x42": "き",
    b"\x89\x43": "を",
    b"\x89\x44": "選",
    b"\x89\x45": "択",
    b"\x89\x46": "た",
    b"\x89\x47": "だ",
    b"\x89\x48": "。",
    b"\x89\x49": "フ",
    b"\x89\x4A": "⋅",

    b"\x89\x4B": "I",
    b"\x89\x4C": "N",
    b"\x89\x4D": "A",
    b"\x89\x4E": "X",
    b"\x89\x4F": "音",
    b"\x89\x50": "量",
    b"\x89\x51": "お",
    # Valid encodings seem to end here
}

BYTES_TO_CHAR_METRO = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "メ",
    b"\x88\xA0": "ト",
    b"\x88\xA1": "ロ",
    b"\x88\xA2": " ",
    b"\x88\xA3": "P",
    b"\x88\xA4": "M",
    b"\x88\xA5": "0",
    b"\x88\xA6": "6",
    b"\x88\xA7": ":",
    b"\x88\xA8": "3",
    b"\x88\xA9": "2",
    b"\x88\xAA": "ク",
    b"\x88\xAB": "を",
    b"\x88\xAC": "発",
    b"\x88\xAD": "見",
    b"\x88\xAE": "し",
    b"\x88\xAF": "ま",
    b"\x88\xB0": "た",
    b"\x88\xB1": "!",
    b"\x88\xB2": "リ",
    b"\x88\xB3": "ラ",
    b"\x88\xB4": "イ",
    b"\x88\xB5": "タ",
    b"\x88\xB6": "ア",
    b"\x88\xB7": "け",
    b"\x88\xB8": "っ",
    b"\x88\xB9": "て",
    b"\x88\xBA": "い",
    b"\x88\xBB": "も",
    b"\x88\xBC": "ど",
    b"\x88\xBD": "る",
    b"\x88\xBE": "へ",
    b"\x88\xBF": "ん",
    b"\x88\xC0": "こ",
    b"\x88\xC1": "う",
    b"\x88\xC2": "ポ",
    b"\x88\xC3": "ー",
    b"\x88\xC4": "ズ",
    b"\x88\xC5": "ニ",
    b"\x88\xC6": "ュ",
    b"\x88\xC7": "オ",
    b"\x88\xC8": "プ",
    b"\x88\xC9": "シ",
    b"\x88\xCA": "ョ",
    b"\x88\xCB": "ン",
    b"\x88\xCC": "設",
    b"\x88\xCD": "定",
    b"\x88\xCE": "そ",
    b"\x88\xCF": "さ",
    b"\x88\xD0": "説",
    b"\x88\xD1": "明",
    b"\x88\xD2": "ゲ",
    b"\x88\xD3": "ム",
    b"\x88\xD4": "に",
    b"\x88\xD5": "本",
    b"\x88\xD6": "当",
    b"\x88\xD7": "す",
    b"\x88\xD8": "か",
    b"\x88\xD9": "?",
    b"\x88\xDA": "は",
    b"\x88\xDB": "え",
    b"\x88\xDC": "せ",
    b"\x88\xDD": "サ",
    b"\x88\xDE": "ウ",
    b"\x88\xDF": "ド",
    b"\x88\xE0": "バ",
    b"\x88\xE1": "ブ",
    b"\x88\xE2": "レ",
    b"\x88\xE3": "B",
    b"\x88\xE4": "G",
    b"\x88\xE5": "S",
    b"\x88\xE6": "E",
    b"\x88\xE7": "モ",
    b"\x88\xE8": "ノ",
    b"\x88\xE9": "ル",
    b"\x88\xEA": "ス",
    b"\x88\xEB": "テ",
    b"\x88\xEC": "ビ",
    b"\x88\xED": "ジ",
    b"\x88\xEE": "ッ",
    b"\x88\xEF": "の",
    b"\x88\xF0": "出",
    b"\x88\xF1": "カ",
    b"\x88\xF2": "形",
    b"\x88\xF3": "式",
    b"\x88\xF4": "ゅ",
    b"\x88\xF5": "つ",
    b"\x88\xF6": "り",
    b"\x88\xF7": "ょ",
    b"\x88\xF8": "く",
    b"\x88\xF9": "き",
    b"\x88\xFA": "選",
    b"\x88\xFB": "択",
    b"\x88\xFC": "だ",
    b"\x89\x40": "。",
    b"\x89\x41": "フ",
    b"\x89\x42": "⋅",

    b"\x89\x43": "I",
    b"\x89\x44": "N",
    b"\x89\x45": "A",
    b"\x89\x46": "X",
    b"\x89\x47": "音",
    b"\x89\x48": "量",
    b"\x89\x49": "お",
    # Valid encodings seem to end here
}

BYTES_TO_CHAR_TOKYO = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "東",
    b"\x88\xA0": "京",
    b"\x88\xA1": " ",
    b"\x88\xA2": "P",
    b"\x88\xA3": "M",
    b"\x88\xA4": "1",
    b"\x88\xA5": ":",
    b"\x88\xA6": "5",
    b"\x88\xA7": "8",
    b"\x88\xA8": "リ",
    b"\x88\xA9": "ト",
    b"\x88\xAA": "ラ",
    b"\x88\xAB": "イ",
    b"\x88\xAC": "タ",
    b"\x88\xAD": "ア",
    b"\x88\xAE": "け",
    b"\x88\xAF": "っ",
    b"\x88\xB0": "て",
    b"\x88\xB1": "い",
    b"\x88\xB2": "も",
    b"\x88\xB3": "ど",
    b"\x88\xB4": "る",
    b"\x88\xB5": "へ",
    b"\x88\xB6": "ん",
    b"\x88\xB7": "こ",
    b"\x88\xB8": "う",
    b"\x88\xB9": "ポ",
    b"\x88\xBA": "ー",
    b"\x88\xBB": "ズ",
    b"\x88\xBC": "メ",
    b"\x88\xBD": "ニ",
    b"\x88\xBE": "ュ",
    b"\x88\xBF": "オ",
    b"\x88\xC0": "プ",
    b"\x88\xC1": "シ",
    b"\x88\xC2": "ョ",
    b"\x88\xC3": "ン",
    b"\x88\xC4": "設",
    b"\x88\xC5": "定",
    b"\x88\xC6": "そ",
    b"\x88\xC7": "さ",
    b"\x88\xC8": "説",
    b"\x88\xC9": "明",
    b"\x88\xCA": "ゲ",
    b"\x88\xCB": "ム",
    b"\x88\xCC": "に",
    b"\x88\xCD": "本",
    b"\x88\xCE": "当",
    b"\x88\xCF": "し",
    b"\x88\xD0": "ま",
    b"\x88\xD1": "す",
    b"\x88\xD2": "か",
    b"\x88\xD3": "?",
    b"\x88\xD4": "は",
    b"\x88\xD5": "え",
    b"\x88\xD6": "せ",
    b"\x88\xD7": "サ",
    b"\x88\xD8": "ウ",
    b"\x88\xD9": "ド",
    b"\x88\xDA": "バ",
    b"\x88\xDB": "ブ",
    b"\x88\xDC": "レ",
    b"\x88\xDD": "B",
    b"\x88\xDE": "G",
    b"\x88\xDF": "S",
    b"\x88\xE0": "E",
    b"\x88\xE1": "モ",
    b"\x88\xE2": "ノ",
    b"\x88\xE3": "ル",
    b"\x88\xE4": "ス",
    b"\x88\xE5": "テ",
    b"\x88\xE6": "ビ",
    b"\x88\xE7": "ロ",
    b"\x88\xE8": "ジ",
    b"\x88\xE9": "ッ",
    b"\x88\xEA": "ク",
    b"\x88\xEB": "2",
    b"\x88\xEC": "の",
    b"\x88\xED": "出",
    b"\x88\xEE": "カ",
    b"\x88\xEF": "形",
    b"\x88\xF0": "式",
    b"\x88\xF1": "ゅ",
    b"\x88\xF2": "つ",
    b"\x88\xF3": "り",
    b"\x88\xF4": "ょ",
    b"\x88\xF5": "く",
    b"\x88\xF6": "き",
    b"\x88\xF7": "を",
    b"\x88\xF8": "選",
    b"\x88\xF9": "択",
    b"\x88\xFA": "た",
    b"\x88\xFB": "だ",
    b"\x88\xFC": "。",
    b"\x89\x40": "フ",
    b"\x89\x41": "⋅",

    b"\x89\x42": "I",
    b"\x89\x43": "N",
    b"\x89\x44": "A",
    b"\x89\x45": "X",
    b"\x89\x46": "音",
    b"\x89\x47": "量",
    b"\x89\x48": "お",
    b"\x89\x49": "お", # Repeated?
    # Valid encodings seem to end here
}

BYTES_TO_CHAR_TOKYO = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "東",
    b"\x88\xA0": "京",
    b"\x88\xA1": " ",
    b"\x88\xA2": "P",
    b"\x88\xA3": "M",
    b"\x88\xA4": "1",
    b"\x88\xA5": ":",
    b"\x88\xA6": "5",
    b"\x88\xA7": "8",
    b"\x88\xA8": "リ",
    b"\x88\xA9": "ト",
    b"\x88\xAA": "ラ",
    b"\x88\xAB": "イ",
    b"\x88\xAC": "タ",
    b"\x88\xAD": "ア",
    b"\x88\xAE": "け",
    b"\x88\xAF": "っ",
    b"\x88\xB0": "て",
    b"\x88\xB1": "い",
    b"\x88\xB2": "も",
    b"\x88\xB3": "ど",
    b"\x88\xB4": "る",
    b"\x88\xB5": "へ",
    b"\x88\xB6": "ん",
    b"\x88\xB7": "こ",
    b"\x88\xB8": "う",
    b"\x88\xB9": "ポ",
    b"\x88\xBA": "ー",
    b"\x88\xBB": "ズ",
    b"\x88\xBC": "メ",
    b"\x88\xBD": "ニ",
    b"\x88\xBE": "ュ",
    b"\x88\xBF": "オ",
    b"\x88\xC0": "プ",
    b"\x88\xC1": "シ",
    b"\x88\xC2": "ョ",
    b"\x88\xC3": "ン",
    b"\x88\xC4": "設",
    b"\x88\xC5": "定",
    b"\x88\xC6": "そ",
    b"\x88\xC7": "さ",
    b"\x88\xC8": "説",
    b"\x88\xC9": "明",
    b"\x88\xCA": "ゲ",
    b"\x88\xCB": "ム",
    b"\x88\xCC": "に",
    b"\x88\xCD": "本",
    b"\x88\xCE": "当",
    b"\x88\xCF": "し",
    b"\x88\xD0": "ま",
    b"\x88\xD1": "す",
    b"\x88\xD2": "か",
    b"\x88\xD3": "?",
    b"\x88\xD4": "は",
    b"\x88\xD5": "え",
    b"\x88\xD6": "せ",
    b"\x88\xD7": "サ",
    b"\x88\xD8": "ウ",
    b"\x88\xD9": "ド",
    b"\x88\xDA": "バ",
    b"\x88\xDB": "ブ",
    b"\x88\xDC": "レ",
    b"\x88\xDD": "B",
    b"\x88\xDE": "G",
    b"\x88\xDF": "S",
    b"\x88\xE0": "E",
    b"\x88\xE1": "モ",
    b"\x88\xE2": "ノ",
    b"\x88\xE3": "ル",
    b"\x88\xE4": "ス",
    b"\x88\xE5": "テ",
    b"\x88\xE6": "ビ",
    b"\x88\xE7": "ロ",
    b"\x88\xE8": "ジ",
    b"\x88\xE9": "ッ",
    b"\x88\xEA": "ク",
    b"\x88\xEB": "2",
    b"\x88\xEC": "の",
    b"\x88\xED": "出",
    b"\x88\xEE": "カ",
    b"\x88\xEF": "形",
    b"\x88\xF0": "式",
    b"\x88\xF1": "ゅ",
    b"\x88\xF2": "つ",
    b"\x88\xF3": "り",
    b"\x88\xF4": "ょ",
    b"\x88\xF5": "く",
    b"\x88\xF6": "き",
    b"\x88\xF7": "を",
    b"\x88\xF8": "選",
    b"\x88\xF9": "択",
    b"\x88\xFA": "た",
    b"\x88\xFB": "だ",
    b"\x88\xFC": "。",
    b"\x89\x40": "フ",
    b"\x89\x41": "⋅",

    b"\x89\x42": "I",
    b"\x89\x43": "N",
    b"\x89\x44": "A",
    b"\x89\x45": "X",
    b"\x89\x46": "音",
    b"\x89\x47": "量",
    b"\x89\x48": "お",
    b"\x89\x49": "お", # Repeated?
    # Valid encodings seem to end here
}

BYTES_TO_CHAR_CONTAINER_TERMINAL = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "コ",
    b"\x88\xA0": "ン",
    b"\x88\xA1": "テ",
    b"\x88\xA2": "ナ",
    b"\x88\xA3": "タ",
    b"\x88\xA4": "ー",
    b"\x88\xA5": "ミ",
    b"\x88\xA6": "ル",
    b"\x88\xA7": " ",
    b"\x88\xA8": "A",
    b"\x88\xA9": "M",
    b"\x88\xAA": "0",
    b"\x88\xAB": "5",
    b"\x88\xAC": ":",
    b"\x88\xAD": "8",
    b"\x88\xAE": "ク",
    b"\x88\xAF": "ロ",
    b"\x88\xB0": "を",
    b"\x88\xB1": "発",
    b"\x88\xB2": "見",
    b"\x88\xB3": "し",
    b"\x88\xB4": "ま",
    b"\x88\xB5": "た",
    b"\x88\xB6": "!",
    b"\x88\xB7": "リ",
    b"\x88\xB8": "ト",
    b"\x88\xB9": "ラ",
    b"\x88\xBA": "イ",
    b"\x88\xBB": "ア",
    b"\x88\xBC": "け",
    b"\x88\xBD": "っ",
    b"\x88\xBE": "て",
    b"\x88\xBF": "い",
    b"\x88\xC0": "も",
    b"\x88\xC1": "ど",
    b"\x88\xC2": "る",
    b"\x88\xC3": "へ",
    b"\x88\xC4": "ん",
    b"\x88\xC5": "こ",
    b"\x88\xC6": "う",
    b"\x88\xC7": "ポ",
    b"\x88\xC8": "ズ",
    b"\x88\xC9": "メ",
    b"\x88\xCA": "ニ",
    b"\x88\xCB": "ュ",
    b"\x88\xCC": "オ",
    b"\x88\xCD": "プ",
    b"\x88\xCE": "シ",
    b"\x88\xCF": "ョ",
    b"\x88\xD0": "設",
    b"\x88\xD1": "定",
    b"\x88\xD2": "そ",
    b"\x88\xD3": "さ",
    b"\x88\xD4": "説",
    b"\x88\xD5": "明",
    b"\x88\xD6": "ゲ",
    b"\x88\xD7": "ム",
    b"\x88\xD8": "に",
    b"\x88\xD9": "本",
    b"\x88\xDA": "当",
    b"\x88\xDB": "す",
    b"\x88\xDC": "か",
    b"\x88\xDD": "?",
    b"\x88\xDE": "は",
    b"\x88\xDF": "え",
    b"\x88\xE0": "せ",
    b"\x88\xE1": "サ",
    b"\x88\xE2": "ウ",
    b"\x88\xE3": "ド",
    b"\x88\xE4": "バ",
    b"\x88\xE5": "ブ",
    b"\x88\xE6": "レ",
    b"\x88\xE7": "B",
    b"\x88\xE8": "G",
    b"\x88\xE9": "S",
    b"\x88\xEA": "E",
    b"\x88\xEB": "モ",
    b"\x88\xEC": "ノ",
    b"\x88\xED": "ス",
    b"\x88\xEE": "ビ",
    b"\x88\xEF": "ジ",
    b"\x88\xF0": "ッ",
    b"\x88\xF1": "2",
    b"\x88\xF2": "の",
    b"\x88\xF3": "出",
    b"\x88\xF4": "力",
    b"\x88\xF5": "形",
    b"\x88\xF6": "式",
    b"\x88\xF7": "ゅ",
    b"\x88\xF8": "つ",
    b"\x88\xF9": "り",
    b"\x88\xFA": "ょ",
    b"\x88\xFB": "く",
    b"\x88\xFC": "き",
    b"\x89\x40": "選",
    b"\x89\x41": "択",
    b"\x89\x42": "だ",
    b"\x89\x43": "。",
    b"\x89\x44": "フ",
    b"\x89\x45": "⋅",
    b"\x89\x46": "I",
    b"\x89\x47": "N",
    b"\x89\x48": "X",
    b"\x89\x49": "音",
    b"\x89\x4A": "量",
    b"\x89\x4B": "お",
}

BYTES_TO_CHAR_INDUSTRIAL_COMPLEX = {
    **BYTES_TO_CHAR_SPECIAL,
    b"\x88\x9F": "コ",
    b"\x88\xA0": "ン",
    b"\x88\xA1": "ビ",
    b"\x88\xA2": "ナ",
    b"\x88\xA3": "ー",
    b"\x88\xA4": "ト",
    b"\x88\xA5": " ",
    b"\x88\xA6": "A",
    b"\x88\xA7": "M",
    b"\x88\xA8": "0",
    b"\x88\xA9": "8",
    b"\x88\xAA": ":",
    b"\x88\xAB": "1",
    b"\x88\xAC": "9",
    b"\x88\xAD": "ス",
    b"\x88\xAE": "ズ",
    b"\x88\xAF": "キ",
    b"\x88\xB0": "を",
    b"\x88\xB1": "発",
    b"\x88\xB2": "見",
    b"\x88\xB3": "し",
    b"\x88\xB4": "ま",
    b"\x88\xB5": "た",
    b"\x88\xB6": "!",
    b"\x88\xB7": "セ",
    b"\x88\xB8": "ュ",
    b"\x88\xB9": "リ",
    b"\x88\xBA": "テ",
    b"\x88\xBB": "ィ",
    b"\x88\xBC": "機",
    b"\x88\xBD": "銃",
    b"\x88\xBE": "停",
    b"\x88\xBF": "止",
    b"\x88\xC0": "バ",
    b"\x88\xC1": "ア",
    b"\x88\xC2": "解",
    b"\x88\xC3": "除",
    b"\x88\xC4": "タ",
    b"\x88\xC5": "ポ",
    b"\x88\xC6": "ッ",
    b"\x88\xC7": "ド",
    b"\x88\xC8": "ラ",
    b"\x88\xC9": "イ",
    b"\x88\xCA": "け",
    b"\x88\xCB": "っ",
    b"\x88\xCC": "て",
    b"\x88\xCD": "い",
    b"\x88\xCE": "も",
    b"\x88\xCF": "ど",
    b"\x88\xD0": "る",
    b"\x88\xD1": "へ",
    b"\x88\xD2": "ん",
    b"\x88\xD3": "こ",
    b"\x88\xD4": "う",
    b"\x88\xD5": "メ",
    b"\x88\xD6": "ニ",
    b"\x88\xD7": "オ",
    b"\x88\xD8": "プ",
    b"\x88\xD9": "シ",
    b"\x88\xDA": "ョ",
    b"\x88\xDB": "設",
    b"\x88\xDC": "定",
    b"\x88\xDD": "そ",
    b"\x88\xDE": "さ",
    b"\x88\xDF": "説",
    b"\x88\xE0": "明",
    b"\x88\xE1": "ゲ",
    b"\x88\xE2": "ム",
    b"\x88\xE3": "に",
    b"\x88\xE4": "本",
    b"\x88\xE5": "当",
    b"\x88\xE6": "す",
    b"\x88\xE7": "か",
    b"\x88\xE8": "?",
    b"\x88\xE9": "は",
    b"\x88\xEA": "え",
    b"\x88\xEB": "せ",
    b"\x88\xEC": "サ",
    b"\x88\xED": "ウ",
    b"\x88\xEE": "ブ",
    b"\x88\xEF": "レ",
    b"\x88\xF0": "B",
    b"\x88\xF1": "G",
    b"\x88\xF2": "S",
    b"\x88\xF3": "E",
    b"\x88\xF4": "モ",
    b"\x88\xF5": "ノ",
    b"\x88\xF6": "ル",
    b"\x88\xF7": "ロ",
    b"\x88\xF8": "ジ",
    b"\x88\xF9": "ク",
    b"\x88\xFA": "2",
    b"\x88\xFB": "の",
    b"\x88\xFC": "出",
    b"\x89\x40": "力",
    b"\x89\x41": "形",
    b"\x89\x42": "式",
    b"\x89\x43": "ゅ",
    b"\x89\x44": "つ",
    b"\x89\x45": "り",
    b"\x89\x46": "ょ",
    b"\x89\x47": "く",
    b"\x89\x48": "き",
    b"\x89\x49": "選",
    b"\x89\x4A": "択",
    b"\x89\x4B": "だ",
    b"\x89\x4C": "。",
    b"\x89\x4D": "フ",
    b"\x89\x4E": "⋅",
    b"\x89\x4F": "I",
    b"\x89\x50": "N",
    b"\x89\x51": "X",
    b"\x89\x52": "音",
    b"\x89\x53": "量",
    b"\x89\x54": "お",
    b"\x89\x55": "お",
}

BYTES_TO_CHAR_YACHT_HARBOR = {
    **BYTES_TO_CHAR_SPECIAL,
    b"\x88\x9F": "ヨ",
    b"\x88\xA0": "ッ",
    b"\x88\xA1": "ト",
    b"\x88\xA2": "ハ",
    b"\x88\xA3": "ー",
    b"\x88\xA4": "バ",
    b"\x88\xA5": " ",
    b"\x88\xA6": "P",
    b"\x88\xA7": "M",
    b"\x88\xA8": "0",
    b"\x88\xA9": "3",
    b"\x88\xAA": ":",
    b"\x88\xAB": "7",
    b"\x88\xAC": "ク",
    b"\x88\xAD": "ロ",
    b"\x88\xAE": "を",
    b"\x88\xAF": "発",
    b"\x88\xB0": "見",
    b"\x88\xB1": "し",
    b"\x88\xB2": "ま",
    b"\x88\xB3": "た",
    b"\x88\xB4": "!",
    b"\x88\xB5": "リ",
    b"\x88\xB6": "ラ",
    b"\x88\xB7": "イ",
    b"\x88\xB8": "タ",
    b"\x88\xB9": "ア",
    b"\x88\xBA": "け",
    b"\x88\xBB": "っ",
    b"\x88\xBC": "て",
    b"\x88\xBD": "い",
    b"\x88\xBE": "も",
    b"\x88\xBF": "ど",
    b"\x88\xC0": "る",
    b"\x88\xC1": "へ",
    b"\x88\xC2": "ん",
    b"\x88\xC3": "こ",
    b"\x88\xC4": "う",
    b"\x88\xC5": "ポ",
    b"\x88\xC6": "ズ",
    b"\x88\xC7": "メ",
    b"\x88\xC8": "ニ",
    b"\x88\xC9": "ュ",
    b"\x88\xCA": "オ",
    b"\x88\xCB": "プ",
    b"\x88\xCC": "シ",
    b"\x88\xCD": "ョ",
    b"\x88\xCE": "ン",
    b"\x88\xCF": "設",
    b"\x88\xD0": "定",
    b"\x88\xD1": "そ",
    b"\x88\xD2": "さ",
    b"\x88\xD3": "説",
    b"\x88\xD4": "明",
    b"\x88\xD5": "ゲ",
    b"\x88\xD6": "ム",
    b"\x88\xD7": "に",
    b"\x88\xD8": "本",
    b"\x88\xD9": "当",
    b"\x88\xDA": "す",
    b"\x88\xDB": "か",
    b"\x88\xDC": "?",
    b"\x88\xDD": "は",
    b"\x88\xDE": "え",
    b"\x88\xDF": "せ",
    b"\x88\xE0": "サ",
    b"\x88\xE1": "ウ",
    b"\x88\xE2": "ド",
    b"\x88\xE3": "ブ",
    b"\x88\xE4": "レ",
    b"\x88\xE5": "B",
    b"\x88\xE6": "G",
    b"\x88\xE7": "S",
    b"\x88\xE8": "E",
    b"\x88\xE9": "モ",
    b"\x88\xEA": "ノ",
    b"\x88\xEB": "ル",
    b"\x88\xEC": "ス",
    b"\x88\xED": "テ",
    b"\x88\xEE": "ビ",
    b"\x88\xEF": "ジ",
    b"\x88\xF0": "2",
    b"\x88\xF1": "の",
    b"\x88\xF2": "出",
    b"\x88\xF3": "力",
    b"\x88\xF4": "形",
    b"\x88\xF5": "式",
    b"\x88\xF6": "ゅ",
    b"\x88\xF7": "つ",
    b"\x88\xF8": "り",
    b"\x88\xF9": "ょ",
    b"\x88\xFA": "く",
    b"\x88\xFB": "き",
    b"\x88\xFC": "選",
    b"\x89\x40": "択",
    b"\x89\x41": "だ",
    b"\x89\x42": "。",
    b"\x89\x43": "フ",
    b"\x89\x44": "⋅",
    b"\x89\x45": "I",
    b"\x89\x46": "N",
    b"\x89\x47": "A",
    b"\x89\x48": "X",
    b"\x89\x49": "音",
    b"\x89\x4A": "量",
    b"\x89\x4B": "お",
}

BYTES_TO_CHAR_09_STADIUM_A = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ス",
    b"\x88\xA0": "タ",
    b"\x88\xA1": "ジ",
    b"\x88\xA2": "ア",
    b"\x88\xA3": "ム",
    b"\x88\xA4": " ",
    b"\x88\xA5": "P",
    b"\x88\xA6": "M",
    b"\x88\xA7": "0",
    b"\x88\xA8": "7",
    b"\x88\xA9": ":",
    b"\x88\xAA": "4",
    b"\x88\xAB": "9",
    b"\x88\xAC": "ク",
    b"\x88\xAD": "ロ",
    b"\x88\xAE": "を",
    b"\x88\xAF": "発",
    b"\x88\xB0": "見",
    b"\x88\xB1": "し",
    b"\x88\xB2": "ま",
    b"\x88\xB3": "た",
    b"\x88\xB4": "!",
    b"\x88\xB5": "リ",
    b"\x88\xB6": "ト",
    b"\x88\xB7": "ラ",
    b"\x88\xB8": "イ",
    b"\x88\xB9": "け",
    b"\x88\xBA": "っ",
    b"\x88\xBB": "て",
    b"\x88\xBC": "い",
    b"\x88\xBD": "も",
    b"\x88\xBE": "ど",
    b"\x88\xBF": "る",
    b"\x88\xC0": "へ",
    b"\x88\xC1": "ん",
    b"\x88\xC2": "こ",
    b"\x88\xC3": "う",
    b"\x88\xC4": "ポ",
    b"\x88\xC5": "ー",
    b"\x88\xC6": "ズ",
    b"\x88\xC7": "メ",
    b"\x88\xC8": "ニ",
    b"\x88\xC9": "ュ",
    b"\x88\xCA": "オ",
    b"\x88\xCB": "プ",
    b"\x88\xCC": "シ",
    b"\x88\xCD": "ョ",
    b"\x88\xCE": "ン",
    b"\x88\xCF": "設",
    b"\x88\xD0": "定",
    b"\x88\xD1": "そ",
    b"\x88\xD2": "さ",
    b"\x88\xD3": "説",
    b"\x88\xD4": "明",
    b"\x88\xD5": "ゲ",
    b"\x88\xD6": "に",
    b"\x88\xD7": "本",
    b"\x88\xD8": "当",
    b"\x88\xD9": "す",
    b"\x88\xDA": "か",
    b"\x88\xDB": "?",
    b"\x88\xDC": "は",
    b"\x88\xDD": "え",
    b"\x88\xDE": "せ",
    b"\x88\xDF": "サ",
    b"\x88\xE0": "ウ",
    b"\x88\xE1": "ド",
    b"\x88\xE2": "バ",
    b"\x88\xE3": "ブ",
    b"\x88\xE4": "レ",
    b"\x88\xE5": "B",
    b"\x88\xE6": "G",
    b"\x88\xE7": "S",
    b"\x88\xE8": "E",
    b"\x88\xE9": "モ",
    b"\x88\xEA": "ノ",
    b"\x88\xEB": "ル",
    b"\x88\xEC": "テ",
    b"\x88\xED": "ビ",
    b"\x88\xEE": "ッ",
    b"\x88\xEF": "2",
    b"\x88\xF0": "の",
    b"\x88\xF1": "出",
    b"\x88\xF2": "力",
    b"\x88\xF3": "形",
    b"\x88\xF4": "式",
    b"\x88\xF5": "ゅ",
    b"\x88\xF6": "つ",
    b"\x88\xF7": "り",
    b"\x88\xF8": "ょ",
    b"\x88\xF9": "く",
    b"\x88\xFA": "き",
    b"\x88\xFB": "選",
    b"\x88\xFC": "択",
    b"\x89\x40": "だ",
    b"\x89\x41": "。",
    b"\x89\x42": "フ",
    b"\x89\x43": "⋅",
    b"\x89\x44": "I",
    b"\x89\x45": "N",
    b"\x89\x46": "A",
    b"\x89\x47": "X",
    b"\x89\x48": "音",
    b"\x89\x49": "量",
    b"\x89\x4A": "お",
    b"\x89\x4B": "お",
}

BYTES_TO_CHAR_10_BOSS03_FLY = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ス",
    b"\x88\xA0": "タ",
    b"\x88\xA1": "ジ",
    b"\x88\xA2": "ア",
    b"\x88\xA3": "ム",
    b"\x88\xA4": " ",
    b"\x88\xA5": "P",
    b"\x88\xA6": "M",
    b"\x88\xA7": "1",
    b"\x88\xA8": ":",
    b"\x88\xA9": "8",
    b"\x88\xAA": "リ",
    b"\x88\xAB": "ト",
    b"\x88\xAC": "ラ",
    b"\x88\xAD": "イ",
    b"\x88\xAE": "け",
    b"\x88\xAF": "っ",
    b"\x88\xB0": "て",
    b"\x88\xB1": "い",
    b"\x88\xB2": "も",
    b"\x88\xB3": "ど",
    b"\x88\xB4": "る",
    b"\x88\xB5": "へ",
    b"\x88\xB6": "ん",
    b"\x88\xB7": "こ",
    b"\x88\xB8": "う",
    b"\x88\xB9": "ポ",
    b"\x88\xBA": "ー",
    b"\x88\xBB": "ズ",
    b"\x88\xBC": "メ",
    b"\x88\xBD": "ニ",
    b"\x88\xBE": "ュ",
    b"\x88\xBF": "オ",
    b"\x88\xC0": "プ",
    b"\x88\xC1": "シ",
    b"\x88\xC2": "ョ",
    b"\x88\xC3": "ン",
    b"\x88\xC4": "設",
    b"\x88\xC5": "定",
    b"\x88\xC6": "そ",
    b"\x88\xC7": "さ",
    b"\x88\xC8": "説",
    b"\x88\xC9": "明",
    b"\x88\xCA": "ゲ",
    b"\x88\xCB": "に",
    b"\x88\xCC": "本",
    b"\x88\xCD": "当",
    b"\x88\xCE": "し",
    b"\x88\xCF": "ま",
    b"\x88\xD0": "す",
    b"\x88\xD1": "か",
    b"\x88\xD2": "?",
    b"\x88\xD3": "は",
    b"\x88\xD4": "え",
    b"\x88\xD5": "せ",
    b"\x88\xD6": "サ",
    b"\x88\xD7": "ウ",
    b"\x88\xD8": "ド",
    b"\x88\xD9": "バ",
    b"\x88\xDA": "ブ",
    b"\x88\xDB": "レ",
    b"\x88\xDC": "B",
    b"\x88\xDD": "G",
    b"\x88\xDE": "S",
    b"\x88\xDF": "E",
    b"\x88\xE0": "モ",
    b"\x88\xE1": "ノ",
    b"\x88\xE2": "ル",
    b"\x88\xE3": "テ",
    b"\x88\xE4": "ビ",
    b"\x88\xE5": "ロ",
    b"\x88\xE6": "ッ",
    b"\x88\xE7": "ク",
    b"\x88\xE8": "2",
    b"\x88\xE9": "の",
    b"\x88\xEA": "出",
    b"\x88\xEB": "力",
    b"\x88\xEC": "形",
    b"\x88\xED": "式",
    b"\x88\xEE": "ゅ",
    b"\x88\xEF": "つ",
    b"\x88\xF0": "り",
    b"\x88\xF1": "ょ",
    b"\x88\xF2": "く",
    b"\x88\xF3": "き",
    b"\x88\xF4": "を",
    b"\x88\xF5": "選",
    b"\x88\xF6": "択",
    b"\x88\xF7": "た",
    b"\x88\xF8": "だ",
    b"\x88\xF9": "。",
    b"\x88\xFA": "フ",
    b"\x88\xFB": "⋅",
    b"\x88\xFC": "I",
    b"\x89\x40": "N",
    b"\x89\x41": "A",
    b"\x89\x42": "X",
    b"\x89\x43": "音",
    b"\x89\x44": "量",
    b"\x89\x45": "お",
}

BYTES_TO_CHAR_11_HANGAR01_A = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "転",
    b"\x88\xA0": "送",
    b"\x88\xA1": "ル",
    b"\x88\xA2": "ー",
    b"\x88\xA3": "ム",
    b"\x88\xA4": " ",
    b"\x88\xA5": "A",
    b"\x88\xA6": "M",
    b"\x88\xA7": "0",
    b"\x88\xA8": "1",
    b"\x88\xA9": ":",
    b"\x88\xAA": "リ",
    b"\x88\xAB": "ト",
    b"\x88\xAC": "ラ",
    b"\x88\xAD": "イ",
    b"\x88\xAE": "タ",
    b"\x88\xAF": "ア",
    b"\x88\xB0": "け",
    b"\x88\xB1": "っ",
    b"\x88\xB2": "て",
    b"\x88\xB3": "い",
    b"\x88\xB4": "も",
    b"\x88\xB5": "ど",
    b"\x88\xB6": "る",
    b"\x88\xB7": "へ",
    b"\x88\xB8": "ん",
    b"\x88\xB9": "こ",
    b"\x88\xBA": "う",
    b"\x88\xBB": "ポ",
    b"\x88\xBC": "ズ",
    b"\x88\xBD": "メ",
    b"\x88\xBE": "ニ",
    b"\x88\xBF": "ュ",
    b"\x88\xC0": "オ",
    b"\x88\xC1": "プ",
    b"\x88\xC2": "シ",
    b"\x88\xC3": "ョ",
    b"\x88\xC4": "ン",
    b"\x88\xC5": "設",
    b"\x88\xC6": "定",
    b"\x88\xC7": "そ",
    b"\x88\xC8": "さ",
    b"\x88\xC9": "説",
    b"\x88\xCA": "明",
    b"\x88\xCB": "ゲ",
    b"\x88\xCC": "に",
    b"\x88\xCD": "本",
    b"\x88\xCE": "当",
    b"\x88\xCF": "し",
    b"\x88\xD0": "ま",
    b"\x88\xD1": "す",
    b"\x88\xD2": "か",
    b"\x88\xD3": "?",
    b"\x88\xD4": "は",
    b"\x88\xD5": "え",
    b"\x88\xD6": "せ",
    b"\x88\xD7": "サ",
    b"\x88\xD8": "ウ",
    b"\x88\xD9": "ド",
    b"\x88\xDA": "バ",
    b"\x88\xDB": "ブ",
    b"\x88\xDC": "レ",
    b"\x88\xDD": "B",
    b"\x88\xDE": "G",
    b"\x88\xDF": "S",
    b"\x88\xE0": "E",
    b"\x88\xE1": "モ",
    b"\x88\xE2": "ノ",
    b"\x88\xE3": "ス",
    b"\x88\xE4": "テ",
    b"\x88\xE5": "ビ",
    b"\x88\xE6": "ロ",
    b"\x88\xE7": "ジ",
    b"\x88\xE8": "ッ",
    b"\x88\xE9": "ク",
    b"\x88\xEA": "2",
    b"\x88\xEB": "の",
    b"\x88\xEC": "出",
    b"\x88\xED": "力",
    b"\x88\xEE": "形",
    b"\x88\xEF": "式",
    b"\x88\xF0": "ゅ",
    b"\x88\xF1": "つ",
    b"\x88\xF2": "り",
    b"\x88\xF3": "ょ",
    b"\x88\xF4": "く",
    b"\x88\xF5": "き",
    b"\x88\xF6": "を",
    b"\x88\xF7": "選",
    b"\x88\xF8": "択",
    b"\x88\xF9": "た",
    b"\x88\xFA": "だ",
    b"\x88\xFB": "。",
    b"\x88\xFC": "フ",
    b"\x89\x40": "⋅",
    b"\x89\x41": "I",
    b"\x89\x42": "N",
    b"\x89\x43": "X",
    b"\x89\x44": "音",
    b"\x89\x45": "量",
    b"\x89\x46": "お",
    b"\x89\x47": "お",
}

BYTES_TO_CHAR_12_HANGAR02_A = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "重",
    b"\x88\xA0": "機",
    b"\x88\xA1": "格",
    b"\x88\xA2": "納",
    b"\x88\xA3": "庫",
    b"\x88\xA4": " ",
    b"\x88\xA5": "A",
    b"\x88\xA6": "M",
    b"\x88\xA7": "0",
    b"\x88\xA8": "6",
    b"\x88\xA9": ":",
    b"\x88\xAA": "1",
    b"\x88\xAB": "リ",
    b"\x88\xAC": "ト",
    b"\x88\xAD": "ラ",
    b"\x88\xAE": "イ",
    b"\x88\xAF": "タ",
    b"\x88\xB0": "ア",
    b"\x88\xB1": "け",
    b"\x88\xB2": "っ",
    b"\x88\xB3": "て",
    b"\x88\xB4": "い",
    b"\x88\xB5": "も",
    b"\x88\xB6": "ど",
    b"\x88\xB7": "る",
    b"\x88\xB8": "へ",
    b"\x88\xB9": "ん",
    b"\x88\xBA": "こ",
    b"\x88\xBB": "う",
    b"\x88\xBC": "ポ",
    b"\x88\xBD": "ー",
    b"\x88\xBE": "ズ",
    b"\x88\xBF": "メ",
    b"\x88\xC0": "ニ",
    b"\x88\xC1": "ュ",
    b"\x88\xC2": "オ",
    b"\x88\xC3": "プ",
    b"\x88\xC4": "シ",
    b"\x88\xC5": "ョ",
    b"\x88\xC6": "ン",
    b"\x88\xC7": "設",
    b"\x88\xC8": "定",
    b"\x88\xC9": "そ",
    b"\x88\xCA": "さ",
    b"\x88\xCB": "説",
    b"\x88\xCC": "明",
    b"\x88\xCD": "ゲ",
    b"\x88\xCE": "ム",
    b"\x88\xCF": "に",
    b"\x88\xD0": "本",
    b"\x88\xD1": "当",
    b"\x88\xD2": "し",
    b"\x88\xD3": "ま",
    b"\x88\xD4": "す",
    b"\x88\xD5": "か",
    b"\x88\xD6": "?",
    b"\x88\xD7": "は",
    b"\x88\xD8": "え",
    b"\x88\xD9": "せ",
    b"\x88\xDA": "サ",
    b"\x88\xDB": "ウ",
    b"\x88\xDC": "ド",
    b"\x88\xDD": "バ",
    b"\x88\xDE": "ブ",
    b"\x88\xDF": "レ",
    b"\x88\xE0": "B",
    b"\x88\xE1": "G",
    b"\x88\xE2": "S",
    b"\x88\xE3": "E",
    b"\x88\xE4": "モ",
    b"\x88\xE5": "ノ",
    b"\x88\xE6": "ル",
    b"\x88\xE7": "ス",
    b"\x88\xE8": "テ",
    b"\x88\xE9": "ビ",
    b"\x88\xEA": "ロ",
    b"\x88\xEB": "ジ",
    b"\x88\xEC": "ッ",
    b"\x88\xED": "ク",
    b"\x88\xEE": "2",
    b"\x88\xEF": "の",
    b"\x88\xF0": "出",
    b"\x88\xF1": "力",
    b"\x88\xF2": "形",
    b"\x88\xF3": "式",
    b"\x88\xF4": "ゅ",
    b"\x88\xF5": "つ",
    b"\x88\xF6": "り",
    b"\x88\xF7": "ょ",
    b"\x88\xF8": "く",
    b"\x88\xF9": "き",
    b"\x88\xFA": "を",
    b"\x88\xFB": "選",
    b"\x88\xFC": "択",
    b"\x89\x40": "た",
    b"\x89\x41": "だ",
    b"\x89\x42": "。",
    b"\x89\x43": "フ",
    b"\x89\x44": "⋅",
    b"\x89\x45": "I",
    b"\x89\x46": "N",
    b"\x89\x47": "X",
    b"\x89\x48": "音",
    b"\x89\x49": "量",
    b"\x89\x4A": "お",
    b"\x89\x4B": "お",
}

BYTES_TO_CHAR_13_BOSS04_GORI02 = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "G",
    b"\x88\xA0": "専",
    b"\x88\xA1": "用",
    b"\x88\xA2": "格",
    b"\x88\xA3": "納",
    b"\x88\xA4": "庫",
    b"\x88\xA5": " ",
    b"\x88\xA6": "A",
    b"\x88\xA7": "M",
    b"\x88\xA8": "1",
    b"\x88\xA9": "0",
    b"\x88\xAA": ":",
    b"\x88\xAB": "4",
    b"\x88\xAC": "8",
    b"\x88\xAD": "リ",
    b"\x88\xAE": "ト",
    b"\x88\xAF": "ラ",
    b"\x88\xB0": "イ",
    b"\x88\xB1": "タ",
    b"\x88\xB2": "ア",
    b"\x88\xB3": "け",
    b"\x88\xB4": "っ",
    b"\x88\xB5": "て",
    b"\x88\xB6": "い",
    b"\x88\xB7": "も",
    b"\x88\xB8": "ど",
    b"\x88\xB9": "る",
    b"\x88\xBA": "へ",
    b"\x88\xBB": "ん",
    b"\x88\xBC": "こ",
    b"\x88\xBD": "う",
    b"\x88\xBE": "ポ",
    b"\x88\xBF": "ー",
    b"\x88\xC0": "ズ",
    b"\x88\xC1": "メ",
    b"\x88\xC2": "ニ",
    b"\x88\xC3": "ュ",
    b"\x88\xC4": "オ",
    b"\x88\xC5": "プ",
    b"\x88\xC6": "シ",
    b"\x88\xC7": "ョ",
    b"\x88\xC8": "ン",
    b"\x88\xC9": "設",
    b"\x88\xCA": "定",
    b"\x88\xCB": "そ",
    b"\x88\xCC": "さ",
    b"\x88\xCD": "説",
    b"\x88\xCE": "明",
    b"\x88\xCF": "ゲ",
    b"\x88\xD0": "ム",
    b"\x88\xD1": "に",
    b"\x88\xD2": "本",
    b"\x88\xD3": "当",
    b"\x88\xD4": "し",
    b"\x88\xD5": "ま",
    b"\x88\xD6": "す",
    b"\x88\xD7": "か",
    b"\x88\xD8": "?",
    b"\x88\xD9": "は",
    b"\x88\xDA": "え",
    b"\x88\xDB": "せ",
    b"\x88\xDC": "サ",
    b"\x88\xDD": "ウ",
    b"\x88\xDE": "ド",
    b"\x88\xDF": "バ",
    b"\x88\xE0": "ブ",
    b"\x88\xE1": "レ",
    b"\x88\xE2": "B",
    b"\x88\xE3": "S",
    b"\x88\xE4": "E",
    b"\x88\xE5": "モ",
    b"\x88\xE6": "ノ",
    b"\x88\xE7": "ル",
    b"\x88\xE8": "ス",
    b"\x88\xE9": "テ",
    b"\x88\xEA": "ビ",
    b"\x88\xEB": "ロ",
    b"\x88\xEC": "ジ",
    b"\x88\xED": "ッ",
    b"\x88\xEE": "ク",
    b"\x88\xEF": "2",
    b"\x88\xF0": "の",
    b"\x88\xF1": "出",
    b"\x88\xF2": "力",
    b"\x88\xF3": "形",
    b"\x88\xF4": "式",
    b"\x88\xF5": "ゅ",
    b"\x88\xF6": "つ",
    b"\x88\xF7": "り",
    b"\x88\xF8": "ょ",
    b"\x88\xF9": "く",
    b"\x88\xFA": "き",
    b"\x88\xFB": "を",
    b"\x88\xFC": "選",
    b"\x89\x40": "択",
    b"\x89\x41": "た",
    b"\x89\x42": "だ",
    b"\x89\x43": "。",
    b"\x89\x44": "フ",
    b"\x89\x45": "⋅",
    b"\x89\x46": "I",
    b"\x89\x47": "N",
    b"\x89\x48": "X",
    b"\x89\x49": "音",
    b"\x89\x4A": "量",
    b"\x89\x4B": "お",
    b"\x89\x6A": " ",
    b"\x89\x6B": " ",
    b"\x89\x72": " ",
    b"\x89\x73": " ",
    b"\x89\x74": " ",
    b"\x89\x75": " ",
    b"\x89\x76": " ",
    b"\x89\x77": " ",
}

BYTES_TO_CHAR_14_ELEVATOR_A = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "搬",
    b"\x88\xA0": "入",
    b"\x88\xA1": "エ",
    b"\x88\xA2": "レ",
    b"\x88\xA3": "ベ",
    b"\x88\xA4": "ー",
    b"\x88\xA5": "タ",
    b"\x88\xA6": " ",
    b"\x88\xA7": "P",
    b"\x88\xA8": "M",
    b"\x88\xA9": "0",
    b"\x88\xAA": ":",
    b"\x88\xAB": "3",
    b"\x88\xAC": "1",
    b"\x88\xAD": "リ",
    b"\x88\xAE": "ト",
    b"\x88\xAF": "ラ",
    b"\x88\xB0": "イ",
    b"\x88\xB1": "ア",
    b"\x88\xB2": "け",
    b"\x88\xB3": "っ",
    b"\x88\xB4": "て",
    b"\x88\xB5": "い",
    b"\x88\xB6": "も",
    b"\x88\xB7": "ど",
    b"\x88\xB8": "る",
    b"\x88\xB9": "へ",
    b"\x88\xBA": "ん",
    b"\x88\xBB": "こ",
    b"\x88\xBC": "う",
    b"\x88\xBD": "ポ",
    b"\x88\xBE": "ズ",
    b"\x88\xBF": "メ",
    b"\x88\xC0": "ニ",
    b"\x88\xC1": "ュ",
    b"\x88\xC2": "オ",
    b"\x88\xC3": "プ",
    b"\x88\xC4": "シ",
    b"\x88\xC5": "ョ",
    b"\x88\xC6": "ン",
    b"\x88\xC7": "設",
    b"\x88\xC8": "定",
    b"\x88\xC9": "そ",
    b"\x88\xCA": "さ",
    b"\x88\xCB": "説",
    b"\x88\xCC": "明",
    b"\x88\xCD": "ゲ",
    b"\x88\xCE": "ム",
    b"\x88\xCF": "に",
    b"\x88\xD0": "本",
    b"\x88\xD1": "当",
    b"\x88\xD2": "し",
    b"\x88\xD3": "ま",
    b"\x88\xD4": "す",
    b"\x88\xD5": "か",
    b"\x88\xD6": "?",
    b"\x88\xD7": "は",
    b"\x88\xD8": "え",
    b"\x88\xD9": "せ",
    b"\x88\xDA": "サ",
    b"\x88\xDB": "ウ",
    b"\x88\xDC": "ド",
    b"\x88\xDD": "バ",
    b"\x88\xDE": "ブ",
    b"\x88\xDF": "B",
    b"\x88\xE0": "G",
    b"\x88\xE1": "S",
    b"\x88\xE2": "E",
    b"\x88\xE3": "モ",
    b"\x88\xE4": "ノ",
    b"\x88\xE5": "ル",
    b"\x88\xE6": "ス",
    b"\x88\xE7": "テ",
    b"\x88\xE8": "ビ",
    b"\x88\xE9": "ロ",
    b"\x88\xEA": "ジ",
    b"\x88\xEB": "ッ",
    b"\x88\xEC": "ク",
    b"\x88\xED": "2",
    b"\x88\xEE": "の",
    b"\x88\xEF": "出",
    b"\x88\xF0": "力",
    b"\x88\xF1": "形",
    b"\x88\xF2": "式",
    b"\x88\xF3": "ゅ",
    b"\x88\xF4": "つ",
    b"\x88\xF5": "り",
    b"\x88\xF6": "ょ",
    b"\x88\xF7": "く",
    b"\x88\xF8": "き",
    b"\x88\xF9": "を",
    b"\x88\xFA": "選",
    b"\x88\xFB": "択",
    b"\x88\xFC": "た",
    b"\x89\x40": "だ",
    b"\x89\x41": "。",
    b"\x89\x42": "フ",
    b"\x89\x43": "⋅",
    b"\x89\x44": "I",
    b"\x89\x45": "N",
    b"\x89\x46": "A",
    b"\x89\x47": "X",
    b"\x89\x48": "音",
    b"\x89\x49": "量",
    b"\x89\x4A": "お",
    b"\x89\x4B": "お",
}

BYTES_TO_CHAR_15_KAKERU_SPECTOR = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ブ",
    b"\x88\xA0": "リ",
    b"\x88\xA1": "ッ",
    b"\x88\xA2": "ジ",
    b"\x88\xA3": " ",
    b"\x88\xA4": "P",
    b"\x88\xA5": "M",
    b"\x88\xA6": "0",
    b"\x88\xA7": "3",
    b"\x88\xA8": ":",
    b"\x88\xA9": "5",
    b"\x88\xAA": "2",
    b"\x88\xAB": "ト",
    b"\x88\xAC": "ラ",
    b"\x88\xAD": "イ",
    b"\x88\xAE": "タ",
    b"\x88\xAF": "ア",
    b"\x88\xB0": "け",
    b"\x88\xB1": "っ",
    b"\x88\xB2": "て",
    b"\x88\xB3": "い",
    b"\x88\xB4": "も",
    b"\x88\xB5": "ど",
    b"\x88\xB6": "る",
    b"\x88\xB7": "へ",
    b"\x88\xB8": "ん",
    b"\x88\xB9": "こ",
    b"\x88\xBA": "う",
    b"\x88\xBB": "ポ",
    b"\x88\xBC": "ー",
    b"\x88\xBD": "ズ",
    b"\x88\xBE": "メ",
    b"\x88\xBF": "ニ",
    b"\x88\xC0": "ュ",
    b"\x88\xC1": "オ",
    b"\x88\xC2": "プ",
    b"\x88\xC3": "シ",
    b"\x88\xC4": "ョ",
    b"\x88\xC5": "ン",
    b"\x88\xC6": "設",
    b"\x88\xC7": "定",
    b"\x88\xC8": "そ",
    b"\x88\xC9": "さ",
    b"\x88\xCA": "説",
    b"\x88\xCB": "明",
    b"\x88\xCC": "ゲ",
    b"\x88\xCD": "ム",
    b"\x88\xCE": "に",
    b"\x88\xCF": "本",
    b"\x88\xD0": "当",
    b"\x88\xD1": "し",
    b"\x88\xD2": "ま",
    b"\x88\xD3": "す",
    b"\x88\xD4": "か",
    b"\x88\xD5": "?",
    b"\x88\xD6": "は",
    b"\x88\xD7": "え",
    b"\x88\xD8": "せ",
    b"\x88\xD9": "サ",
    b"\x88\xDA": "ウ",
    b"\x88\xDB": "ド",
    b"\x88\xDC": "バ",
    b"\x88\xDD": "レ",
    b"\x88\xDE": "B",
    b"\x88\xDF": "G",
    b"\x88\xE0": "S",
    b"\x88\xE1": "E",
    b"\x88\xE2": "モ",
    b"\x88\xE3": "ノ",
    b"\x88\xE4": "ル",
    b"\x88\xE5": "ス",
    b"\x88\xE6": "テ",
    b"\x88\xE7": "ビ",
    b"\x88\xE8": "ロ",
    b"\x88\xE9": "ク",
    b"\x88\xEA": "の",
    b"\x88\xEB": "出",
    b"\x88\xEC": "力",
    b"\x88\xED": "形",
    b"\x88\xEE": "式",
    b"\x88\xEF": "ゅ",
    b"\x88\xF0": "つ",
    b"\x88\xF1": "り",
    b"\x88\xF2": "ょ",
    b"\x88\xF3": "く",
    b"\x88\xF4": "き",
    b"\x88\xF5": "を",
    b"\x88\xF6": "選",
    b"\x88\xF7": "択",
    b"\x88\xF8": "た",
    b"\x88\xF9": "だ",
    b"\x88\xFA": "。",
    b"\x88\xFB": "フ",
    b"\x88\xFC": "⋅",
    b"\x89\x40": "I",
    b"\x89\x41": "N",
    b"\x89\x42": "A",
    b"\x89\x43": "X",
    b"\x89\x44": "音",
    b"\x89\x45": "量",
    b"\x89\x46": "お",
    b"\x89\x47": "お",
}

BYTES_TO_CHAR_15_PIPO_SPECTOR = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ブ",
    b"\x88\xA0": "リ",
    b"\x88\xA1": "ッ",
    b"\x88\xA2": "ジ",
    b"\x88\xA3": " ",
    b"\x88\xA4": "P",
    b"\x88\xA5": "M",
    b"\x88\xA6": "0",
    b"\x88\xA7": "4",
    b"\x88\xA8": ":",
    b"\x88\xA9": "1",
    b"\x88\xAA": "ト",
    b"\x88\xAB": "ラ",
    b"\x88\xAC": "イ",
    b"\x88\xAD": "タ",
    b"\x88\xAE": "ア",
    b"\x88\xAF": "け",
    b"\x88\xB0": "っ",
    b"\x88\xB1": "て",
    b"\x88\xB2": "い",
    b"\x88\xB3": "も",
    b"\x88\xB4": "ど",
    b"\x88\xB5": "る",
    b"\x88\xB6": "へ",
    b"\x88\xB7": "ん",
    b"\x88\xB8": "こ",
    b"\x88\xB9": "う",
    b"\x88\xBA": "ポ",
    b"\x88\xBB": "ー",
    b"\x88\xBC": "ズ",
    b"\x88\xBD": "メ",
    b"\x88\xBE": "ニ",
    b"\x88\xBF": "ュ",
    b"\x88\xC0": "オ",
    b"\x88\xC1": "プ",
    b"\x88\xC2": "シ",
    b"\x88\xC3": "ョ",
    b"\x88\xC4": "ン",
    b"\x88\xC5": "設",
    b"\x88\xC6": "定",
    b"\x88\xC7": "そ",
    b"\x88\xC8": "さ",
    b"\x88\xC9": "説",
    b"\x88\xCA": "明",
    b"\x88\xCB": "ゲ",
    b"\x88\xCC": "ム",
    b"\x88\xCD": "に",
    b"\x88\xCE": "本",
    b"\x88\xCF": "当",
    b"\x88\xD0": "し",
    b"\x88\xD1": "ま",
    b"\x88\xD2": "す",
    b"\x88\xD3": "か",
    b"\x88\xD4": "?",
    b"\x88\xD5": "は",
    b"\x88\xD6": "え",
    b"\x88\xD7": "せ",
    b"\x88\xD8": "サ",
    b"\x88\xD9": "ウ",
    b"\x88\xDA": "ド",
    b"\x88\xDB": "バ",
    b"\x88\xDC": "レ",
    b"\x88\xDD": "B",
    b"\x88\xDE": "G",
    b"\x88\xDF": "S",
    b"\x88\xE0": "E",
    b"\x88\xE1": "モ",
    b"\x88\xE2": "ノ",
    b"\x88\xE3": "ル",
    b"\x88\xE4": "ス",
    b"\x88\xE5": "テ",
    b"\x88\xE6": "ビ",
    b"\x88\xE7": "ロ",
    b"\x88\xE8": "ク",
    b"\x88\xE9": "2",
    b"\x88\xEA": "の",
    b"\x88\xEB": "出",
    b"\x88\xEC": "力",
    b"\x88\xED": "形",
    b"\x88\xEE": "式",
    b"\x88\xEF": "ゅ",
    b"\x88\xF0": "つ",
    b"\x88\xF1": "り",
    b"\x88\xF2": "ょ",
    b"\x88\xF3": "く",
    b"\x88\xF4": "き",
    b"\x88\xF5": "を",
    b"\x88\xF6": "選",
    b"\x88\xF7": "択",
    b"\x88\xF8": "た",
    b"\x88\xF9": "だ",
    b"\x88\xFA": "。",
    b"\x88\xFB": "フ",
    b"\x88\xFC": "⋅",
    b"\x89\x40": "I",
    b"\x89\x41": "N",
    b"\x89\x42": "A",
    b"\x89\x43": "X",
    b"\x89\x44": "音",
    b"\x89\x45": "量",
    b"\x89\x46": "お",
    #b"\x89\x47": "お",
}

BYTES_TO_CHAR_16_KAKERU_GORITRON = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ブ",
    b"\x88\xA0": "リ",
    b"\x88\xA1": "ッ",
    b"\x88\xA2": "ジ",
    b"\x88\xA3": " ",
    b"\x88\xA4": "P",
    b"\x88\xA5": "M",
    b"\x88\xA6": "0",
    b"\x88\xA7": "4",
    b"\x88\xA8": ":",
    b"\x88\xA9": "3",
    b"\x88\xAA": "6",
    b"\x88\xAB": "ト",
    b"\x88\xAC": "ラ",
    b"\x88\xAD": "イ",
    b"\x88\xAE": "タ",
    b"\x88\xAF": "ア",
    b"\x88\xB0": "け",
    b"\x88\xB1": "っ",
    b"\x88\xB2": "て",
    b"\x88\xB3": "い",
    b"\x88\xB4": "も",
    b"\x88\xB5": "ど",
    b"\x88\xB6": "る",
    b"\x88\xB7": "へ",
    b"\x88\xB8": "ん",
    b"\x88\xB9": "こ",
    b"\x88\xBA": "う",
    b"\x88\xBB": "ポ",
    b"\x88\xBC": "ー",
    b"\x88\xBD": "ズ",
    b"\x88\xBE": "メ",
    b"\x88\xBF": "ニ",
    b"\x88\xC0": "ュ",
    b"\x88\xC1": "オ",
    b"\x88\xC2": "プ",
    b"\x88\xC3": "シ",
    b"\x88\xC4": "ョ",
    b"\x88\xC5": "ン",
    b"\x88\xC6": "設",
    b"\x88\xC7": "定",
    b"\x88\xC8": "そ",
    b"\x88\xC9": "さ",
    b"\x88\xCA": "説",
    b"\x88\xCB": "明",
    b"\x88\xCC": "ゲ",
    b"\x88\xCD": "ム",
    b"\x88\xCE": "に",
    b"\x88\xCF": "本",
    b"\x88\xD0": "当",
    b"\x88\xD1": "し",
    b"\x88\xD2": "ま",
    b"\x88\xD3": "す",
    b"\x88\xD4": "か",
    b"\x88\xD5": "?",
    b"\x88\xD6": "は",
    b"\x88\xD7": "え",
    b"\x88\xD8": "せ",
    b"\x88\xD9": "サ",
    b"\x88\xDA": "ウ",
    b"\x88\xDB": "ド",
    b"\x88\xDC": "バ",
    b"\x88\xDD": "レ",
    b"\x88\xDE": "B",
    b"\x88\xDF": "G",
    b"\x88\xE0": "S",
    b"\x88\xE1": "E",
    b"\x88\xE2": "モ",
    b"\x88\xE3": "ノ",
    b"\x88\xE4": "ル",
    b"\x88\xE5": "ス",
    b"\x88\xE6": "テ",
    b"\x88\xE7": "ビ",
    b"\x88\xE8": "ロ",
    b"\x88\xE9": "ク",
    b"\x88\xEA": "2",
    b"\x88\xEB": "の",
    b"\x88\xEC": "出",
    b"\x88\xED": "力",
    b"\x88\xEE": "形",
    b"\x88\xEF": "式",
    b"\x88\xF0": "ゅ",
    b"\x88\xF1": "つ",
    b"\x88\xF2": "り",
    b"\x88\xF3": "ょ",
    b"\x88\xF4": "く",
    b"\x88\xF5": "き",
    b"\x88\xF6": "を",
    b"\x88\xF7": "選",
    b"\x88\xF8": "択",
    b"\x88\xF9": "た",
    b"\x88\xFA": "だ",
    b"\x88\xFB": "。",
    b"\x88\xFC": "フ",
    b"\x89\x40": "⋅",
    b"\x89\x41": "I",
    b"\x89\x42": "N",
    b"\x89\x43": "A",
    b"\x89\x44": "X",
    b"\x89\x45": "音",
    b"\x89\x46": "量",
    b"\x89\x47": "お",
}

BYTES_TO_CHAR_16_PIPO_KAKERU = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ブ",
    b"\x88\xA0": "リ",
    b"\x88\xA1": "ッ",
    b"\x88\xA2": "ジ",
    b"\x88\xA3": " ",
    b"\x88\xA4": "P",
    b"\x88\xA5": "M",
    b"\x88\xA6": "0",
    b"\x88\xA7": "4",
    b"\x88\xA8": ":",
    b"\x88\xA9": "3",
    b"\x88\xAA": "ト",
    b"\x88\xAB": "ラ",
    b"\x88\xAC": "イ",
    b"\x88\xAD": "タ",
    b"\x88\xAE": "ア",
    b"\x88\xAF": "け",
    b"\x88\xB0": "っ",
    b"\x88\xB1": "て",
    b"\x88\xB2": "い",
    b"\x88\xB3": "も",
    b"\x88\xB4": "ど",
    b"\x88\xB5": "る",
    b"\x88\xB6": "へ",
    b"\x88\xB7": "ん",
    b"\x88\xB8": "こ",
    b"\x88\xB9": "う",
    b"\x88\xBA": "ポ",
    b"\x88\xBB": "ー",
    b"\x88\xBC": "ズ",
    b"\x88\xBD": "メ",
    b"\x88\xBE": "ニ",
    b"\x88\xBF": "ュ",
    b"\x88\xC0": "オ",
    b"\x88\xC1": "プ",
    b"\x88\xC2": "シ",
    b"\x88\xC3": "ョ",
    b"\x88\xC4": "ン",
    b"\x88\xC5": "設",
    b"\x88\xC6": "定",
    b"\x88\xC7": "そ",
    b"\x88\xC8": "さ",
    b"\x88\xC9": "説",
    b"\x88\xCA": "明",
    b"\x88\xCB": "ゲ",
    b"\x88\xCC": "ム",
    b"\x88\xCD": "に",
    b"\x88\xCE": "本",
    b"\x88\xCF": "当",
    b"\x88\xD0": "し",
    b"\x88\xD1": "ま",
    b"\x88\xD2": "す",
    b"\x88\xD3": "か",
    b"\x88\xD4": "?",
    b"\x88\xD5": "は",
    b"\x88\xD6": "え",
    b"\x88\xD7": "せ",
    b"\x88\xD8": "サ",
    b"\x88\xD9": "ウ",
    b"\x88\xDA": "ド",
    b"\x88\xDB": "バ",
    b"\x88\xDC": "レ",
    b"\x88\xDD": "B",
    b"\x88\xDE": "G",
    b"\x88\xDF": "S",
    b"\x88\xE0": "E",
    b"\x88\xE1": "モ",
    b"\x88\xE2": "ノ",
    b"\x88\xE3": "ル",
    b"\x88\xE4": "ス",
    b"\x88\xE5": "テ",
    b"\x88\xE6": "ビ",
    b"\x88\xE7": "ロ",
    b"\x88\xE8": "ク",
    b"\x88\xE9": "2",
    b"\x88\xEA": "の",
    b"\x88\xEB": "出",
    b"\x88\xEC": "力",
    b"\x88\xED": "形",
    b"\x88\xEE": "式",
    b"\x88\xEF": "ゅ",
    b"\x88\xF0": "つ",
    b"\x88\xF1": "り",
    b"\x88\xF2": "ょ",
    b"\x88\xF3": "く",
    b"\x88\xF4": "き",
    b"\x88\xF5": "を",
    b"\x88\xF6": "選",
    b"\x88\xF7": "択",
    b"\x88\xF8": "た",
    b"\x88\xF9": "だ",
    b"\x88\xFA": "。",
    b"\x88\xFB": "フ",
    b"\x88\xFC": "⋅",
    b"\x89\x40": "I",
    b"\x89\x41": "N",
    b"\x89\x42": "A",
    b"\x89\x43": "X",
    b"\x89\x44": "音",
    b"\x89\x45": "量",
    b"\x89\x46": "お",
    #b"\x89\x47": "お",
}

BYTES_TO_CHAR_17_CITY01_VR = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "バ",
    b"\x88\xA0": "ー",
    b"\x88\xA1": "チ",
    b"\x88\xA2": "ャ",
    b"\x88\xA3": "ル",
    b"\x88\xA4": "ス",
    b"\x88\xA5": "ペ",
    b"\x88\xA6": "0",
    b"\x88\xA7": "1",
    b"\x88\xA8": " ",
    b"\x88\xA9": "-",
    b"\x88\xAA": ":",
    b"\x88\xAB": "リ",
    b"\x88\xAC": "ト",
    b"\x88\xAD": "ラ",
    b"\x88\xAE": "イ",
    b"\x88\xAF": "タ",
    b"\x88\xB0": "ア",
    b"\x88\xB1": "け",
    b"\x88\xB2": "っ",
    b"\x88\xB3": "て",
    b"\x88\xB4": "い",
    b"\x88\xB5": "も",
    b"\x88\xB6": "ど",
    b"\x88\xB7": "る",
    b"\x88\xB8": "へ",
    b"\x88\xB9": "ん",
    b"\x88\xBA": "こ",
    b"\x88\xBB": "う",
    b"\x88\xBC": "ポ",
    b"\x88\xBD": "ズ",
    b"\x88\xBE": "メ",
    b"\x88\xBF": "ニ",
    b"\x88\xC0": "ュ",
    b"\x88\xC1": "オ",
    b"\x88\xC2": "プ",
    b"\x88\xC3": "シ",
    b"\x88\xC4": "ョ",
    b"\x88\xC5": "ン",
    b"\x88\xC6": "設",
    b"\x88\xC7": "定",
    b"\x88\xC8": "そ",
    b"\x88\xC9": "さ",
    b"\x88\xCA": "説",
    b"\x88\xCB": "明",
    b"\x88\xCC": "ゲ",
    b"\x88\xCD": "ム",
    b"\x88\xCE": "に",
    b"\x88\xCF": "本",
    b"\x88\xD0": "当",
    b"\x88\xD1": "し",
    b"\x88\xD2": "ま",
    b"\x88\xD3": "す",
    b"\x88\xD4": "か",
    b"\x88\xD5": "?",
    b"\x88\xD6": "は",
    b"\x88\xD7": "え",
    b"\x88\xD8": "せ",
    b"\x88\xD9": "サ",
    b"\x88\xDA": "ウ",
    b"\x88\xDB": "ド",
    b"\x88\xDC": "ブ",
    b"\x88\xDD": "レ",
    b"\x88\xDE": "B",
    b"\x88\xDF": "G",
    b"\x88\xE0": "M",
    b"\x88\xE1": "S",
    b"\x88\xE2": "E",
    b"\x88\xE3": "モ",
    b"\x88\xE4": "ノ",
    b"\x88\xE5": "テ",
    b"\x88\xE6": "ビ",
    b"\x88\xE7": "ロ",
    b"\x88\xE8": "ジ",
    b"\x88\xE9": "ッ",
    b"\x88\xEA": "ク",
    b"\x88\xEB": "2",
    b"\x88\xEC": "の",
    b"\x88\xED": "出",
    b"\x88\xEE": "力",
    b"\x88\xEF": "形",
    b"\x88\xF0": "式",
    b"\x88\xF1": "ゅ",
    b"\x88\xF2": "つ",
    b"\x88\xF3": "り",
    b"\x88\xF4": "ょ",
    b"\x88\xF5": "く",
    b"\x88\xF6": "き",
    b"\x88\xF7": "を",
    b"\x88\xF8": "選",
    b"\x88\xF9": "択",
    b"\x88\xFA": "た",
    b"\x88\xFB": "だ",
    b"\x88\xFC": "。",
    b"\x89\x40": "フ",
    b"\x89\x41": "⋅",
    b"\x89\x42": "I",
    b"\x89\x43": "N",
    b"\x89\x44": "A",
    b"\x89\x45": "X",
    b"\x89\x46": "音",
    b"\x89\x47": "量",
    b"\x89\x48": "お",
    #b"\x89\x49": "お",
}

BYTES_TO_CHAR_18_BAY01_VR = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "バ",
    b"\x88\xA0": "ー",
    b"\x88\xA1": "チ",
    b"\x88\xA2": "ャ",
    b"\x88\xA3": "ル",
    b"\x88\xA4": "ス",
    b"\x88\xA5": "ペ",
    b"\x88\xA6": "0",
    b"\x88\xA7": "2",
    b"\x88\xA8": " ",
    b"\x88\xA9": "-",
    b"\x88\xAA": ":",
    b"\x88\xAB": "リ",
    b"\x88\xAC": "ト",
    b"\x88\xAD": "ラ",
    b"\x88\xAE": "イ",
    b"\x88\xAF": "タ",
    b"\x88\xB0": "ア",
    b"\x88\xB1": "け",
    b"\x88\xB2": "っ",
    b"\x88\xB3": "て",
    b"\x88\xB4": "い",
    b"\x88\xB5": "も",
    b"\x88\xB6": "ど",
    b"\x88\xB7": "る",
    b"\x88\xB8": "へ",
    b"\x88\xB9": "ん",
    b"\x88\xBA": "こ",
    b"\x88\xBB": "う",
    b"\x88\xBC": "ポ",
    b"\x88\xBD": "ズ",
    b"\x88\xBE": "メ",
    b"\x88\xBF": "ニ",
    b"\x88\xC0": "ュ",
    b"\x88\xC1": "オ",
    b"\x88\xC2": "プ",
    b"\x88\xC3": "シ",
    b"\x88\xC4": "ョ",
    b"\x88\xC5": "ン",
    b"\x88\xC6": "設",
    b"\x88\xC7": "定",
    b"\x88\xC8": "そ",
    b"\x88\xC9": "さ",
    b"\x88\xCA": "説",
    b"\x88\xCB": "明",
    b"\x88\xCC": "ゲ",
    b"\x88\xCD": "ム",
    b"\x88\xCE": "に",
    b"\x88\xCF": "本",
    b"\x88\xD0": "当",
    b"\x88\xD1": "し",
    b"\x88\xD2": "ま",
    b"\x88\xD3": "す",
    b"\x88\xD4": "か",
    b"\x88\xD5": "?",
    b"\x88\xD6": "は",
    b"\x88\xD7": "え",
    b"\x88\xD8": "せ",
    b"\x88\xD9": "サ",
    b"\x88\xDA": "ウ",
    b"\x88\xDB": "ド",
    b"\x88\xDC": "ブ",
    b"\x88\xDD": "レ",
    b"\x88\xDE": "B",
    b"\x88\xDF": "G",
    b"\x88\xE0": "M",
    b"\x88\xE1": "S",
    b"\x88\xE2": "E",
    b"\x88\xE3": "モ",
    b"\x88\xE4": "ノ",
    b"\x88\xE5": "テ",
    b"\x88\xE6": "ビ",
    b"\x88\xE7": "ロ",
    b"\x88\xE8": "ジ",
    b"\x88\xE9": "ッ",
    b"\x88\xEA": "ク",
    b"\x88\xEB": "の",
    b"\x88\xEC": "出",
    b"\x88\xED": "力",
    b"\x88\xEE": "形",
    b"\x88\xEF": "式",
    b"\x88\xF0": "ゅ",
    b"\x88\xF1": "つ",
    b"\x88\xF2": "り",
    b"\x88\xF3": "ょ",
    b"\x88\xF4": "く",
    b"\x88\xF5": "き",
    b"\x88\xF6": "を",
    b"\x88\xF7": "選",
    b"\x88\xF8": "択",
    b"\x88\xF9": "た",
    b"\x88\xFA": "だ",
    b"\x88\xFB": "。",
    b"\x88\xFC": "フ",
    b"\x89\x40": "⋅",
    b"\x89\x41": "I",
    b"\x89\x42": "N",
    b"\x89\x43": "A",
    b"\x89\x44": "X",
    b"\x89\x45": "音",
    b"\x89\x46": "量",
    b"\x89\x47": "お",
}

BYTES_TO_CHAR_19_PARK01_VR = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "バ",
    b"\x88\xA0": "ー",
    b"\x88\xA1": "チ",
    b"\x88\xA2": "ャ",
    b"\x88\xA3": "ル",
    b"\x88\xA4": "ス",
    b"\x88\xA5": "ペ",
    b"\x88\xA6": "0",
    b"\x88\xA7": "3",
    b"\x88\xA8": " ",
    b"\x88\xA9": "-",
    b"\x88\xAA": ":",
    b"\x88\xAB": "リ",
    b"\x88\xAC": "ト",
    b"\x88\xAD": "ラ",
    b"\x88\xAE": "イ",
    b"\x88\xAF": "タ",
    b"\x88\xB0": "ア",
    b"\x88\xB1": "け",
    b"\x88\xB2": "っ",
    b"\x88\xB3": "て",
    b"\x88\xB4": "い",
    b"\x88\xB5": "も",
    b"\x88\xB6": "ど",
    b"\x88\xB7": "る",
    b"\x88\xB8": "へ",
    b"\x88\xB9": "ん",
    b"\x88\xBA": "こ",
    b"\x88\xBB": "う",
    b"\x88\xBC": "ポ",
    b"\x88\xBD": "ズ",
    b"\x88\xBE": "メ",
    b"\x88\xBF": "ニ",
    b"\x88\xC0": "ュ",
    b"\x88\xC1": "オ",
    b"\x88\xC2": "プ",
    b"\x88\xC3": "シ",
    b"\x88\xC4": "ョ",
    b"\x88\xC5": "ン",
    b"\x88\xC6": "設",
    b"\x88\xC7": "定",
    b"\x88\xC8": "そ",
    b"\x88\xC9": "さ",
    b"\x88\xCA": "説",
    b"\x88\xCB": "明",
    b"\x88\xCC": "ゲ",
    b"\x88\xCD": "ム",
    b"\x88\xCE": "に",
    b"\x88\xCF": "本",
    b"\x88\xD0": "当",
    b"\x88\xD1": "し",
    b"\x88\xD2": "ま",
    b"\x88\xD3": "す",
    b"\x88\xD4": "か",
    b"\x88\xD5": "?",
    b"\x88\xD6": "は",
    b"\x88\xD7": "え",
    b"\x88\xD8": "せ",
    b"\x88\xD9": "サ",
    b"\x88\xDA": "ウ",
    b"\x88\xDB": "ド",
    b"\x88\xDC": "ブ",
    b"\x88\xDD": "レ",
    b"\x88\xDE": "B",
    b"\x88\xDF": "G",
    b"\x88\xE0": "M",
    b"\x88\xE1": "S",
    b"\x88\xE2": "E",
    b"\x88\xE3": "モ",
    b"\x88\xE4": "ノ",
    b"\x88\xE5": "テ",
    b"\x88\xE6": "ビ",
    b"\x88\xE7": "ロ",
    b"\x88\xE8": "ジ",
    b"\x88\xE9": "ッ",
    b"\x88\xEA": "ク",
    b"\x88\xEB": "2",
    b"\x88\xEC": "の",
    b"\x88\xED": "出",
    b"\x88\xEE": "力",
    b"\x88\xEF": "形",
    b"\x88\xF0": "式",
    b"\x88\xF1": "ゅ",
    b"\x88\xF2": "つ",
    b"\x88\xF3": "り",
    b"\x88\xF4": "ょ",
    b"\x88\xF5": "く",
    b"\x88\xF6": "き",
    b"\x88\xF7": "を",
    b"\x88\xF8": "選",
    b"\x88\xF9": "択",
    b"\x88\xFA": "た",
    b"\x88\xFB": "だ",
    b"\x88\xFC": "。",
    b"\x89\x40": "フ",
    b"\x89\x41": "⋅",
    b"\x89\x42": "I",
    b"\x89\x43": "N",
    b"\x89\x44": "A",
    b"\x89\x45": "X",
    b"\x89\x46": "音",
    b"\x89\x47": "量",
    b"\x89\x48": "お",
    #b"\x89\x49": "お",
}

BYTES_TO_CHAR_20_BOSS07_GRID = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "バ",
    b"\x88\xA0": "ー",
    b"\x88\xA1": "チ",
    b"\x88\xA2": "ャ",
    b"\x88\xA3": "ル",
    b"\x88\xA4": "ス",
    b"\x88\xA5": "ペ",
    b"\x88\xA6": "0",
    b"\x88\xA7": "4",
    b"\x88\xA8": " ",
    b"\x88\xA9": "-",
    b"\x88\xAA": ":",
    b"\x88\xAB": "リ",
    b"\x88\xAC": "ト",
    b"\x88\xAD": "ラ",
    b"\x88\xAE": "イ",
    b"\x88\xAF": "タ",
    b"\x88\xB0": "ア",
    b"\x88\xB1": "け",
    b"\x88\xB2": "っ",
    b"\x88\xB3": "て",
    b"\x88\xB4": "い",
    b"\x88\xB5": "も",
    b"\x88\xB6": "ど",
    b"\x88\xB7": "る",
    b"\x88\xB8": "へ",
    b"\x88\xB9": "ん",
    b"\x88\xBA": "こ",
    b"\x88\xBB": "う",
    b"\x88\xBC": "ポ",
    b"\x88\xBD": "ズ",
    b"\x88\xBE": "メ",
    b"\x88\xBF": "ニ",
    b"\x88\xC0": "ュ",
    b"\x88\xC1": "オ",
    b"\x88\xC2": "プ",
    b"\x88\xC3": "シ",
    b"\x88\xC4": "ョ",
    b"\x88\xC5": "ン",
    b"\x88\xC6": "設",
    b"\x88\xC7": "定",
    b"\x88\xC8": "そ",
    b"\x88\xC9": "さ",
    b"\x88\xCA": "説",
    b"\x88\xCB": "明",
    b"\x88\xCC": "ゲ",
    b"\x88\xCD": "ム",
    b"\x88\xCE": "に",
    b"\x88\xCF": "本",
    b"\x88\xD0": "当",
    b"\x88\xD1": "し",
    b"\x88\xD2": "ま",
    b"\x88\xD3": "す",
    b"\x88\xD4": "か",
    b"\x88\xD5": "?",
    b"\x88\xD6": "は",
    b"\x88\xD7": "え",
    b"\x88\xD8": "せ",
    b"\x88\xD9": "サ",
    b"\x88\xDA": "ウ",
    b"\x88\xDB": "ド",
    b"\x88\xDC": "ブ",
    b"\x88\xDD": "レ",
    b"\x88\xDE": "B",
    b"\x88\xDF": "G",
    b"\x88\xE0": "M",
    b"\x88\xE1": "S",
    b"\x88\xE2": "E",
    b"\x88\xE3": "モ",
    b"\x88\xE4": "ノ",
    b"\x88\xE5": "テ",
    b"\x88\xE6": "ビ",
    b"\x88\xE7": "ロ",
    b"\x88\xE8": "ジ",
    b"\x88\xE9": "ッ",
    b"\x88\xEA": "ク",
    b"\x88\xEB": "2",
    b"\x88\xEC": "の",
    b"\x88\xED": "出",
    b"\x88\xEE": "力",
    b"\x88\xEF": "形",
    b"\x88\xF0": "式",
    b"\x88\xF1": "ゅ",
    b"\x88\xF2": "つ",
    b"\x88\xF3": "り",
    b"\x88\xF4": "ょ",
    b"\x88\xF5": "く",
    b"\x88\xF6": "き",
    b"\x88\xF7": "を",
    b"\x88\xF8": "選",
    b"\x88\xF9": "択",
    b"\x88\xFA": "た",
    b"\x88\xFB": "だ",
    b"\x88\xFC": "。",
    b"\x89\x40": "フ",
    b"\x89\x41": "⋅",
    b"\x89\x42": "I",
    b"\x89\x43": "N",
    b"\x89\x44": "A",
    b"\x89\x45": "X",
    b"\x89\x46": "音",
    b"\x89\x47": "量",
    b"\x89\x48": "お",
    #b"\x89\x49": "お",
}

BYTES_TO_CHAR_21_STADIUM_B = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ス",
    b"\x88\xA0": "タ",
    b"\x88\xA1": "ジ",
    b"\x88\xA2": "ア",
    b"\x88\xA3": "ム",
    b"\x88\xA4": " ",
    b"\x88\xA5": "A",
    b"\x88\xA6": "M",
    b"\x88\xA7": "0",
    b"\x88\xA8": "3",
    b"\x88\xA9": ":",
    b"\x88\xAA": "2",
    b"\x88\xAB": "ク",
    b"\x88\xAC": "ロ",
    b"\x88\xAD": "を",
    b"\x88\xAE": "発",
    b"\x88\xAF": "見",
    b"\x88\xB0": "し",
    b"\x88\xB1": "ま",
    b"\x88\xB2": "た",
    b"\x88\xB3": "!",
    b"\x88\xB4": "リ",
    b"\x88\xB5": "ト",
    b"\x88\xB6": "ラ",
    b"\x88\xB7": "イ",
    b"\x88\xB8": "け",
    b"\x88\xB9": "っ",
    b"\x88\xBA": "て",
    b"\x88\xBB": "い",
    b"\x88\xBC": "も",
    b"\x88\xBD": "ど",
    b"\x88\xBE": "る",
    b"\x88\xBF": "へ",
    b"\x88\xC0": "ん",
    b"\x88\xC1": "こ",
    b"\x88\xC2": "う",
    b"\x88\xC3": "ポ",
    b"\x88\xC4": "ー",
    b"\x88\xC5": "ズ",
    b"\x88\xC6": "メ",
    b"\x88\xC7": "ニ",
    b"\x88\xC8": "ュ",
    b"\x88\xC9": "オ",
    b"\x88\xCA": "プ",
    b"\x88\xCB": "シ",
    b"\x88\xCC": "ョ",
    b"\x88\xCD": "ン",
    b"\x88\xCE": "設",
    b"\x88\xCF": "定",
    b"\x88\xD0": "そ",
    b"\x88\xD1": "さ",
    b"\x88\xD2": "説",
    b"\x88\xD3": "明",
    b"\x88\xD4": "ゲ",
    b"\x88\xD5": "に",
    b"\x88\xD6": "本",
    b"\x88\xD7": "当",
    b"\x88\xD8": "す",
    b"\x88\xD9": "か",
    b"\x88\xDA": "?",
    b"\x88\xDB": "は",
    b"\x88\xDC": "え",
    b"\x88\xDD": "せ",
    b"\x88\xDE": "サ",
    b"\x88\xDF": "ウ",
    b"\x88\xE0": "ド",
    b"\x88\xE1": "バ",
    b"\x88\xE2": "ブ",
    b"\x88\xE3": "レ",
    b"\x88\xE4": "B",
    b"\x88\xE5": "G",
    b"\x88\xE6": "S",
    b"\x88\xE7": "E",
    b"\x88\xE8": "モ",
    b"\x88\xE9": "ノ",
    b"\x88\xEA": "ル",
    b"\x88\xEB": "テ",
    b"\x88\xEC": "ビ",
    b"\x88\xED": "ッ",
    b"\x88\xEE": "の",
    b"\x88\xEF": "出",
    b"\x88\xF0": "力",
    b"\x88\xF1": "形",
    b"\x88\xF2": "式",
    b"\x88\xF3": "ゅ",
    b"\x88\xF4": "つ",
    b"\x88\xF5": "り",
    b"\x88\xF6": "ょ",
    b"\x88\xF7": "く",
    b"\x88\xF8": "き",
    b"\x88\xF9": "選",
    b"\x88\xFA": "択",
    b"\x88\xFB": "だ",
    b"\x88\xFC": "。",
    b"\x89\x40": "フ",
    b"\x89\x41": "⋅",
    b"\x89\x42": "I",
    b"\x89\x43": "N",
    b"\x89\x44": "X",
    b"\x89\x45": "音",
    b"\x89\x46": "量",
    b"\x89\x47": "お",
}

BYTES_TO_CHAR_22_PARK01_B = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ヨ",
    b"\x88\xA0": "ッ",
    b"\x88\xA1": "ト",
    b"\x88\xA2": "ハ",
    b"\x88\xA3": "ー",
    b"\x88\xA4": "バ",
    b"\x88\xA5": " ",
    b"\x88\xA6": "A",
    b"\x88\xA7": "M",
    b"\x88\xA8": "1",
    b"\x88\xA9": ":",
    b"\x88\xAA": "ク",
    b"\x88\xAB": "ロ",
    b"\x88\xAC": "を",
    b"\x88\xAD": "発",
    b"\x88\xAE": "見",
    b"\x88\xAF": "し",
    b"\x88\xB0": "ま",
    b"\x88\xB1": "た",
    b"\x88\xB2": "!",
    b"\x88\xB3": "リ",
    b"\x88\xB4": "ラ",
    b"\x88\xB5": "イ",
    b"\x88\xB6": "タ",
    b"\x88\xB7": "ア",
    b"\x88\xB8": "け",
    b"\x88\xB9": "っ",
    b"\x88\xBA": "て",
    b"\x88\xBB": "い",
    b"\x88\xBC": "も",
    b"\x88\xBD": "ど",
    b"\x88\xBE": "る",
    b"\x88\xBF": "へ",
    b"\x88\xC0": "ん",
    b"\x88\xC1": "こ",
    b"\x88\xC2": "う",
    b"\x88\xC3": "ポ",
    b"\x88\xC4": "ズ",
    b"\x88\xC5": "メ",
    b"\x88\xC6": "ニ",
    b"\x88\xC7": "ュ",
    b"\x88\xC8": "オ",
    b"\x88\xC9": "プ",
    b"\x88\xCA": "シ",
    b"\x88\xCB": "ョ",
    b"\x88\xCC": "ン",
    b"\x88\xCD": "設",
    b"\x88\xCE": "定",
    b"\x88\xCF": "そ",
    b"\x88\xD0": "さ",
    b"\x88\xD1": "説",
    b"\x88\xD2": "明",
    b"\x88\xD3": "ゲ",
    b"\x88\xD4": "ム",
    b"\x88\xD5": "に",
    b"\x88\xD6": "本",
    b"\x88\xD7": "当",
    b"\x88\xD8": "す",
    b"\x88\xD9": "か",
    b"\x88\xDA": "?",
    b"\x88\xDB": "は",
    b"\x88\xDC": "え",
    b"\x88\xDD": "せ",
    b"\x88\xDE": "サ",
    b"\x88\xDF": "ウ",
    b"\x88\xE0": "ド",
    b"\x88\xE1": "ブ",
    b"\x88\xE2": "レ",
    b"\x88\xE3": "B",
    b"\x88\xE4": "G",
    b"\x88\xE5": "S",
    b"\x88\xE6": "E",
    b"\x88\xE7": "モ",
    b"\x88\xE8": "ノ",
    b"\x88\xE9": "ル",
    b"\x88\xEA": "ス",
    b"\x88\xEB": "テ",
    b"\x88\xEC": "ビ",
    b"\x88\xED": "ジ",
    b"\x88\xEE": "2",
    b"\x88\xEF": "の",
    b"\x88\xF0": "出",
    b"\x88\xF1": "力",
    b"\x88\xF2": "形",
    b"\x88\xF3": "式",
    b"\x88\xF4": "ゅ",
    b"\x88\xF5": "つ",
    b"\x88\xF6": "り",
    b"\x88\xF7": "ょ",
    b"\x88\xF8": "く",
    b"\x88\xF9": "き",
    b"\x88\xFA": "選",
    b"\x88\xFB": "択",
    b"\x88\xFC": "だ",
    b"\x89\x40": "。",
    b"\x89\x41": "フ",
    b"\x89\x42": "⋅",
    b"\x89\x43": "I",
    b"\x89\x44": "N",
    b"\x89\x45": "X",
    b"\x89\x46": "音",
    b"\x89\x47": "量",
    b"\x89\x48": "お",
    #b"\x89\x49": "お",
}

BYTES_TO_CHAR_23_BAY02_B = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "コ",
    b"\x88\xA0": "ン",
    b"\x88\xA1": "ビ",
    b"\x88\xA2": "ナ",
    b"\x88\xA3": "ー",
    b"\x88\xA4": "ト",
    b"\x88\xA5": " ",
    b"\x88\xA6": "P",
    b"\x88\xA7": "M",
    b"\x88\xA8": "0",
    b"\x88\xA9": "2",
    b"\x88\xAA": ":",
    b"\x88\xAB": "3",
    b"\x88\xAC": "6",
    b"\x88\xAD": "ク",
    b"\x88\xAE": "ロ",
    b"\x88\xAF": "を",
    b"\x88\xB0": "発",
    b"\x88\xB1": "見",
    b"\x88\xB2": "し",
    b"\x88\xB3": "ま",
    b"\x88\xB4": "た",
    b"\x88\xB5": "!",
    b"\x88\xB6": "エ",
    b"\x88\xB7": "ネ",
    b"\x88\xB8": "ル",
    b"\x88\xB9": "ギ",
    b"\x88\xBA": "供",
    b"\x88\xBB": "給",
    b"\x88\xBC": "炉",
    b"\x88\xBD": "l",
    b"\x88\xBE": "号",
    b"\x88\xBF": "機",
    b"\x88\xC0": "起",
    b"\x88\xC1": "動",
    b"\x88\xC2": "デ",
    b"\x88\xC3": "タ",
    b"\x88\xC4": "A",
    b"\x88\xC5": "抽",
    b"\x88\xC6": "出",
    b"\x88\xC7": "完",
    b"\x88\xC8": "了",
    b"\x88\xC9": "B",
    b"\x88\xCA": "リ",
    b"\x88\xCB": "ラ",
    b"\x88\xCC": "イ",
    b"\x88\xCD": "ア",
    b"\x88\xCE": "け",
    b"\x88\xCF": "っ",
    b"\x88\xD0": "て",
    b"\x88\xD1": "い",
    b"\x88\xD2": "も",
    b"\x88\xD3": "ど",
    b"\x88\xD4": "る",
    b"\x88\xD5": "へ",
    b"\x88\xD6": "ん",
    b"\x88\xD7": "こ",
    b"\x88\xD8": "う",
    b"\x88\xD9": "ポ",
    b"\x88\xDA": "ズ",
    b"\x88\xDB": "メ",
    b"\x88\xDC": "ニ",
    b"\x88\xDD": "ュ",
    b"\x88\xDE": "オ",
    b"\x88\xDF": "プ",
    b"\x88\xE0": "シ",
    b"\x88\xE1": "ョ",
    b"\x88\xE2": "設",
    b"\x88\xE3": "定",
    b"\x88\xE4": "そ",
    b"\x88\xE5": "さ",
    b"\x88\xE6": "説",
    b"\x88\xE7": "明",
    b"\x88\xE8": "ゲ",
    b"\x88\xE9": "ム",
    b"\x88\xEA": "に",
    b"\x88\xEB": "本",
    b"\x88\xEC": "当",
    b"\x88\xED": "す",
    b"\x88\xEE": "か",
    b"\x88\xEF": "?",
    b"\x88\xF0": "は",
    b"\x88\xF1": "え",
    b"\x88\xF2": "せ",
    b"\x88\xF3": "サ",
    b"\x88\xF4": "ウ",
    b"\x88\xF5": "ド",
    b"\x88\xF6": "バ",
    b"\x88\xF7": "ブ",
    b"\x88\xF8": "レ",
    b"\x88\xF9": "G",
    b"\x88\xFA": "S",
    b"\x88\xFB": "E",
    b"\x88\xFC": "モ",
    b"\x89\x40": "ノ",
    b"\x89\x41": "ス",
    b"\x89\x42": "テ",
    b"\x89\x43": "ジ",
    b"\x89\x44": "ッ",
    b"\x89\x45": "の",
    b"\x89\x46": "力",
    b"\x89\x47": "形",
    b"\x89\x48": "式",
    b"\x89\x49": "ゅ",
    b"\x89\x4A": "つ",
    b"\x89\x4B": "り",
    b"\x89\x4C": "ょ",
    b"\x89\x4D": "く",
    b"\x89\x4E": "き",
    b"\x89\x4F": "選",
    b"\x89\x50": "択",
    b"\x89\x51": "だ",
    b"\x89\x52": "。",
    b"\x89\x53": "フ",
    b"\x89\x54": "⋅",
    b"\x89\x55": "I",
    b"\x89\x56": "N",
    b"\x89\x57": "X",
    b"\x89\x58": "音",
    b"\x89\x59": "量",
    b"\x89\x5A": "お",
    #b"\x89\x5B": "お",
    b"\x89\x67": "??",
    b"\x89\x6A": "??",
    b"\x89\x6B": "??",
    b"\x89\x6C": "??",
    b"\x89\x6D": "??",
    b"\x89\x6E": "??",
    b"\x89\x6F": "??",
    b"\x89\x70": "??",
    b"\x89\x71": "??",
    b"\x89\x72": "??",
    b"\x89\x73": "??",
    b"\x89\x74": "??",
    b"\x89\x75": "??",
    b"\x89\x76": "??",
    b"\x89\x77": "??",
}

BYTES_TO_CHAR_24_BAY01_B = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "コ",
    b"\x88\xA0": "ン",
    b"\x88\xA1": "テ",
    b"\x88\xA2": "ナ",
    b"\x88\xA3": "タ",
    b"\x88\xA4": "ー",
    b"\x88\xA5": "ミ",
    b"\x88\xA6": "ル",
    b"\x88\xA7": " ",
    b"\x88\xA8": "P",
    b"\x88\xA9": "M",
    b"\x88\xAA": "0",
    b"\x88\xAB": "5",
    b"\x88\xAC": ":",
    b"\x88\xAD": "1",
    b"\x88\xAE": "6",
    b"\x88\xAF": "リ",
    b"\x88\xB0": "ッ",
    b"\x88\xB1": "キ",
    b"\x88\xB2": "を",
    b"\x88\xB3": "発",
    b"\x88\xB4": "見",
    b"\x88\xB5": "し",
    b"\x88\xB6": "ま",
    b"\x88\xB7": "た",
    b"\x88\xB8": "!",
    b"\x88\xB9": "ト",
    b"\x88\xBA": "ラ",
    b"\x88\xBB": "イ",
    b"\x88\xBC": "ア",
    b"\x88\xBD": "け",
    b"\x88\xBE": "っ",
    b"\x88\xBF": "て",
    b"\x88\xC0": "い",
    b"\x88\xC1": "も",
    b"\x88\xC2": "ど",
    b"\x88\xC3": "る",
    b"\x88\xC4": "へ",
    b"\x88\xC5": "ん",
    b"\x88\xC6": "こ",
    b"\x88\xC7": "う",
    b"\x88\xC8": "ポ",
    b"\x88\xC9": "ズ",
    b"\x88\xCA": "メ",
    b"\x88\xCB": "ニ",
    b"\x88\xCC": "ュ",
    b"\x88\xCD": "オ",
    b"\x88\xCE": "プ",
    b"\x88\xCF": "シ",
    b"\x88\xD0": "ョ",
    b"\x88\xD1": "設",
    b"\x88\xD2": "定",
    b"\x88\xD3": "そ",
    b"\x88\xD4": "さ",
    b"\x88\xD5": "説",
    b"\x88\xD6": "明",
    b"\x88\xD7": "ゲ",
    b"\x88\xD8": "ム",
    b"\x88\xD9": "に",
    b"\x88\xDA": "本",
    b"\x88\xDB": "当",
    b"\x88\xDC": "す",
    b"\x88\xDD": "か",
    b"\x88\xDE": "?",
    b"\x88\xDF": "は",
    b"\x88\xE0": "え",
    b"\x88\xE1": "せ",
    b"\x88\xE2": "サ",
    b"\x88\xE3": "ウ",
    b"\x88\xE4": "ド",
    b"\x88\xE5": "バ",
    b"\x88\xE6": "ブ",
    b"\x88\xE7": "レ",
    b"\x88\xE8": "B",
    b"\x88\xE9": "G",
    b"\x88\xEA": "S",
    b"\x88\xEB": "E",
    b"\x88\xEC": "モ",
    b"\x88\xED": "ノ",
    b"\x88\xEE": "ス",
    b"\x88\xEF": "ビ",
    b"\x88\xF0": "ロ",
    b"\x88\xF1": "ジ",
    b"\x88\xF2": "ク",
    b"\x88\xF3": "2",
    b"\x88\xF4": "の",
    b"\x88\xF5": "出",
    b"\x88\xF6": "力",
    b"\x88\xF7": "形",
    b"\x88\xF8": "式",
    b"\x88\xF9": "ゅ",
    b"\x88\xFA": "つ",
    b"\x88\xFB": "り",
    b"\x88\xFC": "ょ",
    b"\x89\x40": "く",
    b"\x89\x41": "き",
    b"\x89\x42": "選",
    b"\x89\x43": "択",
    b"\x89\x44": "だ",
    b"\x89\x45": "。",
    b"\x89\x46": "フ",
    b"\x89\x47": "⋅",
    b"\x89\x48": "I",
    b"\x89\x49": "N",
    b"\x89\x4A": "A",
    b"\x89\x4B": "X",
    b"\x89\x4C": "音",
    b"\x89\x4D": "量",
    b"\x89\x4E": "お",
    #b"\x89\x4F": "お",
}

BYTES_TO_CHAR_25_BOSS08_BOSS = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "東",
    b"\x88\xA0": "京",
    b"\x88\xA1": " ",
    b"\x88\xA2": "P",
    b"\x88\xA3": "M",
    b"\x88\xA4": "1",
    b"\x88\xA5": "0",
    b"\x88\xA6": ":",
    b"\x88\xA7": "5",
    b"\x88\xA8": "リ",
    b"\x88\xA9": "ト",
    b"\x88\xAA": "ラ",
    b"\x88\xAB": "イ",
    b"\x88\xAC": "タ",
    b"\x88\xAD": "ア",
    b"\x88\xAE": "け",
    b"\x88\xAF": "っ",
    b"\x88\xB0": "て",
    b"\x88\xB1": "い",
    b"\x88\xB2": "も",
    b"\x88\xB3": "ど",
    b"\x88\xB4": "る",
    b"\x88\xB5": "へ",
    b"\x88\xB6": "ん",
    b"\x88\xB7": "こ",
    b"\x88\xB8": "う",
    b"\x88\xB9": "ポ",
    b"\x88\xBA": "ー",
    b"\x88\xBB": "ズ",
    b"\x88\xBC": "メ",
    b"\x88\xBD": "ニ",
    b"\x88\xBE": "ュ",
    b"\x88\xBF": "オ",
    b"\x88\xC0": "プ",
    b"\x88\xC1": "シ",
    b"\x88\xC2": "ョ",
    b"\x88\xC3": "ン",
    b"\x88\xC4": "設",
    b"\x88\xC5": "定",
    b"\x88\xC6": "そ",
    b"\x88\xC7": "さ",
    b"\x88\xC8": "説",
    b"\x88\xC9": "明",
    b"\x88\xCA": "ゲ",
    b"\x88\xCB": "ム",
    b"\x88\xCC": "に",
    b"\x88\xCD": "本",
    b"\x88\xCE": "当",
    b"\x88\xCF": "し",
    b"\x88\xD0": "ま",
    b"\x88\xD1": "す",
    b"\x88\xD2": "か",
    b"\x88\xD3": "?",
    b"\x88\xD4": "は",
    b"\x88\xD5": "え",
    b"\x88\xD6": "せ",
    b"\x88\xD7": "サ",
    b"\x88\xD8": "ウ",
    b"\x88\xD9": "ド",
    b"\x88\xDA": "バ",
    b"\x88\xDB": "ブ",
    b"\x88\xDC": "レ",
    b"\x88\xDD": "B",
    b"\x88\xDE": "G",
    b"\x88\xDF": "S",
    b"\x88\xE0": "E",
    b"\x88\xE1": "モ",
    b"\x88\xE2": "ノ",
    b"\x88\xE3": "ル",
    b"\x88\xE4": "ス",
    b"\x88\xE5": "テ",
    b"\x88\xE6": "ビ",
    b"\x88\xE7": "ロ",
    b"\x88\xE8": "ジ",
    b"\x88\xE9": "ッ",
    b"\x88\xEA": "ク",
    b"\x88\xEB": "2",
    b"\x88\xEC": "の",
    b"\x88\xED": "出",
    b"\x88\xEE": "力",
    b"\x88\xEF": "形",
    b"\x88\xF0": "式",
    b"\x88\xF1": "ゅ",
    b"\x88\xF2": "つ",
    b"\x88\xF3": "り",
    b"\x88\xF4": "ょ",
    b"\x88\xF5": "く",
    b"\x88\xF6": "き",
    b"\x88\xF7": "を",
    b"\x88\xF8": "選",
    b"\x88\xF9": "択",
    b"\x88\xFA": "た",
    b"\x88\xFB": "だ",
    b"\x88\xFC": "。",
    b"\x89\x40": "フ",
    b"\x89\x41": "⋅",
    b"\x89\x42": "I",
    b"\x89\x43": "N",
    b"\x89\x44": "A",
    b"\x89\x45": "X",
    b"\x89\x46": "音",
    b"\x89\x47": "量",
    b"\x89\x48": "お",
    #b"\x89\x49": "お",
}

BYTES_TO_CHAR_26_METRO01_B = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "メ",
    b"\x88\xA0": "ト",
    b"\x88\xA1": "ロ",
    b"\x88\xA2": " ",
    b"\x88\xA3": "A",
    b"\x88\xA4": "M",
    b"\x88\xA5": "0",
    b"\x88\xA6": "8",
    b"\x88\xA7": ":",
    b"\x88\xA8": "1",
    b"\x88\xA9": "3",
    b"\x88\xAA": "ジ",
    b"\x88\xAB": "ュ",
    b"\x88\xAC": "ン",
    b"\x88\xAD": "を",
    b"\x88\xAE": "発",
    b"\x88\xAF": "見",
    b"\x88\xB0": "し",
    b"\x88\xB1": "ま",
    b"\x88\xB2": "た",
    b"\x88\xB3": "!",
    b"\x88\xB4": "リ",
    b"\x88\xB5": "ラ",
    b"\x88\xB6": "イ",
    b"\x88\xB7": "タ",
    b"\x88\xB8": "ア",
    b"\x88\xB9": "け",
    b"\x88\xBA": "っ",
    b"\x88\xBB": "て",
    b"\x88\xBC": "い",
    b"\x88\xBD": "も",
    b"\x88\xBE": "ど",
    b"\x88\xBF": "る",
    b"\x88\xC0": "へ",
    b"\x88\xC1": "ん",
    b"\x88\xC2": "こ",
    b"\x88\xC3": "う",
    b"\x88\xC4": "ポ",
    b"\x88\xC5": "ー",
    b"\x88\xC6": "ズ",
    b"\x88\xC7": "ニ",
    b"\x88\xC8": "オ",
    b"\x88\xC9": "プ",
    b"\x88\xCA": "シ",
    b"\x88\xCB": "ョ",
    b"\x88\xCC": "設",
    b"\x88\xCD": "定",
    b"\x88\xCE": "そ",
    b"\x88\xCF": "さ",
    b"\x88\xD0": "説",
    b"\x88\xD1": "明",
    b"\x88\xD2": "ゲ",
    b"\x88\xD3": "ム",
    b"\x88\xD4": "に",
    b"\x88\xD5": "本",
    b"\x88\xD6": "当",
    b"\x88\xD7": "す",
    b"\x88\xD8": "か",
    b"\x88\xD9": "?",
    b"\x88\xDA": "は",
    b"\x88\xDB": "え",
    b"\x88\xDC": "せ",
    b"\x88\xDD": "サ",
    b"\x88\xDE": "ウ",
    b"\x88\xDF": "ド",
    b"\x88\xE0": "バ",
    b"\x88\xE1": "ブ",
    b"\x88\xE2": "レ",
    b"\x88\xE3": "B",
    b"\x88\xE4": "G",
    b"\x88\xE5": "S",
    b"\x88\xE6": "E",
    b"\x88\xE7": "モ",
    b"\x88\xE8": "ノ",
    b"\x88\xE9": "ル",
    b"\x88\xEA": "ス",
    b"\x88\xEB": "テ",
    b"\x88\xEC": "ビ",
    b"\x88\xED": "ッ",
    b"\x88\xEE": "ク",
    b"\x88\xEF": "2",
    b"\x88\xF0": "の",
    b"\x88\xF1": "出",
    b"\x88\xF2": "力",
    b"\x88\xF3": "形",
    b"\x88\xF4": "式",
    b"\x88\xF5": "ゅ",
    b"\x88\xF6": "つ",
    b"\x88\xF7": "り",
    b"\x88\xF8": "ょ",
    b"\x88\xF9": "く",
    b"\x88\xFA": "き",
    b"\x88\xFB": "選",
    b"\x88\xFC": "択",
    b"\x89\x40": "だ",
    b"\x89\x41": "。",
    b"\x89\x42": "フ",
    b"\x89\x43": "⋅",
    b"\x89\x44": "I",
    b"\x89\x45": "N",
    b"\x89\x46": "X",
    b"\x89\x47": "音",
    b"\x89\x48": "量",
    b"\x89\x49": "お",
}

BYTES_TO_CHAR_27_CITY02_B = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "秋",
    b"\x88\xA0": "葉",
    b"\x88\xA1": "原",
    b"\x88\xA2": " ",
    b"\x88\xA3": "A",
    b"\x88\xA4": "M",
    b"\x88\xA5": "1",
    b"\x88\xA6": "0",
    b"\x88\xA7": ":",
    b"\x88\xA8": "3",
    b"\x88\xA9": "2",
    b"\x88\xAA": "エ",
    b"\x88\xAB": "ン",
    b"\x88\xAC": "ジ",
    b"\x88\xAD": "爆",
    b"\x88\xAE": "発",
    b"\x88\xAF": "!",
    b"\x88\xB0": "残",
    b"\x88\xB1": "り",
    b"\x88\xB2": "基",
    b"\x88\xB3": "で",
    b"\x88\xB4": "す",
    b"\x88\xB5": "リ",
    b"\x88\xB6": "ト",
    b"\x88\xB7": "ラ",
    b"\x88\xB8": "イ",
    b"\x88\xB9": "タ",
    b"\x88\xBA": "ア",
    b"\x88\xBB": "け",
    b"\x88\xBC": "っ",
    b"\x88\xBD": "て",
    b"\x88\xBE": "い",
    b"\x88\xBF": "も",
    b"\x88\xC0": "ど",
    b"\x88\xC1": "る",
    b"\x88\xC2": "へ",
    b"\x88\xC3": "ん",
    b"\x88\xC4": "こ",
    b"\x88\xC5": "う",
    b"\x88\xC6": "ポ",
    b"\x88\xC7": "ー",
    b"\x88\xC8": "ズ",
    b"\x88\xC9": "メ",
    b"\x88\xCA": "ニ",
    b"\x88\xCB": "ュ",
    b"\x88\xCC": "オ",
    b"\x88\xCD": "プ",
    b"\x88\xCE": "シ",
    b"\x88\xCF": "ョ",
    b"\x88\xD0": "設",
    b"\x88\xD1": "定",
    b"\x88\xD2": "そ",
    b"\x88\xD3": "さ",
    b"\x88\xD4": "説",
    b"\x88\xD5": "明",
    b"\x88\xD6": "ゲ",
    b"\x88\xD7": "ム",
    b"\x88\xD8": "に",
    b"\x88\xD9": "本",
    b"\x88\xDA": "当",
    b"\x88\xDB": "し",
    b"\x88\xDC": "ま",
    b"\x88\xDD": "か",
    b"\x88\xDE": "?",
    b"\x88\xDF": "は",
    b"\x88\xE0": "え",
    b"\x88\xE1": "せ",
    b"\x88\xE2": "サ",
    b"\x88\xE3": "ウ",
    b"\x88\xE4": "ド",
    b"\x88\xE5": "バ",
    b"\x88\xE6": "ブ",
    b"\x88\xE7": "レ",
    b"\x88\xE8": "B",
    b"\x88\xE9": "G",
    b"\x88\xEA": "S",
    b"\x88\xEB": "E",
    b"\x88\xEC": "モ",
    b"\x88\xED": "ノ",
    b"\x88\xEE": "ル",
    b"\x88\xEF": "ス",
    b"\x88\xF0": "テ",
    b"\x88\xF1": "ビ",
    b"\x88\xF2": "ロ",
    b"\x88\xF3": "ッ",
    b"\x88\xF4": "ク",
    b"\x88\xF5": "の",
    b"\x88\xF6": "出",
    b"\x88\xF7": "力",
    b"\x88\xF8": "形",
    b"\x88\xF9": "式",
    b"\x88\xFA": "ゅ",
    b"\x88\xFB": "つ",
    b"\x88\xFC": "ょ",
    b"\x89\x40": "く",
    b"\x89\x41": "き",
    b"\x89\x42": "を",
    b"\x89\x43": "選",
    b"\x89\x44": "択",
    b"\x89\x45": "た",
    b"\x89\x46": "だ",
    b"\x89\x47": "。",
    b"\x89\x48": "フ",
    b"\x89\x49": "⋅",
    b"\x89\x4A": "I",
    b"\x89\x4B": "N",
    b"\x89\x4C": "X",
    b"\x89\x4D": "音",
    b"\x89\x4E": "量",
    b"\x89\x4F": "お",
}

BYTES_TO_CHAR_28_CITY01_B = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "新",
    b"\x88\xA0": "宿",
    b"\x88\xA1": " ",
    b"\x88\xA2": "P",
    b"\x88\xA3": "M",
    b"\x88\xA4": "0",
    b"\x88\xA5": ":",
    b"\x88\xA6": "2",
    b"\x88\xA7": "6",
    b"\x88\xA8": "ク",
    b"\x88\xA9": "ロ",
    b"\x88\xAA": "を",
    b"\x88\xAB": "発",
    b"\x88\xAC": "見",
    b"\x88\xAD": "し",
    b"\x88\xAE": "ま",
    b"\x88\xAF": "た",
    b"\x88\xB0": "!",
    b"\x88\xB1": "リ",
    b"\x88\xB2": "ト",
    b"\x88\xB3": "ラ",
    b"\x88\xB4": "イ",
    b"\x88\xB5": "タ",
    b"\x88\xB6": "ア",
    b"\x88\xB7": "け",
    b"\x88\xB8": "っ",
    b"\x88\xB9": "て",
    b"\x88\xBA": "い",
    b"\x88\xBB": "も",
    b"\x88\xBC": "ど",
    b"\x88\xBD": "る",
    b"\x88\xBE": "へ",
    b"\x88\xBF": "ん",
    b"\x88\xC0": "こ",
    b"\x88\xC1": "う",
    b"\x88\xC2": "ポ",
    b"\x88\xC3": "ー",
    b"\x88\xC4": "ズ",
    b"\x88\xC5": "メ",
    b"\x88\xC6": "ニ",
    b"\x88\xC7": "ュ",
    b"\x88\xC8": "オ",
    b"\x88\xC9": "プ",
    b"\x88\xCA": "シ",
    b"\x88\xCB": "ョ",
    b"\x88\xCC": "ン",
    b"\x88\xCD": "設",
    b"\x88\xCE": "定",
    b"\x88\xCF": "そ",
    b"\x88\xD0": "さ",
    b"\x88\xD1": "説",
    b"\x88\xD2": "明",
    b"\x88\xD3": "ゲ",
    b"\x88\xD4": "ム",
    b"\x88\xD5": "に",
    b"\x88\xD6": "本",
    b"\x88\xD7": "当",
    b"\x88\xD8": "す",
    b"\x88\xD9": "か",
    b"\x88\xDA": "?",
    b"\x88\xDB": "は",
    b"\x88\xDC": "え",
    b"\x88\xDD": "せ",
    b"\x88\xDE": "サ",
    b"\x88\xDF": "ウ",
    b"\x88\xE0": "ド",
    b"\x88\xE1": "バ",
    b"\x88\xE2": "ブ",
    b"\x88\xE3": "レ",
    b"\x88\xE4": "B",
    b"\x88\xE5": "G",
    b"\x88\xE6": "S",
    b"\x88\xE7": "E",
    b"\x88\xE8": "モ",
    b"\x88\xE9": "ノ",
    b"\x88\xEA": "ル",
    b"\x88\xEB": "ス",
    b"\x88\xEC": "テ",
    b"\x88\xED": "ビ",
    b"\x88\xEE": "ジ",
    b"\x88\xEF": "ッ",
    b"\x88\xF0": "の",
    b"\x88\xF1": "出",
    b"\x88\xF2": "力",
    b"\x88\xF3": "形",
    b"\x88\xF4": "式",
    b"\x88\xF5": "ゅ",
    b"\x88\xF6": "つ",
    b"\x88\xF7": "り",
    b"\x88\xF8": "ょ",
    b"\x88\xF9": "く",
    b"\x88\xFA": "き",
    b"\x88\xFB": "選",
    b"\x88\xFC": "択",
    b"\x89\x40": "だ",
    b"\x89\x41": "。",
    b"\x89\x42": "フ",
    b"\x89\x43": "⋅",
    b"\x89\x44": "I",
    b"\x89\x45": "N",
    b"\x89\x46": "A",
    b"\x89\x47": "X",
    b"\x89\x48": "音",
    b"\x89\x49": "量",
    b"\x89\x4A": "お",
    #b"\x89\x4B": "お",
}

BYTES_TO_CHAR_29_BOSS09_BOSS = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "東",
    b"\x88\xA0": "京",
    b"\x88\xA1": "テ",
    b"\x88\xA2": "レ",
    b"\x88\xA3": "ポ",
    b"\x88\xA4": "ー",
    b"\x88\xA5": "ト",
    b"\x88\xA6": " ",
    b"\x88\xA7": "P",
    b"\x88\xA8": "M",
    b"\x88\xA9": "0",
    b"\x88\xAA": "2",
    b"\x88\xAB": ":",
    b"\x88\xAC": "リ",
    b"\x88\xAD": "ラ",
    b"\x88\xAE": "イ",
    b"\x88\xAF": "タ",
    b"\x88\xB0": "ア",
    b"\x88\xB1": "け",
    b"\x88\xB2": "っ",
    b"\x88\xB3": "て",
    b"\x88\xB4": "い",
    b"\x88\xB5": "も",
    b"\x88\xB6": "ど",
    b"\x88\xB7": "る",
    b"\x88\xB8": "へ",
    b"\x88\xB9": "ん",
    b"\x88\xBA": "こ",
    b"\x88\xBB": "う",
    b"\x88\xBC": "ズ",
    b"\x88\xBD": "メ",
    b"\x88\xBE": "ニ",
    b"\x88\xBF": "ュ",
    b"\x88\xC0": "オ",
    b"\x88\xC1": "プ",
    b"\x88\xC2": "シ",
    b"\x88\xC3": "ョ",
    b"\x88\xC4": "ン",
    b"\x88\xC5": "設",
    b"\x88\xC6": "定",
    b"\x88\xC7": "そ",
    b"\x88\xC8": "さ",
    b"\x88\xC9": "説",
    b"\x88\xCA": "明",
    b"\x88\xCB": "ゲ",
    b"\x88\xCC": "ム",
    b"\x88\xCD": "に",
    b"\x88\xCE": "本",
    b"\x88\xCF": "当",
    b"\x88\xD0": "し",
    b"\x88\xD1": "ま",
    b"\x88\xD2": "す",
    b"\x88\xD3": "か",
    b"\x88\xD4": "?",
    b"\x88\xD5": "は",
    b"\x88\xD6": "え",
    b"\x88\xD7": "せ",
    b"\x88\xD8": "サ",
    b"\x88\xD9": "ウ",
    b"\x88\xDA": "ド",
    b"\x88\xDB": "バ",
    b"\x88\xDC": "ブ",
    b"\x88\xDD": "B",
    b"\x88\xDE": "G",
    b"\x88\xDF": "S",
    b"\x88\xE0": "E",
    b"\x88\xE1": "モ",
    b"\x88\xE2": "ノ",
    b"\x88\xE3": "ル",
    b"\x88\xE4": "ス",
    b"\x88\xE5": "ビ",
    b"\x88\xE6": "ロ",
    b"\x88\xE7": "ジ",
    b"\x88\xE8": "ッ",
    b"\x88\xE9": "ク",
    b"\x88\xEA": "の",
    b"\x88\xEB": "出",
    b"\x88\xEC": "力",
    b"\x88\xED": "形",
    b"\x88\xEE": "式",
    b"\x88\xEF": "ゅ",
    b"\x88\xF0": "つ",
    b"\x88\xF1": "り",
    b"\x88\xF2": "ょ",
    b"\x88\xF3": "く",
    b"\x88\xF4": "き",
    b"\x88\xF5": "を",
    b"\x88\xF6": "選",
    b"\x88\xF7": "択",
    b"\x88\xF8": "た",
    b"\x88\xF9": "だ",
    b"\x88\xFA": "。",
    b"\x88\xFB": "フ",
    b"\x88\xFC": "⋅",
    b"\x89\x40": "I",
    b"\x89\x41": "N",
    b"\x89\x42": "A",
    b"\x89\x43": "X",
    b"\x89\x44": "音",
    b"\x89\x45": "量",
    b"\x89\x46": "お",
    #b"\x89\x47": "お",
}

BYTES_TO_CHAR_30_DAIBA02_B = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "お",
    b"\x88\xA0": "台",
    b"\x88\xA1": "場",
    b"\x88\xA2": " ",
    b"\x88\xA3": "P",
    b"\x88\xA4": "M",
    b"\x88\xA5": "0",
    b"\x88\xA6": "5",
    b"\x88\xA7": ":",
    b"\x88\xA8": "3",
    b"\x88\xA9": "2",
    b"\x88\xAA": "ト",
    b"\x88\xAB": "ロ",
    b"\x88\xAC": "を",
    b"\x88\xAD": "発",
    b"\x88\xAE": "見",
    b"\x88\xAF": "し",
    b"\x88\xB0": "ま",
    b"\x88\xB1": "た",
    b"\x88\xB2": "!",
    b"\x88\xB3": "リ",
    b"\x88\xB4": "ラ",
    b"\x88\xB5": "イ",
    b"\x88\xB6": "タ",
    b"\x88\xB7": "ア",
    b"\x88\xB8": "け",
    b"\x88\xB9": "っ",
    b"\x88\xBA": "て",
    b"\x88\xBB": "い",
    b"\x88\xBC": "も",
    b"\x88\xBD": "ど",
    b"\x88\xBE": "る",
    b"\x88\xBF": "へ",
    b"\x88\xC0": "ん",
    b"\x88\xC1": "こ",
    b"\x88\xC2": "う",
    b"\x88\xC3": "ポ",
    b"\x88\xC4": "ー",
    b"\x88\xC5": "ズ",
    b"\x88\xC6": "メ",
    b"\x88\xC7": "ニ",
    b"\x88\xC8": "ュ",
    b"\x88\xC9": "オ",
    b"\x88\xCA": "プ",
    b"\x88\xCB": "シ",
    b"\x88\xCC": "ョ",
    b"\x88\xCD": "ン",
    b"\x88\xCE": "設",
    b"\x88\xCF": "定",
    b"\x88\xD0": "そ",
    b"\x88\xD1": "さ",
    b"\x88\xD2": "説",
    b"\x88\xD3": "明",
    b"\x88\xD4": "ゲ",
    b"\x88\xD5": "ム",
    b"\x88\xD6": "に",
    b"\x88\xD7": "本",
    b"\x88\xD8": "当",
    b"\x88\xD9": "す",
    b"\x88\xDA": "か",
    b"\x88\xDB": "?",
    b"\x88\xDC": "は",
    b"\x88\xDD": "え",
    b"\x88\xDE": "せ",
    b"\x88\xDF": "サ",
    b"\x88\xE0": "ウ",
    b"\x88\xE1": "ド",
    b"\x88\xE2": "バ",
    b"\x88\xE3": "ブ",
    b"\x88\xE4": "レ",
    b"\x88\xE5": "B",
    b"\x88\xE6": "G",
    b"\x88\xE7": "S",
    b"\x88\xE8": "E",
    b"\x88\xE9": "モ",
    b"\x88\xEA": "ノ",
    b"\x88\xEB": "ル",
    b"\x88\xEC": "ス",
    b"\x88\xED": "テ",
    b"\x88\xEE": "ビ",
    b"\x88\xEF": "ジ",
    b"\x88\xF0": "ッ",
    b"\x88\xF1": "ク",
    b"\x88\xF2": "の",
    b"\x88\xF3": "出",
    b"\x88\xF4": "力",
    b"\x88\xF5": "形",
    b"\x88\xF6": "式",
    b"\x88\xF7": "ゅ",
    b"\x88\xF8": "つ",
    b"\x88\xF9": "り",
    b"\x88\xFA": "ょ",
    b"\x88\xFB": "く",
    b"\x88\xFC": "き",
    b"\x89\x40": "選",
    b"\x89\x41": "択",
    b"\x89\x42": "だ",
    b"\x89\x43": "。",
    b"\x89\x44": "フ",
    b"\x89\x45": "⋅",
    b"\x89\x46": "I",
    b"\x89\x47": "N",
    b"\x89\x48": "A",
    b"\x89\x49": "X",
    b"\x89\x4A": "音",
    b"\x89\x4B": "量",
}

BYTES_TO_CHAR_31_BOSS10_BOSS = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "墜",
    b"\x88\xA0": "落",
    b"\x88\xA1": "地",
    b"\x88\xA2": "点",
    b"\x88\xA3": " ",
    b"\x88\xA4": "A",
    b"\x88\xA5": "M",
    b"\x88\xA6": "0",
    b"\x88\xA7": "4",
    b"\x88\xA8": ":",
    b"\x88\xA9": "リ",
    b"\x88\xAA": "ト",
    b"\x88\xAB": "ラ",
    b"\x88\xAC": "イ",
    b"\x88\xAD": "タ",
    b"\x88\xAE": "ア",
    b"\x88\xAF": "け",
    b"\x88\xB0": "っ",
    b"\x88\xB1": "て",
    b"\x88\xB2": "い",
    b"\x88\xB3": "も",
    b"\x88\xB4": "ど",
    b"\x88\xB5": "る",
    b"\x88\xB6": "へ",
    b"\x88\xB7": "ん",
    b"\x88\xB8": "こ",
    b"\x88\xB9": "う",
    b"\x88\xBA": "ポ",
    b"\x88\xBB": "ー",
    b"\x88\xBC": "ズ",
    b"\x88\xBD": "メ",
    b"\x88\xBE": "ニ",
    b"\x88\xBF": "ュ",
    b"\x88\xC0": "オ",
    b"\x88\xC1": "プ",
    b"\x88\xC2": "シ",
    b"\x88\xC3": "ョ",
    b"\x88\xC4": "ン",
    b"\x88\xC5": "設",
    b"\x88\xC6": "定",
    b"\x88\xC7": "そ",
    b"\x88\xC8": "さ",
    b"\x88\xC9": "説",
    b"\x88\xCA": "明",
    b"\x88\xCB": "ゲ",
    b"\x88\xCC": "ム",
    b"\x88\xCD": "に",
    b"\x88\xCE": "本",
    b"\x88\xCF": "当",
    b"\x88\xD0": "し",
    b"\x88\xD1": "ま",
    b"\x88\xD2": "す",
    b"\x88\xD3": "か",
    b"\x88\xD4": "?",
    b"\x88\xD5": "は",
    b"\x88\xD6": "え",
    b"\x88\xD7": "せ",
    b"\x88\xD8": "サ",
    b"\x88\xD9": "ウ",
    b"\x88\xDA": "ド",
    b"\x88\xDB": "バ",
    b"\x88\xDC": "ブ",
    b"\x88\xDD": "レ",
    b"\x88\xDE": "B",
    b"\x88\xDF": "G",
    b"\x88\xE0": "S",
    b"\x88\xE1": "E",
    b"\x88\xE2": "モ",
    b"\x88\xE3": "ノ",
    b"\x88\xE4": "ル",
    b"\x88\xE5": "ス",
    b"\x88\xE6": "テ",
    b"\x88\xE7": "ビ",
    b"\x88\xE8": "ロ",
    b"\x88\xE9": "ジ",
    b"\x88\xEA": "ッ",
    b"\x88\xEB": "ク",
    b"\x88\xEC": "2",
    b"\x88\xED": "の",
    b"\x88\xEE": "出",
    b"\x88\xEF": "力",
    b"\x88\xF0": "形",
    b"\x88\xF1": "式",
    b"\x88\xF2": "ゅ",
    b"\x88\xF3": "つ",
    b"\x88\xF4": "り",
    b"\x88\xF5": "ょ",
    b"\x88\xF6": "く",
    b"\x88\xF7": "き",
    b"\x88\xF8": "を",
    b"\x88\xF9": "選",
    b"\x88\xFA": "択",
    b"\x88\xFB": "た",
    b"\x88\xFC": "だ",
    b"\x89\x40": "。",
    b"\x89\x41": "フ",
    b"\x89\x42": "⋅",
    b"\x89\x43": "I",
    b"\x89\x44": "N",
    b"\x89\x45": "X",
    b"\x89\x46": "音",
    b"\x89\x47": "量",
    b"\x89\x48": "お",
    #b"\x89\x49": "お",
}

BYTES_TO_CHAR_50_K1 = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ー",
    b"\x88\xA0": "リ",
    b"\x88\xA1": "ト",
    b"\x88\xA2": "ラ",
    b"\x88\xA3": "イ",
    b"\x88\xA4": "タ",
    b"\x88\xA5": "ア",
    b"\x88\xA6": "け",
    b"\x88\xA7": "っ",
    b"\x88\xA8": "て",
    b"\x88\xA9": "い",
    b"\x88\xAA": "も",
    b"\x88\xAB": "ど",
    b"\x88\xAC": "る",
    b"\x88\xAD": "へ",
    b"\x88\xAE": "ん",
    b"\x88\xAF": "こ",
    b"\x88\xB0": "う",
    b"\x88\xB1": "ポ",
    b"\x88\xB2": "ズ",
    b"\x88\xB3": "メ",
    b"\x88\xB4": "ニ",
    b"\x88\xB5": "ュ",
    b"\x88\xB6": "オ",
    b"\x88\xB7": "プ",
    b"\x88\xB8": "シ",
    b"\x88\xB9": "ョ",
    b"\x88\xBA": "ン",
    b"\x88\xBB": "設",
    b"\x88\xBC": "定",
    b"\x88\xBD": "そ",
    b"\x88\xBE": "さ",
    b"\x88\xBF": "説",
    b"\x88\xC0": "明",
    b"\x88\xC1": "ゲ",
    b"\x88\xC2": "ム",
    b"\x88\xC3": "に",
    b"\x88\xC4": "本",
    b"\x88\xC5": "当",
    b"\x88\xC6": "し",
    b"\x88\xC7": "ま",
    b"\x88\xC8": "す",
    b"\x88\xC9": "か",
    b"\x88\xCA": "?",
    b"\x88\xCB": "は",
    b"\x88\xCC": "え",
    b"\x88\xCD": "せ",
    b"\x88\xCE": "サ",
    b"\x88\xCF": "ウ",
    b"\x88\xD0": "ド",
    b"\x88\xD1": "バ",
    b"\x88\xD2": "ブ",
    b"\x88\xD3": "レ",
    b"\x88\xD4": "B",
    b"\x88\xD5": "G",
    b"\x88\xD6": "M",
    b"\x88\xD7": "S",
    b"\x88\xD8": "E",
    b"\x88\xD9": "モ",
    b"\x88\xDA": "ノ",
    b"\x88\xDB": "ル",
    b"\x88\xDC": "ス",
    b"\x88\xDD": "テ",
    b"\x88\xDE": "ビ",
    b"\x88\xDF": "ロ",
    b"\x88\xE0": "ジ",
    b"\x88\xE1": "ッ",
    b"\x88\xE2": "ク",
    b"\x88\xE3": "2",
    b"\x88\xE4": "の",
    b"\x88\xE5": "出",
    b"\x88\xE6": "力",
    b"\x88\xE7": "形",
    b"\x88\xE8": "式",
    b"\x88\xE9": "ゅ",
    b"\x88\xEA": "つ",
    b"\x88\xEB": "り",
    b"\x88\xEC": "ょ",
    b"\x88\xED": "く",
    b"\x88\xEE": "き",
    b"\x88\xEF": "を",
    b"\x88\xF0": "選",
    b"\x88\xF1": "択",
    b"\x88\xF2": "た",
    b"\x88\xF3": "だ",
    b"\x88\xF4": "。",
    b"\x88\xF5": "フ",
    b"\x88\xF6": "⋅",
    b"\x88\xF7": "I",
    b"\x88\xF8": "N",
    b"\x88\xF9": "A",
    b"\x88\xFA": "X",
    b"\x88\xFB": "音",
    b"\x88\xFC": "量",
    b"\x89\x40": "お",
    #b"\x89\x41": "お",
}

BYTES_TO_CHAR_51_CITY = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ー",
    b"\x88\xA0": "リ",
    b"\x88\xA1": "ト",
    b"\x88\xA2": "ラ",
    b"\x88\xA3": "イ",
    b"\x88\xA4": "タ",
    b"\x88\xA5": "ア",
    b"\x88\xA6": "け",
    b"\x88\xA7": "っ",
    b"\x88\xA8": "て",
    b"\x88\xA9": "い",
    b"\x88\xAA": "も",
    b"\x88\xAB": "ど",
    b"\x88\xAC": "る",
    b"\x88\xAD": "へ",
    b"\x88\xAE": "ん",
    b"\x88\xAF": "こ",
    b"\x88\xB0": "う",
    b"\x88\xB1": "ポ",
    b"\x88\xB2": "ズ",
    b"\x88\xB3": "メ",
    b"\x88\xB4": "ニ",
    b"\x88\xB5": "ュ",
    b"\x88\xB6": "オ",
    b"\x88\xB7": "プ",
    b"\x88\xB8": "シ",
    b"\x88\xB9": "ョ",
    b"\x88\xBA": "ン",
    b"\x88\xBB": "設",
    b"\x88\xBC": "定",
    b"\x88\xBD": "そ",
    b"\x88\xBE": "さ",
    b"\x88\xBF": "説",
    b"\x88\xC0": "明",
    b"\x88\xC1": "ゲ",
    b"\x88\xC2": "ム",
    b"\x88\xC3": "に",
    b"\x88\xC4": "本",
    b"\x88\xC5": "当",
    b"\x88\xC6": "し",
    b"\x88\xC7": "ま",
    b"\x88\xC8": "す",
    b"\x88\xC9": "か",
    b"\x88\xCA": "?",
    b"\x88\xCB": "は",
    b"\x88\xCC": "え",
    b"\x88\xCD": "せ",
    b"\x88\xCE": "サ",
    b"\x88\xCF": "ウ",
    b"\x88\xD0": "ド",
    b"\x88\xD1": "バ",
    b"\x88\xD2": "ブ",
    b"\x88\xD3": "レ",
    b"\x88\xD4": "B",
    b"\x88\xD5": "G",
    b"\x88\xD6": "M",
    b"\x88\xD7": "S",
    b"\x88\xD8": "E",
    b"\x88\xD9": "モ",
    b"\x88\xDA": "ノ",
    b"\x88\xDB": "ル",
    b"\x88\xDC": "ス",
    b"\x88\xDD": "テ",
    b"\x88\xDE": "ビ",
    b"\x88\xDF": "ロ",
    b"\x88\xE0": "ジ",
    b"\x88\xE1": "ッ",
    b"\x88\xE2": "ク",
    b"\x88\xE3": "2",
    b"\x88\xE4": "の",
    b"\x88\xE5": "出",
    b"\x88\xE6": "力",
    b"\x88\xE7": "形",
    b"\x88\xE8": "式",
    b"\x88\xE9": "ゅ",
    b"\x88\xEA": "つ",
    b"\x88\xEB": "り",
    b"\x88\xEC": "ょ",
    b"\x88\xED": "く",
    b"\x88\xEE": "き",
    b"\x88\xEF": "を",
    b"\x88\xF0": "選",
    b"\x88\xF1": "択",
    b"\x88\xF2": "た",
    b"\x88\xF3": "だ",
    b"\x88\xF4": "。",
    b"\x88\xF5": "フ",
    b"\x88\xF6": "⋅",
    b"\x88\xF7": "I",
    b"\x88\xF8": "N",
    b"\x88\xF9": "A",
    b"\x88\xFA": "X",
    b"\x88\xFB": "音",
    b"\x88\xFC": "量",
    b"\x89\x40": "お",
    #b"\x89\x41": "お",
    #b"\x89\x46": " ",
    #b"\x89\x47": " ",
    #b"\x89\x48": " ",
    #b"\x89\x49": " ",
}

BYTES_TO_CHAR_52_METRO = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ー",
    b"\x88\xA0": "リ",
    b"\x88\xA1": "ト",
    b"\x88\xA2": "ラ",
    b"\x88\xA3": "イ",
    b"\x88\xA4": "タ",
    b"\x88\xA5": "ア",
    b"\x88\xA6": "け",
    b"\x88\xA7": "っ",
    b"\x88\xA8": "て",
    b"\x88\xA9": "い",
    b"\x88\xAA": "も",
    b"\x88\xAB": "ど",
    b"\x88\xAC": "る",
    b"\x88\xAD": "へ",
    b"\x88\xAE": "ん",
    b"\x88\xAF": "こ",
    b"\x88\xB0": "う",
    b"\x88\xB1": "ポ",
    b"\x88\xB2": "ズ",
    b"\x88\xB3": "メ",
    b"\x88\xB4": "ニ",
    b"\x88\xB5": "ュ",
    b"\x88\xB6": "オ",
    b"\x88\xB7": "プ",
    b"\x88\xB8": "シ",
    b"\x88\xB9": "ョ",
    b"\x88\xBA": "ン",
    b"\x88\xBB": "設",
    b"\x88\xBC": "定",
    b"\x88\xBD": "そ",
    b"\x88\xBE": "さ",
    b"\x88\xBF": "説",
    b"\x88\xC0": "明",
    b"\x88\xC1": "ゲ",
    b"\x88\xC2": "ム",
    b"\x88\xC3": "に",
    b"\x88\xC4": "本",
    b"\x88\xC5": "当",
    b"\x88\xC6": "し",
    b"\x88\xC7": "ま",
    b"\x88\xC8": "す",
    b"\x88\xC9": "か",
    b"\x88\xCA": "?",
    b"\x88\xCB": "は",
    b"\x88\xCC": "え",
    b"\x88\xCD": "せ",
    b"\x88\xCE": "サ",
    b"\x88\xCF": "ウ",
    b"\x88\xD0": "ド",
    b"\x88\xD1": "バ",
    b"\x88\xD2": "ブ",
    b"\x88\xD3": "レ",
    b"\x88\xD4": "B",
    b"\x88\xD5": "G",
    b"\x88\xD6": "M",
    b"\x88\xD7": "S",
    b"\x88\xD8": "E",
    b"\x88\xD9": "モ",
    b"\x88\xDA": "ノ",
    b"\x88\xDB": "ル",
    b"\x88\xDC": "ス",
    b"\x88\xDD": "テ",
    b"\x88\xDE": "ビ",
    b"\x88\xDF": "ロ",
    b"\x88\xE0": "ジ",
    b"\x88\xE1": "ッ",
    b"\x88\xE2": "ク",
    b"\x88\xE3": "2",
    b"\x88\xE4": "の",
    b"\x88\xE5": "出",
    b"\x88\xE6": "力",
    b"\x88\xE7": "形",
    b"\x88\xE8": "式",
    b"\x88\xE9": "ゅ",
    b"\x88\xEA": "つ",
    b"\x88\xEB": "り",
    b"\x88\xEC": "ょ",
    b"\x88\xED": "く",
    b"\x88\xEE": "き",
    b"\x88\xEF": "を",
    b"\x88\xF0": "選",
    b"\x88\xF1": "択",
    b"\x88\xF2": "た",
    b"\x88\xF3": "だ",
    b"\x88\xF4": "。",
    b"\x88\xF5": "フ",
    b"\x88\xF6": "⋅",
    b"\x88\xF7": "I",
    b"\x88\xF8": "N",
    b"\x88\xF9": "A",
    b"\x88\xFA": "X",
    b"\x88\xFB": "音",
    b"\x88\xFC": "量",
    b"\x89\x40": "お",
    #b"\x89\x41": "お",
}

BYTES_TO_CHAR_53_BAY = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ー",
    b"\x88\xA0": "リ",
    b"\x88\xA1": "ト",
    b"\x88\xA2": "ラ",
    b"\x88\xA3": "イ",
    b"\x88\xA4": "タ",
    b"\x88\xA5": "ア",
    b"\x88\xA6": "け",
    b"\x88\xA7": "っ",
    b"\x88\xA8": "て",
    b"\x88\xA9": "い",
    b"\x88\xAA": "も",
    b"\x88\xAB": "ど",
    b"\x88\xAC": "る",
    b"\x88\xAD": "へ",
    b"\x88\xAE": "ん",
    b"\x88\xAF": "こ",
    b"\x88\xB0": "う",
    b"\x88\xB1": "ポ",
    b"\x88\xB2": "ズ",
    b"\x88\xB3": "メ",
    b"\x88\xB4": "ニ",
    b"\x88\xB5": "ュ",
    b"\x88\xB6": "オ",
    b"\x88\xB7": "プ",
    b"\x88\xB8": "シ",
    b"\x88\xB9": "ョ",
    b"\x88\xBA": "ン",
    b"\x88\xBB": "設",
    b"\x88\xBC": "定",
    b"\x88\xBD": "そ",
    b"\x88\xBE": "さ",
    b"\x88\xBF": "説",
    b"\x88\xC0": "明",
    b"\x88\xC1": "ゲ",
    b"\x88\xC2": "ム",
    b"\x88\xC3": "に",
    b"\x88\xC4": "本",
    b"\x88\xC5": "当",
    b"\x88\xC6": "し",
    b"\x88\xC7": "ま",
    b"\x88\xC8": "す",
    b"\x88\xC9": "か",
    b"\x88\xCA": "?",
    b"\x88\xCB": "は",
    b"\x88\xCC": "え",
    b"\x88\xCD": "せ",
    b"\x88\xCE": "サ",
    b"\x88\xCF": "ウ",
    b"\x88\xD0": "ド",
    b"\x88\xD1": "バ",
    b"\x88\xD2": "ブ",
    b"\x88\xD3": "レ",
    b"\x88\xD4": "B",
    b"\x88\xD5": "G",
    b"\x88\xD6": "M",
    b"\x88\xD7": "S",
    b"\x88\xD8": "E",
    b"\x88\xD9": "モ",
    b"\x88\xDA": "ノ",
    b"\x88\xDB": "ル",
    b"\x88\xDC": "ス",
    b"\x88\xDD": "テ",
    b"\x88\xDE": "ビ",
    b"\x88\xDF": "ロ",
    b"\x88\xE0": "ジ",
    b"\x88\xE1": "ッ",
    b"\x88\xE2": "ク",
    b"\x88\xE3": "2",
    b"\x88\xE4": "の",
    b"\x88\xE5": "出",
    b"\x88\xE6": "力",
    b"\x88\xE7": "形",
    b"\x88\xE8": "式",
    b"\x88\xE9": "ゅ",
    b"\x88\xEA": "つ",
    b"\x88\xEB": "り",
    b"\x88\xEC": "ょ",
    b"\x88\xED": "く",
    b"\x88\xEE": "き",
    b"\x88\xEF": "を",
    b"\x88\xF0": "選",
    b"\x88\xF1": "択",
    b"\x88\xF2": "た",
    b"\x88\xF3": "だ",
    b"\x88\xF4": "。",
    b"\x88\xF5": "フ",
    b"\x88\xF6": "⋅",
    b"\x88\xF7": "I",
    b"\x88\xF8": "N",
    b"\x88\xF9": "A",
    b"\x88\xFA": "X",
    b"\x88\xFB": "音",
    b"\x88\xFC": "量",
    b"\x89\x40": "お",
    #b"\x89\x41": "お",
}

BYTES_TO_CHAR_54_UFO = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ー",
    b"\x88\xA0": "リ",
    b"\x88\xA1": "ト",
    b"\x88\xA2": "ラ",
    b"\x88\xA3": "イ",
    b"\x88\xA4": "タ",
    b"\x88\xA5": "ア",
    b"\x88\xA6": "け",
    b"\x88\xA7": "っ",
    b"\x88\xA8": "て",
    b"\x88\xA9": "い",
    b"\x88\xAA": "も",
    b"\x88\xAB": "ど",
    b"\x88\xAC": "る",
    b"\x88\xAD": "へ",
    b"\x88\xAE": "ん",
    b"\x88\xAF": "こ",
    b"\x88\xB0": "う",
    b"\x88\xB1": "ポ",
    b"\x88\xB2": "ズ",
    b"\x88\xB3": "メ",
    b"\x88\xB4": "ニ",
    b"\x88\xB5": "ュ",
    b"\x88\xB6": "オ",
    b"\x88\xB7": "プ",
    b"\x88\xB8": "シ",
    b"\x88\xB9": "ョ",
    b"\x88\xBA": "ン",
    b"\x88\xBB": "設",
    b"\x88\xBC": "定",
    b"\x88\xBD": "そ",
    b"\x88\xBE": "さ",
    b"\x88\xBF": "説",
    b"\x88\xC0": "明",
    b"\x88\xC1": "ゲ",
    b"\x88\xC2": "ム",
    b"\x88\xC3": "に",
    b"\x88\xC4": "本",
    b"\x88\xC5": "当",
    b"\x88\xC6": "し",
    b"\x88\xC7": "ま",
    b"\x88\xC8": "す",
    b"\x88\xC9": "か",
    b"\x88\xCA": "?",
    b"\x88\xCB": "は",
    b"\x88\xCC": "え",
    b"\x88\xCD": "せ",
    b"\x88\xCE": "サ",
    b"\x88\xCF": "ウ",
    b"\x88\xD0": "ド",
    b"\x88\xD1": "バ",
    b"\x88\xD2": "ブ",
    b"\x88\xD3": "レ",
    b"\x88\xD4": "B",
    b"\x88\xD5": "G",
    b"\x88\xD6": "M",
    b"\x88\xD7": "S",
    b"\x88\xD8": "E",
    b"\x88\xD9": "モ",
    b"\x88\xDA": "ノ",
    b"\x88\xDB": "ル",
    b"\x88\xDC": "ス",
    b"\x88\xDD": "テ",
    b"\x88\xDE": "ビ",
    b"\x88\xDF": "ロ",
    b"\x88\xE0": "ジ",
    b"\x88\xE1": "ッ",
    b"\x88\xE2": "ク",
    b"\x88\xE3": "2",
    b"\x88\xE4": "の",
    b"\x88\xE5": "出",
    b"\x88\xE6": "力",
    b"\x88\xE7": "形",
    b"\x88\xE8": "式",
    b"\x88\xE9": "ゅ",
    b"\x88\xEA": "つ",
    b"\x88\xEB": "り",
    b"\x88\xEC": "ょ",
    b"\x88\xED": "く",
    b"\x88\xEE": "き",
    b"\x88\xEF": "を",
    b"\x88\xF0": "選",
    b"\x88\xF1": "択",
    b"\x88\xF2": "た",
    b"\x88\xF3": "だ",
    b"\x88\xF4": "。",
    b"\x88\xF5": "フ",
    b"\x88\xF6": "⋅",
    b"\x88\xF7": "I",
    b"\x88\xF8": "N",
    b"\x88\xF9": "A",
    b"\x88\xFA": "X",
    b"\x88\xFB": "音",
    b"\x88\xFC": "量",
    b"\x89\x40": "お",
    #b"\x89\x41": "お",
}

BYTES_TO_CHAR_55_PARK = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ー",
    b"\x88\xA0": "リ",
    b"\x88\xA1": "ト",
    b"\x88\xA2": "ラ",
    b"\x88\xA3": "イ",
    b"\x88\xA4": "タ",
    b"\x88\xA5": "ア",
    b"\x88\xA6": "け",
    b"\x88\xA7": "っ",
    b"\x88\xA8": "て",
    b"\x88\xA9": "い",
    b"\x88\xAA": "も",
    b"\x88\xAB": "ど",
    b"\x88\xAC": "る",
    b"\x88\xAD": "へ",
    b"\x88\xAE": "ん",
    b"\x88\xAF": "こ",
    b"\x88\xB0": "う",
    b"\x88\xB1": "ポ",
    b"\x88\xB2": "ズ",
    b"\x88\xB3": "メ",
    b"\x88\xB4": "ニ",
    b"\x88\xB5": "ュ",
    b"\x88\xB6": "オ",
    b"\x88\xB7": "プ",
    b"\x88\xB8": "シ",
    b"\x88\xB9": "ョ",
    b"\x88\xBA": "ン",
    b"\x88\xBB": "設",
    b"\x88\xBC": "定",
    b"\x88\xBD": "そ",
    b"\x88\xBE": "さ",
    b"\x88\xBF": "説",
    b"\x88\xC0": "明",
    b"\x88\xC1": "ゲ",
    b"\x88\xC2": "ム",
    b"\x88\xC3": "に",
    b"\x88\xC4": "本",
    b"\x88\xC5": "当",
    b"\x88\xC6": "し",
    b"\x88\xC7": "ま",
    b"\x88\xC8": "す",
    b"\x88\xC9": "か",
    b"\x88\xCA": "?",
    b"\x88\xCB": "は",
    b"\x88\xCC": "え",
    b"\x88\xCD": "せ",
    b"\x88\xCE": "サ",
    b"\x88\xCF": "ウ",
    b"\x88\xD0": "ド",
    b"\x88\xD1": "バ",
    b"\x88\xD2": "ブ",
    b"\x88\xD3": "レ",
    b"\x88\xD4": "B",
    b"\x88\xD5": "G",
    b"\x88\xD6": "M",
    b"\x88\xD7": "S",
    b"\x88\xD8": "E",
    b"\x88\xD9": "モ",
    b"\x88\xDA": "ノ",
    b"\x88\xDB": "ル",
    b"\x88\xDC": "ス",
    b"\x88\xDD": "テ",
    b"\x88\xDE": "ビ",
    b"\x88\xDF": "ロ",
    b"\x88\xE0": "ジ",
    b"\x88\xE1": "ッ",
    b"\x88\xE2": "ク",
    b"\x88\xE3": "2",
    b"\x88\xE4": "の",
    b"\x88\xE5": "出",
    b"\x88\xE6": "力",
    b"\x88\xE7": "形",
    b"\x88\xE8": "式",
    b"\x88\xE9": "ゅ",
    b"\x88\xEA": "つ",
    b"\x88\xEB": "り",
    b"\x88\xEC": "ょ",
    b"\x88\xED": "く",
    b"\x88\xEE": "き",
    b"\x88\xEF": "を",
    b"\x88\xF0": "選",
    b"\x88\xF1": "択",
    b"\x88\xF2": "た",
    b"\x88\xF3": "だ",
    b"\x88\xF4": "。",
    b"\x88\xF5": "フ",
    b"\x88\xF6": "⋅",
    b"\x88\xF7": "I",
    b"\x88\xF8": "N",
    b"\x88\xF9": "A",
    b"\x88\xFA": "X",
    b"\x88\xFB": "音",
    b"\x88\xFC": "量",
    b"\x89\x40": "お",
    #b"\x89\x41": "お",
}

BYTES_TO_CHAR_56_DAIBA = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ー",
    b"\x88\xA0": "リ",
    b"\x88\xA1": "ト",
    b"\x88\xA2": "ラ",
    b"\x88\xA3": "イ",
    b"\x88\xA4": "タ",
    b"\x88\xA5": "ア",
    b"\x88\xA6": "け",
    b"\x88\xA7": "っ",
    b"\x88\xA8": "て",
    b"\x88\xA9": "い",
    b"\x88\xAA": "も",
    b"\x88\xAB": "ど",
    b"\x88\xAC": "る",
    b"\x88\xAD": "へ",
    b"\x88\xAE": "ん",
    b"\x88\xAF": "こ",
    b"\x88\xB0": "う",
    b"\x88\xB1": "ポ",
    b"\x88\xB2": "ズ",
    b"\x88\xB3": "メ",
    b"\x88\xB4": "ニ",
    b"\x88\xB5": "ュ",
    b"\x88\xB6": "オ",
    b"\x88\xB7": "プ",
    b"\x88\xB8": "シ",
    b"\x88\xB9": "ョ",
    b"\x88\xBA": "ン",
    b"\x88\xBB": "設",
    b"\x88\xBC": "定",
    b"\x88\xBD": "そ",
    b"\x88\xBE": "さ",
    b"\x88\xBF": "説",
    b"\x88\xC0": "明",
    b"\x88\xC1": "ゲ",
    b"\x88\xC2": "ム",
    b"\x88\xC3": "に",
    b"\x88\xC4": "本",
    b"\x88\xC5": "当",
    b"\x88\xC6": "し",
    b"\x88\xC7": "ま",
    b"\x88\xC8": "す",
    b"\x88\xC9": "か",
    b"\x88\xCA": "?",
    b"\x88\xCB": "は",
    b"\x88\xCC": "え",
    b"\x88\xCD": "せ",
    b"\x88\xCE": "サ",
    b"\x88\xCF": "ウ",
    b"\x88\xD0": "ド",
    b"\x88\xD1": "バ",
    b"\x88\xD2": "ブ",
    b"\x88\xD3": "レ",
    b"\x88\xD4": "B",
    b"\x88\xD5": "G",
    b"\x88\xD6": "M",
    b"\x88\xD7": "S",
    b"\x88\xD8": "E",
    b"\x88\xD9": "モ",
    b"\x88\xDA": "ノ",
    b"\x88\xDB": "ル",
    b"\x88\xDC": "ス",
    b"\x88\xDD": "テ",
    b"\x88\xDE": "ビ",
    b"\x88\xDF": "ロ",
    b"\x88\xE0": "ジ",
    b"\x88\xE1": "ッ",
    b"\x88\xE2": "ク",
    b"\x88\xE3": "2",
    b"\x88\xE4": "の",
    b"\x88\xE5": "出",
    b"\x88\xE6": "力",
    b"\x88\xE7": "形",
    b"\x88\xE8": "式",
    b"\x88\xE9": "ゅ",
    b"\x88\xEA": "つ",
    b"\x88\xEB": "り",
    b"\x88\xEC": "ょ",
    b"\x88\xED": "く",
    b"\x88\xEE": "き",
    b"\x88\xEF": "を",
    b"\x88\xF0": "選",
    b"\x88\xF1": "択",
    b"\x88\xF2": "た",
    b"\x88\xF3": "だ",
    b"\x88\xF4": "。",
    b"\x88\xF5": "フ",
    b"\x88\xF6": "⋅",
    b"\x88\xF7": "I",
    b"\x88\xF8": "N",
    b"\x88\xF9": "A",
    b"\x88\xFA": "X",
    b"\x88\xFB": "音",
    b"\x88\xFC": "量",
    b"\x89\x40": "お",
    #b"\x89\x41": "お",
    #b"\x89\x46": " ",
    #b"\x89\x47": " ",
    #b"\x89\x48": " ",
    #b"\x89\x49": " ",
    #b"\x89\x4A": " ",
    #b"\x89\x4B": " ",
    #b"\x89\x4C": " ",
    #b"\x89\x4D": " ",
    #b"\x89\x4E": " ",
    #b"\x89\x4F": " ",
    #b"\x89\x50": " ",
    #b"\x89\x51": " ",
    #b"\x89\x52": " ",
    #b"\x89\x53": " ",
    #b"\x89\x54": " ",
    #b"\x89\x55": " ",
    #b"\x89\x56": " ",
    #b"\x89\x57": " ",
}

BYTES_TO_CHAR_57_K1DEATH = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ー",
    b"\x88\xA0": "リ",
    b"\x88\xA1": "ト",
    b"\x88\xA2": "ラ",
    b"\x88\xA3": "イ",
    b"\x88\xA4": "タ",
    b"\x88\xA5": "ア",
    b"\x88\xA6": "け",
    b"\x88\xA7": "っ",
    b"\x88\xA8": "て",
    b"\x88\xA9": "い",
    b"\x88\xAA": "も",
    b"\x88\xAB": "ど",
    b"\x88\xAC": "る",
    b"\x88\xAD": "へ",
    b"\x88\xAE": "ん",
    b"\x88\xAF": "こ",
    b"\x88\xB0": "う",
    b"\x88\xB1": "ポ",
    b"\x88\xB2": "ズ",
    b"\x88\xB3": "メ",
    b"\x88\xB4": "ニ",
    b"\x88\xB5": "ュ",
    b"\x88\xB6": "オ",
    b"\x88\xB7": "プ",
    b"\x88\xB8": "シ",
    b"\x88\xB9": "ョ",
    b"\x88\xBA": "ン",
    b"\x88\xBB": "設",
    b"\x88\xBC": "定",
    b"\x88\xBD": "そ",
    b"\x88\xBE": "さ",
    b"\x88\xBF": "説",
    b"\x88\xC0": "明",
    b"\x88\xC1": "ゲ",
    b"\x88\xC2": "ム",
    b"\x88\xC3": "に",
    b"\x88\xC4": "本",
    b"\x88\xC5": "当",
    b"\x88\xC6": "し",
    b"\x88\xC7": "ま",
    b"\x88\xC8": "す",
    b"\x88\xC9": "か",
    b"\x88\xCA": "?",
    b"\x88\xCB": "は",
    b"\x88\xCC": "え",
    b"\x88\xCD": "せ",
    b"\x88\xCE": "サ",
    b"\x88\xCF": "ウ",
    b"\x88\xD0": "ド",
    b"\x88\xD1": "バ",
    b"\x88\xD2": "ブ",
    b"\x88\xD3": "レ",
    b"\x88\xD4": "B",
    b"\x88\xD5": "G",
    b"\x88\xD6": "M",
    b"\x88\xD7": "S",
    b"\x88\xD8": "E",
    b"\x88\xD9": "モ",
    b"\x88\xDA": "ノ",
    b"\x88\xDB": "ル",
    b"\x88\xDC": "ス",
    b"\x88\xDD": "テ",
    b"\x88\xDE": "ビ",
    b"\x88\xDF": "ロ",
    b"\x88\xE0": "ジ",
    b"\x88\xE1": "ッ",
    b"\x88\xE2": "ク",
    b"\x88\xE3": "2",
    b"\x88\xE4": "の",
    b"\x88\xE5": "出",
    b"\x88\xE6": "力",
    b"\x88\xE7": "形",
    b"\x88\xE8": "式",
    b"\x88\xE9": "ゅ",
    b"\x88\xEA": "つ",
    b"\x88\xEB": "り",
    b"\x88\xEC": "ょ",
    b"\x88\xED": "く",
    b"\x88\xEE": "き",
    b"\x88\xEF": "を",
    b"\x88\xF0": "選",
    b"\x88\xF1": "択",
    b"\x88\xF2": "た",
    b"\x88\xF3": "だ",
    b"\x88\xF4": "。",
    b"\x88\xF5": "フ",
    b"\x88\xF6": "⋅",
    b"\x88\xF7": "I",
    b"\x88\xF8": "N",
    b"\x88\xF9": "A",
    b"\x88\xFA": "X",
    b"\x88\xFB": "音",
    b"\x88\xFC": "量",
    b"\x89\x40": "お",
    #b"\x89\x41": "お",
}

BYTES_TO_CHAR_58_UFO2 = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ー",
    b"\x88\xA0": "リ",
    b"\x88\xA1": "ト",
    b"\x88\xA2": "ラ",
    b"\x88\xA3": "イ",
    b"\x88\xA4": "タ",
    b"\x88\xA5": "ア",
    b"\x88\xA6": "け",
    b"\x88\xA7": "っ",
    b"\x88\xA8": "て",
    b"\x88\xA9": "い",
    b"\x88\xAA": "も",
    b"\x88\xAB": "ど",
    b"\x88\xAC": "る",
    b"\x88\xAD": "へ",
    b"\x88\xAE": "ん",
    b"\x88\xAF": "こ",
    b"\x88\xB0": "う",
    b"\x88\xB1": "ポ",
    b"\x88\xB2": "ズ",
    b"\x88\xB3": "メ",
    b"\x88\xB4": "ニ",
    b"\x88\xB5": "ュ",
    b"\x88\xB6": "オ",
    b"\x88\xB7": "プ",
    b"\x88\xB8": "シ",
    b"\x88\xB9": "ョ",
    b"\x88\xBA": "ン",
    b"\x88\xBB": "設",
    b"\x88\xBC": "定",
    b"\x88\xBD": "そ",
    b"\x88\xBE": "さ",
    b"\x88\xBF": "説",
    b"\x88\xC0": "明",
    b"\x88\xC1": "ゲ",
    b"\x88\xC2": "ム",
    b"\x88\xC3": "に",
    b"\x88\xC4": "本",
    b"\x88\xC5": "当",
    b"\x88\xC6": "し",
    b"\x88\xC7": "ま",
    b"\x88\xC8": "す",
    b"\x88\xC9": "か",
    b"\x88\xCA": "?",
    b"\x88\xCB": "は",
    b"\x88\xCC": "え",
    b"\x88\xCD": "せ",
    b"\x88\xCE": "サ",
    b"\x88\xCF": "ウ",
    b"\x88\xD0": "ド",
    b"\x88\xD1": "バ",
    b"\x88\xD2": "ブ",
    b"\x88\xD3": "レ",
    b"\x88\xD4": "B",
    b"\x88\xD5": "G",
    b"\x88\xD6": "M",
    b"\x88\xD7": "S",
    b"\x88\xD8": "E",
    b"\x88\xD9": "モ",
    b"\x88\xDA": "ノ",
    b"\x88\xDB": "ル",
    b"\x88\xDC": "ス",
    b"\x88\xDD": "テ",
    b"\x88\xDE": "ビ",
    b"\x88\xDF": "ロ",
    b"\x88\xE0": "ジ",
    b"\x88\xE1": "ッ",
    b"\x88\xE2": "ク",
    b"\x88\xE3": "2",
    b"\x88\xE4": "の",
    b"\x88\xE5": "出",
    b"\x88\xE6": "力",
    b"\x88\xE7": "形",
    b"\x88\xE8": "式",
    b"\x88\xE9": "ゅ",
    b"\x88\xEA": "つ",
    b"\x88\xEB": "り",
    b"\x88\xEC": "ょ",
    b"\x88\xED": "く",
    b"\x88\xEE": "き",
    b"\x88\xEF": "を",
    b"\x88\xF0": "選",
    b"\x88\xF1": "択",
    b"\x88\xF2": "た",
    b"\x88\xF3": "だ",
    b"\x88\xF4": "。",
    b"\x88\xF5": "フ",
    b"\x88\xF6": "⋅",
    b"\x88\xF7": "I",
    b"\x88\xF8": "N",
    b"\x88\xF9": "A",
    b"\x88\xFA": "X",
    b"\x88\xFB": "音",
    b"\x88\xFC": "量",
    b"\x89\x40": "お",
    #b"\x89\x41": "お",
}

BYTES_TO_CHAR_60_K1 = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ー",
    b"\x88\xA0": "リ",
    b"\x88\xA1": "ト",
    b"\x88\xA2": "ラ",
    b"\x88\xA3": "イ",
    b"\x88\xA4": "タ",
    b"\x88\xA5": "ア",
    b"\x88\xA6": "け",
    b"\x88\xA7": "っ",
    b"\x88\xA8": "て",
    b"\x88\xA9": "い",
    b"\x88\xAA": "も",
    b"\x88\xAB": "ど",
    b"\x88\xAC": "る",
    b"\x88\xAD": "へ",
    b"\x88\xAE": "ん",
    b"\x88\xAF": "こ",
    b"\x88\xB0": "う",
    b"\x88\xB1": "ポ",
    b"\x88\xB2": "ズ",
    b"\x88\xB3": "メ",
    b"\x88\xB4": "ニ",
    b"\x88\xB5": "ュ",
    b"\x88\xB6": "オ",
    b"\x88\xB7": "プ",
    b"\x88\xB8": "シ",
    b"\x88\xB9": "ョ",
    b"\x88\xBA": "ン",
    b"\x88\xBB": "設",
    b"\x88\xBC": "定",
    b"\x88\xBD": "そ",
    b"\x88\xBE": "さ",
    b"\x88\xBF": "説",
    b"\x88\xC0": "明",
    b"\x88\xC1": "ゲ",
    b"\x88\xC2": "ム",
    b"\x88\xC3": "に",
    b"\x88\xC4": "本",
    b"\x88\xC5": "当",
    b"\x88\xC6": "し",
    b"\x88\xC7": "ま",
    b"\x88\xC8": "す",
    b"\x88\xC9": "か",
    b"\x88\xCA": "?",
    b"\x88\xCB": "は",
    b"\x88\xCC": "え",
    b"\x88\xCD": "せ",
    b"\x88\xCE": "サ",
    b"\x88\xCF": "ウ",
    b"\x88\xD0": "ド",
    b"\x88\xD1": "バ",
    b"\x88\xD2": "ブ",
    b"\x88\xD3": "レ",
    b"\x88\xD4": "B",
    b"\x88\xD5": "G",
    b"\x88\xD6": "M",
    b"\x88\xD7": "S",
    b"\x88\xD8": "E",
    b"\x88\xD9": "モ",
    b"\x88\xDA": "ノ",
    b"\x88\xDB": "ル",
    b"\x88\xDC": "ス",
    b"\x88\xDD": "テ",
    b"\x88\xDE": "ビ",
    b"\x88\xDF": "ロ",
    b"\x88\xE0": "ジ",
    b"\x88\xE1": "ッ",
    b"\x88\xE2": "ク",
    b"\x88\xE3": "2",
    b"\x88\xE4": "の",
    b"\x88\xE5": "出",
    b"\x88\xE6": "力",
    b"\x88\xE7": "形",
    b"\x88\xE8": "式",
    b"\x88\xE9": "ゅ",
    b"\x88\xEA": "つ",
    b"\x88\xEB": "り",
    b"\x88\xEC": "ょ",
    b"\x88\xED": "く",
    b"\x88\xEE": "き",
    b"\x88\xEF": "を",
    b"\x88\xF0": "選",
    b"\x88\xF1": "択",
    b"\x88\xF2": "た",
    b"\x88\xF3": "だ",
    b"\x88\xF4": "。",
    b"\x88\xF5": "フ",
    b"\x88\xF6": "⋅",
    b"\x88\xF7": "I",
    b"\x88\xF8": "N",
    b"\x88\xF9": "A",
    b"\x88\xFA": "X",
    b"\x88\xFB": "音",
    b"\x88\xFC": "量",
    b"\x89\x40": "お",
    #b"\x89\x41": "お",
}

BYTES_TO_CHAR_61_K1DEATH = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "ー",
    b"\x88\xA0": "リ",
    b"\x88\xA1": "ト",
    b"\x88\xA2": "ラ",
    b"\x88\xA3": "イ",
    b"\x88\xA4": "タ",
    b"\x88\xA5": "ア",
    b"\x88\xA6": "け",
    b"\x88\xA7": "っ",
    b"\x88\xA8": "て",
    b"\x88\xA9": "い",
    b"\x88\xAA": "も",
    b"\x88\xAB": "ど",
    b"\x88\xAC": "る",
    b"\x88\xAD": "へ",
    b"\x88\xAE": "ん",
    b"\x88\xAF": "こ",
    b"\x88\xB0": "う",
    b"\x88\xB1": "ポ",
    b"\x88\xB2": "ズ",
    b"\x88\xB3": "メ",
    b"\x88\xB4": "ニ",
    b"\x88\xB5": "ュ",
    b"\x88\xB6": "オ",
    b"\x88\xB7": "プ",
    b"\x88\xB8": "シ",
    b"\x88\xB9": "ョ",
    b"\x88\xBA": "ン",
    b"\x88\xBB": "設",
    b"\x88\xBC": "定",
    b"\x88\xBD": "そ",
    b"\x88\xBE": "さ",
    b"\x88\xBF": "説",
    b"\x88\xC0": "明",
    b"\x88\xC1": "ゲ",
    b"\x88\xC2": "ム",
    b"\x88\xC3": "に",
    b"\x88\xC4": "本",
    b"\x88\xC5": "当",
    b"\x88\xC6": "し",
    b"\x88\xC7": "ま",
    b"\x88\xC8": "す",
    b"\x88\xC9": "か",
    b"\x88\xCA": "?",
    b"\x88\xCB": "は",
    b"\x88\xCC": "え",
    b"\x88\xCD": "せ",
    b"\x88\xCE": "サ",
    b"\x88\xCF": "ウ",
    b"\x88\xD0": "ド",
    b"\x88\xD1": "バ",
    b"\x88\xD2": "ブ",
    b"\x88\xD3": "レ",
    b"\x88\xD4": "B",
    b"\x88\xD5": "G",
    b"\x88\xD6": "M",
    b"\x88\xD7": "S",
    b"\x88\xD8": "E",
    b"\x88\xD9": "モ",
    b"\x88\xDA": "ノ",
    b"\x88\xDB": "ル",
    b"\x88\xDC": "ス",
    b"\x88\xDD": "テ",
    b"\x88\xDE": "ビ",
    b"\x88\xDF": "ロ",
    b"\x88\xE0": "ジ",
    b"\x88\xE1": "ッ",
    b"\x88\xE2": "ク",
    b"\x88\xE3": "2",
    b"\x88\xE4": "の",
    b"\x88\xE5": "出",
    b"\x88\xE6": "力",
    b"\x88\xE7": "形",
    b"\x88\xE8": "式",
    b"\x88\xE9": "ゅ",
    b"\x88\xEA": "つ",
    b"\x88\xEB": "り",
    b"\x88\xEC": "ょ",
    b"\x88\xED": "く",
    b"\x88\xEE": "き",
    b"\x88\xEF": "を",
    b"\x88\xF0": "選",
    b"\x88\xF1": "択",
    b"\x88\xF2": "た",
    b"\x88\xF3": "だ",
    b"\x88\xF4": "。",
    b"\x88\xF5": "フ",
    b"\x88\xF6": "⋅",
    b"\x88\xF7": "I",
    b"\x88\xF8": "N",
    b"\x88\xF9": "A",
    b"\x88\xFA": "X",
    b"\x88\xFB": "音",
    b"\x88\xFC": "量",
    b"\x89\x40": "お",
    #b"\x89\x41": "お",
}

BYTES_TO_CHAR_CREDIT = {
    **BYTES_TO_CHAR_SPECIAL,

    b"\x88\x9F": "■",
    b"\x88\xA0": "ス",
    b"\x88\xA1": "タ",
    b"\x88\xA2": "ッ",
    b"\x88\xA3": "フ",
    b"\x88\xA4": "●",
    b"\x88\xA5": "プ",
    b"\x88\xA6": "ラ",
    b"\x88\xA7": "ン",
    b"\x88\xA8": "ナ",
    b"\x88\xA9": "ー",
    b"\x88\xAA": "キ",
    b"\x88\xAB": "ャ",
    b"\x88\xAC": "ク",
    b"\x88\xAD": "ま",
    b"\x88\xAE": "わ",
    b"\x88\xAF": "り",
    b"\x88\xB0": "大",
    b"\x88\xB1": "場",
    b"\x88\xB2": " ",
    b"\x88\xB3": "俊",
    b"\x88\xB4": "治",
    b"\x88\xB5": "工",
    b"\x88\xB6": "藤",
    b"\x88\xB7": "匡",
    b"\x88\xB8": "史",
    b"\x88\xB9": "テ",
    b"\x88\xBA": "ジ",
    b"\x88\xBB": "設",
    b"\x88\xBC": "計",
    b"\x88\xBD": "/",
    b"\x88\xBE": "リ",
    b"\x88\xBF": "ア",
    b"\x88\xC0": "ル",
    b"\x88\xC1": "イ",
    b"\x88\xC2": "ム",
    b"\x88\xC3": "デ",
    b"\x88\xC4": "モ",
    b"\x88\xC5": "田",
    b"\x88\xC6": "坂",
    b"\x88\xC7": "尚",
    b"\x88\xC8": "昭",
    b"\x88\xC9": "コ",
    b"\x88\xCA": "ョ",
    b"\x88\xCB": "作",
    b"\x88\xCC": "成",
    b"\x88\xCD": "橋",
    b"\x88\xCE": "本",
    b"\x88\xCF": "修",
    b"\x88\xD0": "平",
    b"\x88\xD1": "ボ",
    b"\x88\xD2": "カ",
    b"\x88\xD3": "メ",
    b"\x88\xD4": "定",
    b"\x88\xD5": "H",
    b"\x88\xD6": "A",
    b"\x88\xD7": "P",
    b"\x88\xD8": "Y",
    b"\x88\xD9": "!",
    b"\x88\xDA": "サ",
    b"\x88\xDB": "Χ",
    b"\x88\xDC": "ト",
    b"\x88\xDD": "ロ",
    b"\x88\xDE": "小",
    b"\x88\xDF": "豆",
    b"\x88\xE0": "畑",
    b"\x88\xE1": "裕",
    b"\x88\xE2": "ニ",
    b"\x88\xE3": "ュ",
    b"\x88\xE4": "松",
    b"\x88\xE5": "シ",
    b"\x88\xE6": "也",
    b"\x88\xE7": "石",
    b"\x88\xE8": "川",
    b"\x88\xE9": "千",
    b"\x88\xEA": "愛",
    b"\x88\xEB": "ウ",
    b"\x88\xEC": "ド",
    b"\x88\xED": "エ",
    b"\x88\xEE": "ネ",
    b"\x88\xEF": "ミ",
    b"\x88\xF0": "ブ",
    b"\x88\xF1": "ゲ",
    b"\x88\xF2": "チ",
    b"\x88\xF3": "横",
    b"\x88\xF4": "恒",
    b"\x88\xF5": "宏",
    b"\x88\xF6": "配",
    b"\x88\xF7": "置",
    b"\x88\xF8": "調",
    b"\x88\xF9": "整",
    b"\x88\xFA": "及",
    b"\x88\xFB": "怜",
    b"\x88\xFC": "島",
    b"\x89\x40": "淳",
    b"\x89\x41": "ザ",
    b"\x89\x42": "ィ",
    b"\x89\x43": "レ",
    b"\x89\x44": "寺",
    b"\x89\x45": "忠",
    b"\x89\x46": "亮",
    b"\x89\x47": "谷",
    b"\x89\x48": "口",
    b"\x89\x49": "晋",
    b"\x89\x4A": "太",
    b"\x89\x4B": "一",
    b"\x89\x4C": "U",
    b"\x89\x4D": "I",
    b"\x89\x4E": "ガ",
    b"\x89\x4F": "佐",
    b"\x89\x50": "久",
    b"\x89\x51": "間",
    b"\x89\x52": "英",
    b"\x89\x53": "樹",
    b"\x89\x54": "船",
    b"\x89\x55": "山",
    b"\x89\x56": "征",
    b"\x89\x57": "郎",
    b"\x89\x58": "河",
    b"\x89\x59": "春",
    b"\x89\x5A": "彦",
    b"\x89\x5B": "お",
    b"\x89\x5C": "の",
    b"\x89\x5D": "さ",
    b"\x89\x5E": "と",
    b"\x89\x5F": "し",
    b"\x89\x60": "村",
    b"\x89\x61": "上",
    b"\x89\x62": "西",
    b"\x89\x63": "博",
    b"\x89\x64": "領",
    b"\x89\x65": "真",
    b"\x89\x66": "中",
    b"\x89\x67": "和",
    b"\x89\x68": "清",
    b"\x89\x69": "水",
    b"\x89\x6A": "映",
    b"\x89\x6B": "輔",
    b"\x89\x6C": "根",
    b"\x89\x6D": "健",
    b"\x89\x6E": "井",
    b"\x89\x6F": "慶",
    b"\x89\x70": "女",
    # They distinguish between forward and backwards double quotes?
    b"\x89\x71": "“",
    b"\x89\x72": "h",
    b"\x89\x73": "i",
    b"\x89\x74": "x",
    b"\x89\x75": '”',
    b"\x89\x76": "秀",
    b"\x89\x77": "照",
    b"\x89\x78": "孝",
    b"\x89\x79": "芳",
    b"\x89\x7A": "宮",
    b"\x89\x7B": "崎",
    b"\x89\x7C": "司",
    b"\x89\x7D": "柳",
    b"\x89\x7E": "輝",
    b"\x89\x80": "陽",
    b"\x89\x81": "子",
    b"\x89\x82": "朝",
    b"\x89\x83": "戸",
    # Best guess, the font looks like shit
    b"\x89\x84": "薫",
    b"\x89\x85": "土",
    b"\x89\x86": "誠",
    b"\x89\x87": "ェ",
    b"\x89\x88": "泉",
    b"\x89\x89": "信",
    b"\x89\x8A": "長",
    b"\x89\x8B": "郷",
    b"\x89\x8C": "絵",
    b"\x89\x8D": "美",
    b"\x89\x8E": "ビ",
    b"\x89\x8F": "堀",
    b"\x89\x90": "正",
    b"\x89\x91": "二",
    b"\x89\x92": "保",
    b"\x89\x93": "佳",
    b"\x89\x94": "グ",
    b"\x89\x95": "マ",
    b"\x89\x96": "澤",
    b"\x89\x97": "伸",
    b"\x89\x98": "弘",
    b"\x89\x99": "桑",
    b"\x89\x9A": "直",
    b"\x89\x9B": "文",
    b"\x89\x9C": "穂",
    b"\x89\x9D": "満",
    b"\x89\x9E": "憲",
    b"\x89\x9F": "明",
    b"\x89\xA0": "制",
    b"\x89\xA1": "御",
    b"\x89\xA2": "相",
    b"\x89\xA3": "慎",
    b"\x89\xA4": "竹",
    b"\x89\xA5": "内",
    b"\x89\xA6": "敏",
    b"\x89\xA7": "ツ",
    b"\x89\xA8": "ポ",
    b"\x89\xA9": "高",
    b"\x89\xAA": "智",
    b"\x89\xAB": "之",
    b"\x89\xAC": "描",
    b"\x89\xAD": "画",
    b"\x89\xAE": "や",
    b"\x89\xAF": "ぐ",
    b"\x89\xB0": "ち",
    b"\x89\xB1": "た",
    b"\x89\xB2": "い",
    b"\x89\xB3": "光",
    b"\x89\xB4": "幸",
    b"\x89\xB5": "金",
    b"\x89\xB6": "貴",
    b"\x89\xB7": "増",
    b"\x89\xB8": "延",
    b"\x89\xB9": "朗",
    b"\x89\xBA": "越",
    b"\x89\xBB": "M",
    b"\x89\xBC": "粕",
    b"\x89\xBD": "浩",
    b"\x89\xBE": "祐",
    b"\x89\xBF": "ケ",
    b"\x89\xC0": "瀧",
    b"\x89\xC1": "富",
    b"\x89\xC2": "土",
    b"\x89\xC3": "ハ",
    b"\x89\xC4": "セ",
    b"\x89\xC5": "八",
    b"\x89\xC6": "奈",
    b"\x89\xC7": "見",
    b"\x89\xC8": "乗",
    b"\x89\xC9": "児",
    b"\x89\xCA": "も",
    b"\x89\xCB": "半",
    b"\x89\xCC": "友",
    b"\x89\xCD": "恵",
    b"\x89\xCE": "ヒ",
    b"\x89\xCF": "伊",
    b"\x89\xD0": "倉",
    b"\x89\xD1": "ペ",
    b"\x89\xD2": "夏",
    b"\x89\xD3": "謎",
    b"\x89\xD4": "男",
    b"\x89\xD5": "ピ",
    b"\x89\xD6": "達",
    b"\x89\xD7": "楠",
    b"\x89\xD8": "典",
    b"\x89\xD9": "⋅",
    b"\x89\xDA": "警",
    b"\x89\xDB": "備",
    b"\x89\xDC": "隊",
    b"\x89\xDD": "花",
    b"\x89\xDE": "輪",
    b"\x89\xDF": "T",
    b"\x89\xE0": "V",
    b"\x89\xE1": "前",
    b"\x89\xE2": "里",
    b"\x89\xE3": "下",
    b"\x89\xE4": "由",
    b"\x89\xE5": "紀",
    b"\x89\xE6": "ダ",
    b"\x89\xE7": "野",
    b"\x89\xE8": "拓",
    b"\x89\xE9": "(",
    b"\x89\xEA": "東",
    b"\x89\xEB": "北",
    b"\x89\xEC": "新",
    b"\x89\xED": "社",
    b"\x89\xEE": ")",
    b"\x89\xEF": "林",
    b"\x89\xF0": "菜",
    b"\x89\xF1": "加",
    b"\x89\xF2": "ズ",
    b"\x89\xF3": "創",
    b"\x89\xF4": "パ",
    b"\x89\xF5": "バ",
    b"\x89\xF6": "菊",
    b"\x89\xF7": "地",
    b"\x89\xF8": "敦",
    b"\x89\xF9": "ソ",
    b"\x89\xFA": "沢",
    b"\x89\xFB": "オ",
    b"\x89\xFC": "B",
    b"\x8A\x40": "O",
    b"\x8A\x41": "R",
    b"\x8A\x42": "e",
    b"\x8A\x43": "c",
    b"\x8A\x44": "o",
    b"\x8A\x45": "r",
    b"\x8A\x46": "d",
    b"\x8A\x47": "n",
    b"\x8A\x48": "g",
    b"\x8A\x49": "S",
    b"\x8A\x4A": "t",
    b"\x8A\x4B": "u",
    b"\x8A\x4C": "福",
    b"\x8A\x4D": "じ",
    b"\x8A\x4E": "っ",
    b"\x8A\x4F": "ゃ",
    b"\x8A\x50": "N",
    b"\x8A\x51": "ゅ",
    b"\x8A\x52": "つ",
    b"\x8A\x53": "え",
    b"\x8A\x54": "ん",
    b"\x8A\x55": "ヴ",
    b"\x8A\x56": "き",
    b"\x8A\x57": "う",
    b"\x8A\x58": "ぼ",
    b"\x8A\x59": "せ",
    b"\x8A\x5A": "だ",
    b"\x8A\x5B": "ひ",
    b"\x8A\x5C": "ょ",
    b"\x8A\x5D": "黒",
    b"\x8A\x5E": "康",
    b"\x8A\x5F": "か",
    b"\x8A\x60": "び",
    b"\x8A\x61": "ノ",
    b"\x8A\x62": "ギ",
    b"\x8A\x63": "ど",
    b"\x8A\x64": "く",
    b"\x8A\x65": "京",
    b"\x8A\x66": "&",
    b"\x8A\x67": "F",
    b"\x8A\x68": "l",
    b"\x8A\x69": "L",
    b"\x8A\x6A": "v",
    b"\x8A\x6B": "若",
    b"\x8A\x6C": "曽",
    # Best guess
    b"\x8A\x6D": "藁",
    b"\x8A\x6E": "円",
    b"\x8A\x6F": "ゴ",
    b"\x8A\x70": "均",
    b"\x8A\x71": "が",
    b"\x8A\x72": "篠",
    b"\x8A\x73": "章",
    b"\x8A\x74": "龍",
    b"\x8A\x75": "五",
    b"\x8A\x76": "十",
    b"\x8A\x77": "嵐",
    b"\x8A\x78": "三",
    b"\x8A\x79": "浦",
    b"\x8A\x7A": "徹",
    b"\x8A\x7B": "夫",
    b"\x8A\x7C": "肥",
    b"\x8A\x7D": "桂",
    # Incorrect token encoding??
    b"\x8A\x7E": "介",
    #b"\x8A\x7F": "介",
    b"\x8A\x80": "め",
    b"\x8A\x81": "圭",
    b"\x8A\x82": "掛",
    b"\x8A\x83": "々",
    b"\x8A\x84": "木",
    b"\x8A\x85": "靖",
    b"\x8A\x86": "E",
    b"\x8A\x87": "永",
    b"\x8A\x88": "後",
    b"\x8A\x89": "吉",
    b"\x8A\x8A": "譲",
    b"\x8A\x8B": "総",
    b"\x8A\x8C": "ヘ",
    b"\x8A\x8D": "早",
    b"\x8A\x8E": "実",
    b"\x8A\x8F": "よ",
    b"\x8A\x90": "岡",
    b"\x8A\x91": "ふ",
    b"\x8A\x92": "け",
    b"\x8A\x93": "阿",
    b"\x8A\x94": "部",
    b"\x8A\x95": "ほ",
    b"\x8A\x96": "C",
    b"\x8A\x97": "G",
    b"\x8A\x98": "豊",
    b"\x8A\x99": "教",
    b"\x8A\x9A": "熊",
    b"\x8A\x9B": "秋",
    b"\x8A\x9C": "元",
    b"\x8A\x9D": "純",
    b"\x8A\x9E": "森",
    b"\x8A\x9F": "江",
    b"\x8A\xA0": "渡",
    b"\x8A\xA1": "辺",
    b"\x8A\xA2": "ぞ",
    b"\x8A\xA3": "な",
    b"\x8A\xA4": "渋",
    b"\x8A\xA5": "宗",
    b"\x8A\xA6": "萩",
    b"\x8A\xA7": "原",
    b"\x8A\xA8": "雄",
    b"\x8A\xA9": "遠",
    b"\x8A\xAA": "壮",
    b"\x8A\xAB": "日",
    b"\x8A\xAC": "ァ",
    b"\x8A\xAD": "ワ",
    b"\x8A\xAE": "衣",
    b"\x8A\xAF": "装",
    b"\x8A\xB0": "ば",
    b"\x8A\xB1": "公",
    b"\x8A\xB2": "園",
    b"\x8A\xB3": "ユ",
    b"\x8A\xB4": "ゆ",
    b"\x8A\xB5": "王",
    b"\x8A\xB6": "服",
    b"\x8A\xB7": "剛",
    b"\x8A\xB8": "磯",
    b"\x8A\xB9": "沙",
    b"\x8A\xBA": "邦",
    b"\x8A\xBB": "織",
    b"\x8A\xBC": "広",
    b"\x8A\xBD": "協",
    b"\x8A\xBE": "力",
    b"\x8A\xBF": "義",
    b"\x8A\xC0": "勢",
    b"\x8A\xC1": "飯",
    b"\x8A\xC2": "涼",
    b"\x8A\xC3": "有",
    b"\x8A\xC4": "市",
    b"\x8A\xC5": "量",
    b"\x8A\xC6": "包",
    b"\x8A\xC7": "a",
    b"\x8A\xC8": "p",
    b"\x8A\xC9": "s",
    b"\x8A\xCA": "窪",
    b"\x8A\xCB": "Q",
    b"\x8A\xCC": "菅",
    b"\x8A\xCD": "基",
    b"\x8A\xCE": "常",
    b"\x8A\xCF": "緒",
    b"\x8A\xD0": "峯",
    b"\x8A\xD1": "寿",
    # Best guess
    b"\x8A\xD2": "齋",
    b"\x8A\xD3": "香",
    b"\x8A\xD4": "取",
    b"\x8A\xD5": "鎌",
    b"\x8A\xD6": "昌",
    b"\x8A\xD7": "留",
    b"\x8A\xD8": "暢",
    b"\x8A\xD9": "人",
    b"\x8A\xDA": "七",
    b"\x8A\xDB": "生",
    b"\x8A\xDC": "努",
    b"\x8A\xDD": "鉢",
    b"\x8A\xDE": "琴",
    b"\x8A\xDF": "飛",
    b"\x8A\xE0": "知",
    b"\x8A\xE1": "央",
    b"\x8A\xE2": "範",
    b"\x8A\xE3": "蛭",
    b"\x8A\xE4": "斎",
    b"\x8A\xE5": "穣",
    b"\x8A\xE6": "酒",
    b"\x8A\xE7": "麻",
    b"\x8A\xE8": "廣",
    b"\x8A\xE9": "瀬",
    b"\x8A\xEA": "細",
    b"\x8A\xEB": "夕",
    b"\x8A\xEC": "起",
    b"\x8A\xED": "葉",
    b"\x8A\xEE": "則",
    b"\x8A\xEF": "居",
    b"\x8A\xF0": "沼",
    b"\x8A\xF1": "謙",
    b"\x8A\xF2": "仁",
    b"\x8A\xF3": "功",
    b"\x8A\xF4": "販",
    b"\x8A\xF5": "売",
    b"\x8A\xF6": "企",
    b"\x8A\xF7": "植",
    b"\x8A\xF8": "浅",
    b"\x8A\xF9": "哲",
    b"\x8A\xFA": "亜",
    b"\x8A\xFB": "宮",
    b"\x8A\xFC": "業",
    b"\x8B\x40": "隆",
    b"\x8B\x41": "委",
    b"\x8B\x42": "員",
    b"\x8B\x43": "会",
    b"\x8B\x44": "竜",
    b"\x8B\x45": "理",
    b"\x8B\x46": "径",
    b"\x8B\x47": "緑",
    b"\x8B\x48": "宅",
    b"\x8B\x49": "南",
    b"\x8B\x4A": "徳",
    b"\x8B\x4B": "星",
    b"\x8B\x4C": "哉",
    b"\x8B\x4D": "X",
    b"\x8B\x4E": ",",
    b"\x8B\x4F": ".",
    b"\x8B\x50": "D",
    b"\x8B\x51": "J",
    b"\x8B\x52": "K",
    b"\x8B\x53": "W",
    b"\x8B\x54": "み",
    b"\x8B\x55": "白",
    b"\x8B\x56": "己",
    b"\x8B\x57": "巽",
    b"\x8B\x58": "仲",
    b"\x8B\x59": "記",
    b"\x8B\x5A": "難",
    b"\x8B\x5B": "波",
    b"\x8B\x5C": "優",
    b"\x8B\x5D": "梨",
    b"\x8B\x5E": "馬",
    b"\x8B\x5F": "舞",
    b"\x8B\x60": "夢",
    b"\x8B\x61": "邊",
    b"\x8B\x62": "兼",
    b"\x8B\x63": "克",
    b"\x8B\x64": "志",
    b"\x8B\x65": "ゼ",
    b"\x8B\x66": "桐",
    b"\x8B\x67": "伯",
    b"\x8B\x68": "雅",
    b"\x8B\x69": "雅",
    # Valid encodings seem to end here
}

# Minimal encoding to patch into files with encodings that don't
# contain all the characters we need
BYTES_TO_CHAR_MINIMAL = {
    **BYTES_TO_CHAR_SPECIAL,

    # Numerics
    b"\x88\x9F": "1",
    b"\x88\xA0": "2",
    b"\x88\xA1": "3",
    b"\x88\xA2": "4",
    b"\x88\xA3": "5",
    b"\x88\xA4": "6",
    b"\x88\xA5": "7",
    b"\x88\xA6": "8",
    b"\x88\xA7": "9",
    b"\x88\xA8": "0",

    # Capital letters
    b"\x88\xA9": "A",
    b"\x88\xAA": "B",
    b"\x88\xAB": "C",
    b"\x88\xAC": "D",
    b"\x88\xAD": "E",
    b"\x88\xAE": "F",
    b"\x88\xAF": "G",
    b"\x88\xB0": "H",
    b"\x88\xB1": "I",
    b"\x88\xB2": "J",
    b"\x88\xB3": "K",
    b"\x88\xB4": "L",
    b"\x88\xB5": "M",
    b"\x88\xB6": "N",
    b"\x88\xB7": "O",
    b"\x88\xB8": "P",
    b"\x88\xB9": "Q",
    b"\x88\xBA": "R",
    b"\x88\xBB": "S",
    b"\x88\xBC": "T",
    b"\x88\xBD": "U",
    b"\x88\xBE": "V",
    b"\x88\xBF": "W",
    b"\x88\xC0": "X",
    b"\x88\xC1": "Y",
    b"\x88\xC2": "Z",

    # Lowercase letters
    b"\x88\xC3": "a",
    b"\x88\xC4": "b",
    b"\x88\xC5": "c",
    b"\x88\xC6": "d",
    b"\x88\xC7": "e",
    b"\x88\xC8": "f",
    b"\x88\xC9": "g",
    b"\x88\xCA": "h",
    b"\x88\xCB": "i",
    b"\x88\xCC": "j",
    b"\x88\xCD": "k",
    b"\x88\xCE": "l",
    b"\x88\xCF": "m",
    b"\x88\xD0": "n",
    b"\x88\xD1": "o",
    b"\x88\xD2": "p",
    b"\x88\xD3": "q",
    b"\x88\xD4": "r",
    b"\x88\xD5": "s",
    b"\x88\xD6": "t",
    b"\x88\xD7": "u",
    b"\x88\xD8": "v",
    b"\x88\xD9": "w",
    b"\x88\xDA": "x",
    b"\x88\xDB": "y",
    b"\x88\xDC": "z",

    # Comma and period are two different images unfortunately
    b"\x88\xDD": "ー",
    b"\x88\xDE": "、",
    b"\x88\xDF": "。",
    b"\x88\xE0": "ぁ",

    # Misc punctuation
    b"\x88\xE1": "ヶ",
    b"\x88\xE2": "○",
    b"\x88\xE3": "Χ",
    b"\x88\xE4": "□",
    b"\x88\xE5": "△",
    b"\x88\xE6": "!",
    b"\x88\xE7": "?",
    b"\x88\xE8": "(",
    b"\x88\xE9": ")",
    b"\x88\xEA": "*",
    b"\x88\xEB": "+",
    b"\x88\xEC": "-",
    b"\x88\xED": "&",
    b"\x88\xEE": "@",
    b"\x88\xEF": "⋅",
    b"\x88\xF0": ".",
    b"\x88\xF1": ":",
    b"\x88\xF2": "^",
    b"\x88\xF3": "_",
    b"\x88\xF4": "ω",
    b"\x88\xF5": "~",
    b"\x88\xF6": "➝",
    b"\x88\xF7": " ",
    b"\x88\xF8": "ァ",
    b"\x88\xF9": "'",
    b"\x88\xFA": "\"",
    b"\x88\xFB": "α",
    b"\x88\xFC": "β",
    b"\x89\x40": "γ",  # Lower case gamma
    b"\x89\x41": "/",
    b"\x89\x42": "％",  # Use full width % sign to avoid conflict
    b"\x89\x43": "儀",
    b"\x89\xE1": "♀",
    b"\x89\xE2": "♪",
}

ENCODING_MAP = {
    "C63A0383": BYTES_TO_CHAR_CREDIT,             # gz/credit.gz

    "7243E526": BYTES_TO_CHAR_DEFAULT,            # gz/game_result.story.gz"
    "4ABEE95E": BYTES_TO_CHAR_DEFAULT,            # gz/game_result.vs.gz

    "00940549": BYTES_TO_CHAR_DEFAULT,            # gz/menu_common.gz
    # Pokepi fonts, technically the encoding is
    # slightly different but everything that
    # matters is the same
    "36C5DE03": BYTES_TO_CHAR_DEFAULT,            # ??
    "3943E3CC": BYTES_TO_CHAR_DEFAULT,            # gz/menu_edit_sarubook_in_pipo6.gz

    "87F51E0C": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.01_boss01_gori01.gz
    "41F34892": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.02_city01_a.gz
    "E76BF53F": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.03_city02_a.gz
    "5973AD65": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.04_metro01_a.gz
    "DD798EB5": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.05_boss02_boss.gz
    "44F20E4A": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.06_bay01_a.gz
    "567EA3B3": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.07_bay02_a.gz
    "6CC96933": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.08_park01_a.gz
    "C55668F7": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.09_stadium_a.gz
    "1626079D": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.10_boss03_fly.gz
    "45912030": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.11_hangar01_a.gz
    "C35E32D9": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.12_hangar02_a.gz
    "F2B5553F": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.13_boss04_gori02.gz
    "DCD7B35D": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.14_elevator_a.gz
    "8D3A2011": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.15_kakeru_spector.gz
    "06143EBB": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.15_pipo_spector.gz
    "5EBB5F0B": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.16_kakeru_goritron.gz
    "96696A96": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.16_pipo_kakeru.gz
    "7A8E0451": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.17_city01_vr.gz
    "0E36C98A": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.18_bay01_vr.gz
    "DD45C596": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.19_park01_vr.gz
    "B2B250A8": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.20_boss07_grid.gz
    "4C084C39": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.21_stadium_b.gz
    "CD7A2B3C": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.22_park01_b.gz
    "E13AEF70": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.23_bay02_b.gz
    "49913FD2": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.24_bay01_b.gz
    "A5BEBDA1": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.25_boss08_boss.gz
    "F2BC8CB1": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.26_metro01_b.gz
    "72C71EE8": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.27_city02_b.gz
    "6E0A878D": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.28_city01_b.gz
    "3A1B68CC": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.29_boss09_boss.gz
    "AC56964E": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.30_daiba02_b.gz
    "2F029ADF": BYTES_TO_CHAR_DEFAULT,            # gz/menu_story.31_boss10_boss.gz

    "3C6CF60B": BYTES_TO_CHAR_DEFAULT,            # gz/menu_vs.gz

    "AA6F7A50": BYTES_TO_CHAR_TOKYO_TELEPORT,     # gz/stage.01_boss01_gori01.gz
    "58401EA3": BYTES_TO_CHAR_SHINJUKU,           # gz/stage.02_city01_a.gz
    "FEB8CA50": BYTES_TO_CHAR_AKIHABARA,          # gz/stage.03_city02_a.gz
    "AC9781D4": BYTES_TO_CHAR_METRO,              # gz/stage.04_metro01_a.gz
    "B8B90462": BYTES_TO_CHAR_TOKYO,              # gz/stage.05_boss02_boss.gz
    "CF772913": BYTES_TO_CHAR_CONTAINER_TERMINAL, # gz/stage.06_bay01_a.gz
    "E103BE7C": BYTES_TO_CHAR_INDUSTRIAL_COMPLEX, # gz/stage.07_bay02_a.gz
    "83163F44": BYTES_TO_CHAR_YACHT_HARBOR,       # gz/stage.08_park01_a.gz
    "187B3C66": BYTES_TO_CHAR_09_STADIUM_A,       # gz/stage.09_stadium_a.gz
    "1566B0A1": BYTES_TO_CHAR_10_BOSS03_FLY,      # gz/stage.10_boss03_fly.gz
    "44D1C934": BYTES_TO_CHAR_11_HANGAR01_A,      # gz/stage.11_hangar01_a.gz
    "C29EDBDD": BYTES_TO_CHAR_12_HANGAR02_A,      # gz/stage.12_hangar02_a.gz
    "1530B183": BYTES_TO_CHAR_13_BOSS04_GORI02,   # gz/stage.13_boss04_gori02.gz
    "DB175D62": BYTES_TO_CHAR_14_ELEVATOR_A,      # gz/stage.14_elevator_a.gz
    "9CE158F2": BYTES_TO_CHAR_15_KAKERU_SPECTOR,  # gz/stage.15_kakeru_spector.gz
    "AD4E55A8": BYTES_TO_CHAR_15_PIPO_SPECTOR,    # gz/stage.15_pipo_spector.gz
    "89E08F98": BYTES_TO_CHAR_16_KAKERU_GORITRON, # gz/stage.16_kakeru_goritron.gz
    "71A9E042": BYTES_TO_CHAR_16_PIPO_KAKERU,     # gz/stage.16_pipo_kakeru.gz
    "CDB2D8BF": BYTES_TO_CHAR_17_CITY01_VR,       # gz/stage.17_city01_vr.gz
    "25839E9B": BYTES_TO_CHAR_18_BAY01_VR,        # gz/stage.18_bay01_vr.gz
    "306A9905": BYTES_TO_CHAR_19_PARK01_VR,       # gz/stage.19_park01_vr.gz
    "8DF2C654": BYTES_TO_CHAR_20_BOSS07_GRID,     # gz/stage.20_boss07_grid.gz
    "9F2C20A8": BYTES_TO_CHAR_21_STADIUM_B,       # gz/stage.21_stadium_b.gz
    "E4C7004D": BYTES_TO_CHAR_22_PARK01_B,        # gz/stage.22_park01_b.gz
    "6CC0093A": BYTES_TO_CHAR_23_BAY02_B,         # gz/stage.23_bay02_b.gz
    "D4165A9B": BYTES_TO_CHAR_24_BAY01_B,         # gz/stage.24_bay01_b.gz
    "80FE334E": BYTES_TO_CHAR_25_BOSS08_BOSS,     # gz/stage.25_boss08_boss.gz
    "45E16020": BYTES_TO_CHAR_26_METRO01_B,       # gz/stage.26_metro01_b.gz
    "8914F4F8": BYTES_TO_CHAR_27_CITY02_B,        # gz/stage.27_city02_b.gz
    "85575C9E": BYTES_TO_CHAR_28_CITY01_B,        # gz/stage.28_city01_b.gz
    "155BDE78": BYTES_TO_CHAR_29_BOSS09_BOSS,     # gz/stage.29_boss09_boss.gz
    "FF7A6ABD": BYTES_TO_CHAR_30_DAIBA02_B,       # gz/stage.30_daiba02_b.gz
    "0A42108C": BYTES_TO_CHAR_31_BOSS10_BOSS,     # gz/stage.31_boss10_boss.gz

    "E5D5CEB1": BYTES_TO_CHAR_50_K1,              # gz/stage.50_k1.gz
    "2FF511D3": BYTES_TO_CHAR_51_CITY,            # gz/stage.51_city.gz
    "1E656E38": BYTES_TO_CHAR_52_METRO,           # gz/stage.52_metro.gz
    "9CE16BE2": BYTES_TO_CHAR_53_BAY,             # gz/stage.53_bay.gz
    "2B6AEDC3": BYTES_TO_CHAR_54_UFO,             # gz/stage.54_UFO.gz
    "48D02DB6": BYTES_TO_CHAR_55_PARK,            # gz/stage.55_park.gz
    "4CDE5405": BYTES_TO_CHAR_56_DAIBA,           # gz/stage.56_daiba.gz
    "EA0603BA": BYTES_TO_CHAR_57_K1DEATH,         # gz/stage.57_k1death.gz
    "1988AE91": BYTES_TO_CHAR_58_UFO2,            # gz/stage.58_UFO2.gz
    "F25230CC": BYTES_TO_CHAR_60_K1,              # gz/stage.60_k1.gz
    "4DBDDE02": BYTES_TO_CHAR_61_K1DEATH,         # gz/stage.61_k1death.gz

    "E1CDD8B9": BYTES_TO_CHAR_DEFAULT,            # gz/victory.col.chall.gz"
    "F1453E5F": BYTES_TO_CHAR_DEFAULT,            # gz/victory.col.goritron.gz"
    "3ED3E300": BYTES_TO_CHAR_DEFAULT,            # gz/victory.col.hakase.gz"
    "65322781": BYTES_TO_CHAR_DEFAULT,            # gz/victory.col.haruka.gz"
    "B7A86C3B": BYTES_TO_CHAR_DEFAULT,            # gz/victory.col.hiroki.gz"
    "143CA181": BYTES_TO_CHAR_DEFAULT,            # gz/victory.col.kakeru.gz"
    "94380757": BYTES_TO_CHAR_DEFAULT,            # gz/victory.col.legend.gz"
    "B67E6C4C": BYTES_TO_CHAR_DEFAULT,            # gz/victory.col.natsumi.gz"
    "03A9E5E7": BYTES_TO_CHAR_DEFAULT,            # gz/victory.col.pipo6.gz"
    "604EDA0F": BYTES_TO_CHAR_DEFAULT,            # gz/victory.col.pipotron.gz"
    "FDE9FCBF": BYTES_TO_CHAR_DEFAULT,            # gz/victory.col.spector.gz"
    "AF89EA93": BYTES_TO_CHAR_DEFAULT,            # gz/victory.col.volcano.gz"

    "54110B1E": BYTES_TO_CHAR_DEFAULT,            # gz/victory.vs.chall.gz
    "38FF898C": BYTES_TO_CHAR_DEFAULT,            # gz/victory.vs.goritron.gz
    "DD92277C": BYTES_TO_CHAR_DEFAULT,            # gz/victory.vs.hakase.gz
    "04F26AFC": BYTES_TO_CHAR_DEFAULT,            # gz/victory.vs.haruka.gz
    "5668B0B6": BYTES_TO_CHAR_DEFAULT,            # gz/victory.vs.hiroki.gz
    "B3FBE4FC": BYTES_TO_CHAR_DEFAULT,            # gz/victory.vs.kakeru.gz
    "33F84AD2": BYTES_TO_CHAR_DEFAULT,            # gz/victory.vs.legend.gz
    "B130371D": BYTES_TO_CHAR_DEFAULT,            # gz/victory.vs.natsumi.gz
    "76EC174C": BYTES_TO_CHAR_DEFAULT,            # gz/victory.vs.pipo6.gz
    "A707263D": BYTES_TO_CHAR_DEFAULT,            # gz/victory.vs.pipotron.gz
    "F89BC790": BYTES_TO_CHAR_DEFAULT,            # gz/victory.vs.spector.gz
    "AA3BB564": BYTES_TO_CHAR_DEFAULT,            # gz/victory.vs.volcano.gz
}

ENCODING_LIMITS = {
    "7243E526": None,  # gz/game_result.story.gz"
    "4ABEE95E": None,  # gz/game_result.vs.gz

    "00940549": None,  # gz/menu_common.gz
    "36C5DE03": None,  # ??
    "3943E3CC": None,  # gz/menu_edit_sarubook_in_pipo6.gz

    "87F51E0C": None,  # gz/menu_story.01_boss01_gori01.gz
    "41F34892": None,  # gz/menu_story.02_city01_a.gz
    "E76BF53F": None,  # gz/menu_story.03_city02_a.gz
    "5973AD65": None,  # gz/menu_story.04_metro01_a.gz
    "DD798EB5": None,  # gz/menu_story.05_boss02_boss.gz
    "44F20E4A": None,  # gz/menu_story.06_bay01_a.gz
    "567EA3B3": None,  # gz/menu_story.07_bay02_a.gz
    "6CC96933": None,  # gz/menu_story.08_park01_a.gz
    "C55668F7": None,  # gz/menu_story.09_stadium_a.gz
    "1626079D": None,  # gz/menu_story.10_boss03_fly.gz
    "45912030": None,  # gz/menu_story.11_hangar01_a.gz
    "C35E32D9": None,  # gz/menu_story.12_hangar02_a.gz
    "F2B5553F": None,  # gz/menu_story.13_boss04_gori02.gz
    "DCD7B35D": None,  # gz/menu_story.14_elevator_a.gz
    "8D3A2011": None,  # gz/menu_story.15_kakeru_spector.gz
    "06143EBB": None,  # gz/menu_story.15_pipo_spector.gz
    "5EBB5F0B": None,  # gz/menu_story.16_kakeru_goritron.gz
    "96696A96": None,  # gz/menu_story.16_pipo_kakeru.gz
    "7A8E0451": None,  # gz/menu_story.17_city01_vr.gz
    "0E36C98A": None,  # gz/menu_story.18_bay01_vr.gz
    "DD45C596": None,  # gz/menu_story.19_park01_vr.gz
    "B2B250A8": None,  # gz/menu_story.20_boss07_grid.gz
    "4C084C39": None,  # gz/menu_story.21_stadium_b.gz
    "CD7A2B3C": None,  # gz/menu_story.22_park01_b.gz
    "E13AEF70": None,  # gz/menu_story.23_bay02_b.gz
    "49913FD2": None,  # gz/menu_story.24_bay01_b.gz
    "A5BEBDA1": None,  # gz/menu_story.25_boss08_boss.gz
    "F2BC8CB1": None,  # gz/menu_story.26_metro01_b.gz
    "72C71EE8": None,  # gz/menu_story.27_city02_b.gz
    "6E0A878D": None,  # gz/menu_story.28_city01_b.gz
    "3A1B68CC": None,  # gz/menu_story.29_boss09_boss.gz
    "AC56964E": None,  # gz/menu_story.30_daiba02_b.gz
    "2F029ADF": None,  # gz/menu_story.31_boss10_boss.gz

    "3C6CF60B": None,  # gz/menu_vs.gz

    "AA6F7A50": 96,    # gz/stage.01_boss01_gori01.gz
    "58401EA3": 96,    # gz/stage.02_city01_a.gz
    "FEB8CA50": 96,    # gz/stage.03_city02_a.gz
    "AC9781D4": 96,    # gz/stage.04_metro01_a.gz
    "B8B90462": 96,    # gz/stage.05_boss02_boss.gz
    "CF772913": 96,    # gz/stage.06_bay01_a.gz
    "E103BE7C": 96,    # gz/stage.07_bay02_a.gz
    "83163F44": 96,    # gz/stage.08_park01_a.gz
    "187B3C66": 96,    # gz/stage.09_stadium_a.gz
    "1566B0A1": 96,    # gz/stage.10_boss03_fly.gz
    "44D1C934": 96,    # gz/stage.11_hangar01_a.gz
    "C29EDBDD": 96,    # gz/stage.12_hangar02_a.gz
    "1530B183": 96,    # gz/stage.13_boss04_gori02.gz
    "DB175D62": 96,    # gz/stage.14_elevator_a.gz
    "9CE158F2": 96,    # gz/stage.15_kakeru_spector.gz
    "AD4E55A8": 96,    # gz/stage.15_pipo_spector.gz
    "89E08F98": 96,    # gz/stage.16_kakeru_goritron.gz
    "71A9E042": 96,    # gz/stage.16_pipo_kakeru.gz
    "CDB2D8BF": 96,    # gz/stage.17_city01_vr.gz
    "25839E9B": 96,    # gz/stage.18_bay01_vr.gz
    "306A9905": 96,    # gz/stage.19_park01_vr.gz
    "8DF2C654": 96,    # gz/stage.20_boss07_grid.gz
    "9F2C20A8": 96,    # gz/stage.21_stadium_b.gz
    "E4C7004D": 96,    # gz/stage.22_park01_b.gz
    "6CC0093A": 96,    # gz/stage.23_bay02_b.gz
    "D4165A9B": 96,    # gz/stage.24_bay01_b.gz
    "80FE334E": 96,    # gz/stage.25_boss08_boss.gz
    "45E16020": 96,    # gz/stage.26_metro01_b.gz
    "8914F4F8": 96,    # gz/stage.27_city02_b.gz
    "85575C9E": 96,    # gz/stage.28_city01_b.gz
    "155BDE78": 96,    # gz/stage.29_boss09_boss.gz
    "FF7A6ABD": 96,    # gz/stage.30_daiba02_b.gz
    "0A42108C": 96,    # gz/stage.31_boss10_boss.gz

    "E5D5CEB1": 96,    # gz/stage.50_k1.gz
    "2FF511D3": 96,    # gz/stage.51_city.gz
    "1E656E38": 96,    # gz/stage.52_metro.gz
    "9CE16BE2": 96,    # gz/stage.53_bay.gz
    "2B6AEDC3": 96,    # gz/stage.54_UFO.gz
    "48D02DB6": 96,    # gz/stage.55_park.gz
    "4CDE5405": 96,    # gz/stage.56_daiba.gz
    "EA0603BA": 96,    # gz/stage.57_k1death.gz
    "1988AE91": 96,    # gz/stage.58_UFO2.gz
    "F25230CC": 96,    # gz/stage.60_k1.gz
    "4DBDDE02": 96,    # gz/stage.61_k1death.gz

    "E1CDD8B9": None,  # gz/victory.col.chall.gz"
    "F1453E5F": None,  # gz/victory.col.goritron.gz"
    "3ED3E300": None,  # gz/victory.col.hakase.gz"
    "65322781": None,  # gz/victory.col.haruka.gz"
    "B7A86C3B": None,  # gz/victory.col.hiroki.gz"
    "143CA181": None,  # gz/victory.col.kakeru.gz"
    "94380757": None,  # gz/victory.col.legend.gz"
    "B67E6C4C": None,  # gz/victory.col.natsumi.gz"
    "03A9E5E7": None,  # gz/victory.col.pipo6.gz"
    "604EDA0F": None,  # gz/victory.col.pipotron.gz"
    "FDE9FCBF": None,  # gz/victory.col.spector.gz"
    "AF89EA93": None,  # gz/victory.col.volcano.gz"

    "54110B1E": None,  # gz/victory.vs.chall.gz
    "38FF898C": None,  # gz/victory.vs.goritron.gz
    "DD92277C": None,  # gz/victory.vs.hakase.gz
    "04F26AFC": None,  # gz/victory.vs.haruka.gz
    "5668B0B6": None,  # gz/victory.vs.hiroki.gz
    "B3FBE4FC": None,  # gz/victory.vs.kakeru.gz
    "33F84AD2": None,  # gz/victory.vs.legend.gz
    "B130371D": None,  # gz/victory.vs.natsumi.gz
    "76EC174C": None,  # gz/victory.vs.pipo6.gz
    "A707263D": None,  # gz/victory.vs.pipotron.gz
    "F89BC790": None,  # gz/victory.vs.spector.gz
    "AA3BB564": None,  # gz/victory.vs.volcano.gz

}

class EncodingTranslator:
    def __init__(self, hash: str|None=None, encoding: dict[bytes,str]|None=None):
        if isinstance(hash, str):
            hash = hash.upper()
        if encoding:
            self.bytes_to_char = encoding
        else:
            self.bytes_to_char = ENCODING_MAP[hash]

        self.char_to_bytes = defaultdict(list)
        for b, c in self.bytes_to_char.items():
            self.char_to_bytes[c].append(b)

        try:
            # There's no English comma, use the Japanese one
            self.char_to_bytes[","] = self.char_to_bytes["、"]
        except KeyError:
            pass

    def string_to_bytes(self, s: str) -> list[bytes]:
        def recurse(idx: int):
            #if s == "Accept %?":
            #    import pdb;pdb.set_trace()

            if idx == len(s):
                return [b""]

            char = s[idx]
            encodings = []
            for bin_val in self.char_to_bytes[char]:
                for suffix in recurse(idx + 1):
                    encodings.append(bin_val + suffix)
            return encodings
        out = recurse(0)
        return out

    def tokenize_string(self, string: bytes) -> list[bytes]:
        idx = 0
        out = []
        while idx < len(string):
            byte = string[idx:idx+1]
            token = string[idx:idx+2]
            #print(idx)
            #print(byte.hex())
            #print(token.hex())

            if token in self.bytes_to_char:
                out.append(token)
                #print("TOKEN IS VALID")
                #print(self.bytes_to_char[token], end="")
                idx += 2
                continue
            if byte in self.bytes_to_char:
                out.append(byte)
                #print("BYTE IS VALID")
                #print(self.bytes_to_char[byte], end="")
                idx += 1
                continue
            raise ValueError(
                f"Got invalid byte or token. "
                f"Byte: {byte.hex()}, Token: {token.hex()} "
                f"String: {string.hex(sep=" ")}"
            )
        return out

    def bytes_to_string(self, bts: bytes) -> str:
        tokens = self.tokenize_string(bts)
        return self.tokens_to_string(tokens)

    def tokens_to_string(self, tokens: list[bytes]) -> str:
        return "".join(self.bytes_to_char[token] for token in tokens)

    def wrap_string(self, bs: bytes, id: bytes|None = None) -> bytes:
        out = (
            int(len(bs)).to_bytes(length=4, byteorder="little")
            + bs
            + b"\x00"
        )
        if id:
            return id + out
        return out

    def is_kanji(self, c: bytes | str) -> bool:
        # Assume unknown chars are kanji
        if c == "??":
            return True
        if isinstance(c, bytes):
            c = self.bytes_to_char[c]
        if len(c) != 1:
            raise ValueError(
                f"Can't check whether multichar string '{c}' is a kanji"
            )
        # Check if the character is in the range of CJK Unified Ideographs
        return (
            '\u4E00' <= c <= '\u9FFF' or  # CJK Unified Ideographs
            '\u3400' <= c <= '\u4DBF' or  # CJK Unified Ideographs Extension A
            '\uF900' <= c <= '\uFAFF'     # CJK Compatibility Ideographs
        )
def _token_to_offset(token: bytes) -> int:
    # Not sure what this actually does, but it's copied from the decomp
    if len(token) != 2:
        raise ValueError(f"Invalid token {token}")
    token_int = int.from_bytes(token, byteorder="big")

    group = token_int >> 8
    group_base = group - 0x81
    item = token_int & 0xFF
    if 0x1E < group_base:
        group_base = group
        if group - 0xE0 < 0x10:
            group_base = group - 0xC1
    group_base_2 = group_base*2
    group_base = item - 0x40
    if 0x3E < group_base:
        if item - 0x80 < 0x1F:
            group_base = item - 0x41
        else:
            var1 = item - 0x9F
            group_base = item
            if var1 < 0x5E:
                group_base_2 += 1
                group_base = var1
    out = (group_base_2 + 1)*0x100 + group_base + 0x2021
    return out

def token_to_idx(token: bytes) -> int:
    # Copied from decomp
    ofst = _token_to_offset(token)
    out = (ofst >> 8) * 0x5e + (ofst & 0xff) - 0x11C1
    if out < 0:
        out = 0
    return out

# HACK: precompute idx to token
# Ideally we would just have a function calculate this on the fly
# but I can't be bothered to reverse it
_idx_to_token = None
def idx_to_token(idx: int) -> bytes:
    global _idx_to_token
    if _idx_to_token is None:
        _idx_to_token = {
            token_to_idx(token): token
            for token in BYTES_TO_CHAR_DEFAULT.keys()
            if token not in BYTES_TO_CHAR_SPECIAL.keys()
        }
    return _idx_to_token[idx]

def build_credits_bin(
    credits: list[dict[str,str]],
    translator: EncodingTranslator | None=None
) -> bytes:
    if translator is None:
        translator = EncodingTranslator(encoding=BYTES_TO_CHAR_MINIMAL)
    num_lines = len(credits)
    out = b""
    out += num_lines.to_bytes(4, "little")
    for idx, line in enumerate(credits):
        out += idx.to_bytes(4, "little")
        string: str = line["string"]
        tab_mode: int = line["tab_mode"]
        raw: bool = line.get("raw", False)
        str_len_raw = len(string)
        out += tab_mode.to_bytes(4, "little")
        if str_len_raw != 0:
            if not raw:
                try:
                    str_encoded = translator.string_to_bytes(string)[0]
                except IndexError:
                    string_unk = "?"*str_len_raw
                    print(
                        f"Warning: could not encode '{string}', using "
                        f"'{string_unk}' instead"
                    )
                    str_encoded = translator.string_to_bytes(string_unk)[0]
            else:
                str_encoded = string.encode("utf-8")
            str_len = len(str_encoded)
            out += str_len.to_bytes(4, "little")
            out += str_encoded
            out += b"\x00"
        else:
            # set str_len to 0, no string is encoded
            out += (0).to_bytes(4, "little")

    return out
